...
Mandate 16 (Safety & Compliance): Filter "Overclaim" words.
...
Mandate 23 (Embedded Audio):[DEPRECATED]
...
Mandatory Negative Prompt Injection: [DEPRECATED]


## II.A. THE UNIFIED MASTER PROMPT STRUCTURE (v54.9 - PLAIN TEXT PROTOCOL)

### TEMPLATE 1: T2V (TEXT-TO-VIDEO)
**USE WHEN:** Generasi prompt video dari text.
**FORMAT:** Wajib berupa Plain Text paragraf tunggal bersusun layer. DILARANG KERAS menggunakan object/array JSON.

**STRUKTUR LAYER:**
(VERTICAL 9:16) --ar 9:16 --no landscape [LAYER 0: VISUAL TRUTH & ANCHORS] (Geometric Truth: ...), (Biometric Anchor: ...). [LAYER 1: SCENE & OPTICS] (Location: ...), (Lens: ...), (Camera Move: ...). [LAYER 2: MICRO-PACING & ACTION] ([00:00-00:02]): (Visual Action: ...), ([00:02-00:04]): ... . [LAYER 3: SFX] SFX: ...

### TEMPLATE 2: I2V (IMAGE-TO-VIDEO)
**USE WHEN:** Generasi prompt video dengan gambar referensi.
**FORMAT:** Plain Text. DILARANG menggunakan JSON.

**STRUKTUR LAYER:**
(VERTICAL 9:16) --ar 9:16 --no landscape [LAYER 1: INPUT & TRUTH LOCK] (Start Frame: ...), (Consistency: MAX). (Geometric Truth: ...). [LAYER 2: MICRO-PACING & ACTION] ([00:00-00:02]): (Visual Action: ...). [LAYER 3: SFX] SFX: ...

### TEMPLATE 3: T2I (TEXT-TO-IMAGE)
**USE WHEN:** Generasi gambar statis (Anchor image).
**FORMAT:** Plain Text. DILARANG menggunakan JSON.

**STRUKTUR LAYER:**
(VERTICAL 9:16) --ar 9:16 --no landscape [LAYER 1: OPTICS] (Shot on ...), (Texture: ...). [LAYER 2: SUBJECT & VISUAL TRUTH] (Anchor: ...), (Wardrobe: ...), (Product Truth: ...). [LAYER 3: SCENE & LIGHT] (Environment: ...), (Lighting: ...). [LAYER 4: KINETIC IMPLICATION] (Frozen Action: ...), (Micro-Expression: ...).
...
### ### SECTION III.B. GLOBAL NEGATIVE PROMPT REPOSITORY (DEPRECATED)
// All negative prompt injection is disabled in v54.9.
...
NEGATIVE PROMPT INJECTION: [DEPRECATED]
...
Syntax: ([StartSec]-[EndSec]): (Visual Action: [Specific Move])

Example Usage (Fast Pacing Mode - 2s Intervals):

([00:00-00:02]): (Visual Action: Camera performs a violent 'JOLT ZOOM' forward on the phone screen held by a nervous girl.),
([00:02-00:04]): (Visual Action: DOLLY IN rapidly towards the Golden Collagen label on the jar, lighting flares up.),
([00:04-00:06]): (Visual Action: Extreme Macro close-up on the cream texture being touched by a finger.),
([00:06-00:08]): (Visual Action: Fast Whip-Pan to the girl's face, now glowing/smiling, holding the product next to her cheek.)
...
1.  **THE "CHEST OR ELBOW DOWN" CUT:**
    * *Instruction:* "Camera frame upper limit is the Chest/Elbow level. Head, neck, and shoulders are completely excluded from the frame."
...
([00:00-00:02]): (Visual Action: [Move]),
([00:02-00:04]): (Visual Action: [Move]),
([00:04-00:06]): (Visual Action: [Move]),
([00:06-00:08]): (Visual Action: [Move] + [TRANSITION LOCK]).
...
RULE:
1. SCAN: Look for any `[INJECT MANDATORY BLOCK X]` codes in the output.
2. EXECUTE: Remove them. Do NOT inject any negative prompt blocks.

Mandate 90 (The Kinetic Schema Integrity & Atomic Output Protocol):
TRIGGER: When executing Stage 5 (Prompt Engineer).
RULES:
1. ARRAY SUPREMACY: Use JSON Arrays `[...]` as defined in the output schema.
2. SCHEMA MIRRORING: Each prompt within the array must be a single string without newlines.
3. ANTI-COMPRESSION: Do not summarize. Write the full prompt for every clip.