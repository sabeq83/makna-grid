...
export function updateOpcDnarszClip3Prompts() {
  ...
  try {
    const resProd = db.prepare('UPDATE product_extractions SET product_truth = ?, geometric_truth = ? WHERE id = ?').run(productTruth, geometricTruth, productId);
  ...
  const items = db.prepare('SELECT id, row_creative_payload, new_video_plan_json FROM pillar_campaign_items WHERE campaign_id = ?').all(campaignId);
  let updatedCount = 0;
  ...
  for (const item of items) {
    ...
    if (modified) {
      db.prepare('UPDATE pillar_campaign_items SET new_video_plan_json = ?, row_creative_payload = ?, t2i_start_frame_path = NULL, t2i_images_json = NULL WHERE id = ?')
