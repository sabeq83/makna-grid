...
async function run() {
  ...
  for (const campaignId of targetCampaigns) {
    ...
    if (campaignId === 'opc_260726_5vv88b') {
      const p = db.prepare("SELECT * FROM product_extractions WHERE product_name LIKE '%Omura%' ORDER BY id DESC LIMIT 1").get();
      ...
      const p = db.prepare("SELECT * FROM product_extractions WHERE id = ? OR product_name LIKE '%NEZAFIT%' ORDER BY id DESC LIMIT 1").get(campaign.target_product_id);
    ...
    if (studioPath) {
      db.prepare('UPDATE pillar_campaigns SET product_ref_image_path = ? WHERE id = ?').run(studioPath, campaignId);
    ...
    for (const item of items) {
      ...
      for (const clipNum of bridgeClips) {
        ...
        try {
          ...
          db.prepare('UPDATE pillar_campaign_items SET t2i_start_frame_path = ? WHERE id = ?').run(relativeStartFramePath, item.id);
