...
async function populateSiasatSehat() {
  ...
  const brand = db.prepare('SELECT id FROM brand_profiles WHERE LOWER(brand_name) = ?').get(BRAND_NAME);
  if (brand) {
    db.prepare('UPDATE re_campaigns SET brand_profile_id = ? WHERE id = ?').run(brand.id, CAMPAIGN_ID);
    console.log(`✓ Updated re_campaigns.brand_profile_id = "${brand.id}"`);
  }

  // 2. Get all RE items for this campaign
  const reItems = db.prepare('SELECT * FROM re_campaign_items WHERE campaign_id = ? ORDER BY id ASC').all(CAMPAIGN_ID);
