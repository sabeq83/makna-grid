...
const campaign = db.prepare('SELECT id, campaign_name FROM pillar_campaigns WHERE campaign_name LIKE ? OR id LIKE ?').get(campaignPattern, campaignPattern);
if (!campaign) {
  console.error('❌ Campaign opc_260726_1xk9de not found!');
  process.exit(1);
}

db.prepare('UPDATE pillar_campaigns SET target_product_id = ? WHERE id = ?').run(targetProductId, campaign.id);
console.log(`✅ Linked target_product_id=${targetProductId} to campaign [${campaign.id}]`);

// 2. Update items from row 1 to 18
const items = db.prepare('SELECT id, result_json FROM pillar_campaign_items WHERE campaign_id = ? ORDER BY id ASC').all(campaign.id);
...
for (let i = 0; i < items.length; i++) {
  ...
  try {
    ...
    db.prepare('UPDATE pillar_campaign_items SET result_json = ? WHERE id = ?').run(JSON.stringify(res), item.id);
    repairedCount++;
