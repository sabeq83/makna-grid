...
async function syncProductsToNode1() {
  ...
  execSync('scp ./scratch_products.json makna-ui:~/makna-grid/scratch_products.json', { stdio: 'inherit' });
...
const columnsInfo = db.prepare("PRAGMA table_info(product_extractions)").all();
...
const stmt = db.prepare('INSERT OR REPLACE INTO product_extractions (' + colNamesStr + ') VALUES (' + placeholdersStr + ')');
...
`;
  ...
  execSync('scp ./scratch_import_remote.js makna-ui:~/makna-grid/scratch_import_remote.js', { stdio: 'inherit' });
  fs.unlinkSync('./scratch_import_remote.js');

  execSync('ssh -o ConnectTimeout=10 makna-ui "cd ~/makna-grid && export PATH=/home/sabeqmursyid/.local/bin:$PATH && node scratch_import_remote.js && rm scratch_import_remote.js"', { stdio: 'inherit' });
