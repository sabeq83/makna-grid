...
export function repairOpcDnarszPhoto() {
  ...
  const resCamp = db.prepare('UPDATE pillar_campaigns SET product_ref_image_path = ? WHERE id = ?').run(targetCleanPath, campaignId);
  ...
  const items = db.prepare('SELECT id, row_creative_payload FROM pillar_campaign_items WHERE campaign_id = ?').all(campaignId);
  let updatedItems = 0;
  ...
  for (const item of items) {
    if (item.row_creative_payload) {
      try {
        ...
        db.prepare('UPDATE pillar_campaign_items SET row_creative_payload = ?, t2i_start_frame_path = NULL, t2i_images_json = NULL WHERE id = ?').run(JSON.stringify(payload), item.id);
        updatedItems++;
      } catch (e) {
