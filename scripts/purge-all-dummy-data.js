...
export async function purgeAllDummyData() {
  ...
  try {
    const resBp = db.prepare("DELETE FROM brand_profiles WHERE id LIKE 'dummybrand%' OR brand_name LIKE 'dummybrand%'").run();
    const resUb = db.prepare("DELETE FROM user_brands WHERE brand_id LIKE 'dummybrand%'").run();
