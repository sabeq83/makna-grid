...
async function run() {
  ...
  const product = db.prepare("SELECT * FROM product_extractions WHERE product_name LIKE '%NEZAFIT%' OR product_name LIKE '%Daun Jati%' ORDER BY created_at DESC LIMIT 1").get();
  ...
  if (productRefPath) {
    db.prepare('UPDATE pillar_campaigns SET product_ref_image_path = ? WHERE id = ?').run(productRefPath, campaignId);
