...
async function main() {
  ...
  try {
    ...
    const delRes2 = db.prepare("DELETE FROM content_flow_items WHERE id LIKE 'cf_nutribake_re_%' OR id LIKE 'cf_re_%';").run();
    console.log(` ✅ [SQLite] Deleted rows: ${delRes2.changes}`);

    console.log('🖥️ [SQLite] Converting all account names to lowercase...');
    const updateRes2 = db.prepare("UPDATE content_flow_items SET account_name = LOWER(account_name) WHERE account_name IS NOT NULL;").run();
