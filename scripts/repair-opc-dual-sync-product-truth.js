...
for (const config of campaignsToRepair) {
  const campaign = db.prepare('SELECT id, campaign_name FROM pillar_campaigns WHERE campaign_name LIKE ? OR id LIKE ?').get(config.pattern, config.pattern);
  if (!campaign) {
  ...
  // 1. Link product_id
  db.prepare('UPDATE pillar_campaigns SET target_product_id = ? WHERE id = ?').run(config.productId, campaign.id);
  console.log(`\n✅ Linked target_product_id=${config.productId} to campaign [${campaign.id}]`);

  // 2. Fetch items
  const items = db.prepare('SELECT id, result_json, new_video_plan_json FROM pillar_campaign_items WHERE campaign_id = ? ORDER BY id ASC').all(campaign.id);
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (updatedNewPlan || updatedResult) {
      db.prepare('UPDATE pillar_campaign_items SET new_video_plan_json = ?, result_json = ? WHERE id = ?')
