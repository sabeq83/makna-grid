...
async function run() {
  ...
  const campaigns = db.prepare("SELECT id, campaign_name FROM pillar_campaigns WHERE status IN ('running', 'pending', 'completed') ORDER BY created_at DESC LIMIT 5").all();
  ...
  for (const campaign of campaigns) {
    ...
    const items = db.prepare("SELECT id, result_json, tiktok_safe_voiceover, original_voiceover, compliance_status FROM pillar_campaign_items WHERE campaign_id = ?").all(campaign.id);
