...
async function runImport() {
  ...
  const existingCamp = db.prepare('SELECT id FROM re_campaigns WHERE id = ?').get(campaign.id);
  if (existingCamp) {
    db.prepare(`
  ...
  for (let idx = 0; idx < items.length; idx++) {
    ...
    const existingItem = db.prepare('SELECT id FROM re_campaign_items WHERE id = ?').get(item.id);
    const itemData = {
      id: item.id,
  ...
  const localCount = db.prepare('SELECT COUNT(*) as count FROM re_campaign_items WHERE campaign_id = ?').get(campaign.id).count;
  const cfCount = db.prepare('SELECT COUNT(*) as count FROM content_flow_items WHERE source_campaign_id = ?').get(campaign.id).count;
