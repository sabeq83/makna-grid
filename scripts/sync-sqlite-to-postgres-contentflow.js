...
const sqliteItems = db.prepare(`SELECT * FROM content_flow_items WHERE source_type = 'opc'`).all();
...
for (const item of sqliteItems) {
  ...
  if (!stdVideoId || stdVideoId.startsWith('VID-')) {
    ...
    db.prepare('UPDATE content_flow_items SET video_id = ? WHERE id = ?').run(stdVideoId, item.id);
  }
