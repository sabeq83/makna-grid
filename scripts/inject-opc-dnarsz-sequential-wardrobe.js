...
export function injectOpcDnarszSequentialWardrobe() {
  ...
  const items = db.prepare('SELECT id, row_creative_payload, new_video_plan_json FROM pillar_campaign_items WHERE campaign_id = ? ORDER BY id ASC').all(campaignId);
  let updatedCount = 0;
  ...
  items.forEach((item, index) => {
    ...
    if (modified) {
      db.prepare('UPDATE pillar_campaign_items SET new_video_plan_json = ?, row_creative_payload = ? WHERE id = ?')
