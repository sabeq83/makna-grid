...
export default function SettingsPage() {
  ...
  const [localStoragePath, setLocalStoragePath] = useState('renders');
  ...
  const [driveGlabsFolderId, setDriveGlabsFolderId] = useState('');
  const [driveReMarkdownFolderId, setDriveReMarkdownFolderId] = useState('');
  const [masterReSheetId, setMasterReSheetId] = useState('');
  const [driveProductPhotoFolder, setDriveProductPhotoFolder] = useState('_fotoproduk');
  ...
  const [contentflowApiKey, setContentflowApiKey] = useState('');
  const [maskedContentflowKey, setMaskedContentflowKey] = useState('');
  const [hasContentflowKey, setHasContentflowKey] = useState(false);
  const [editingContentflow, setEditingContentflow] = useState(false);
  const [contentflowApiUrl, setContentflowApiUrl] = useState('http://100.78.186.123:3001/api/v1/content/ingest');
  const [savingContentflow, setSavingContentflow] = useState(false);
  const [testingContentflow, setTestingContentflow] = useState(false);
  const [contentflowTestResult, setContentflowTestResult] = useState(null);
  ...
  useEffect(() => {
    ...
    // Poll pool status every 10 seconds for real-time stats
  ...
  async function fetchSettings() {
    ...
    if (data.success) {
      ...
      setSaveToLocalStorage(data.data.save_to_local_storage === 1);
      setLocalStoragePath(data.data.local_storage_path || 'renders');
      setGeminiApiTier(data.data.gemini_api_tier || 'paid');
      setGeminiContextCaching(data.data.gemini_context_caching || 'on');
      setDriveGlabsFolderId(data.data.drive_glabs_folder_id || '');
      setDriveReMarkdownFolderId(data.data.drive_re_markdown_folder_id || '');
      setMasterReSheetId(data.data.master_re_sheet_id || '');
      setDriveProductPhotoFolder(data.data.drive_product_photo_folder || '_fotoproduk');
      ...
      setMaskedContentflowKey(data.data.contentflow_api_key || '');
      setHasContentflowKey(data.data.has_contentflow_key);
      setContentflowApiUrl(data.data.contentflow_api_url || 'http://100.78.186.123:3001/api/v1/content/ingest');
    }
  }
  ...
  async function saveFbSettings() {
    ...
    try {
      ...
      if (fbPageToken.trim()) body.fb_page_token = fbPageToken;
      const res = await fetch('/api/settings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const data = await res.json();
      if (data.success) {
        showToast('Kredensial Facebook Page berhasil disimpan');
        setFbPageToken('');
        setEditingFb(false);
        setFbTestResult(null);
        fetchSettings();
      } else {
        showToast(data.error || 'Gagal menyimpan kredensial FB', 'error');
      }
    } catch (err) {
  ...
  async function testFbSettings() {
    ...
    try {
      ...
      if (fbPageToken.trim()) body.fb_page_token = fbPageToken;
      if (fbPageIds.trim()) {
        body.fb_page_id = fbPageIds.split(',')[0].trim();
      } else if (fbPageId.trim()) {
        body.fb_page_id = fbPageId.trim();
      }
      ...
      const data = await res.json();
      if (data.success) {
        setFbTestResult({ type: 'success', message: `🟢 ${data.message} (Kategori: ${data.data.category})` });
        if (data.data.page_token) {
          setFbPageToken(data.data.page_token);
        }
      } else {
        setFbTestResult({ type: 'error', message: `🔴 Test Gagal: ${data.error}` });
      }
    } catch (err) {
      setFbTestResult({ type: 'error', message: `🔴 Kesalahan Jaringan: ${err.message}` });
    } finally {
      setTestingFb(false);
    }
  }

  async function saveContentflowSettings() {
    setSavingContentflow(true);
    try {
      const body = { contentflow_api_url: contentflowApiUrl.trim() };
      if (contentflowApiKey.trim()) body.contentflow_api_key = contentflowApiKey.trim();
      const res = await fetch('/api/settings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      ...
      const data = await res.json();
      if (data.success) {
        showToast('Konfigurasi Content Flow berhasil disimpan');
        setContentflowApiKey('');
        setEditingContentflow(false);
        setContentflowTestResult(null);
        fetchSettings();
      } else {
        showToast(data.error || 'Gagal menyimpan konfigurasi Content Flow', 'error');
      }
    } catch (err) {
      showToast('Terjadi kesalahan jaringan', 'error');
    } finally {
      setSavingContentflow(false);
    }
  }

  async function testContentflowSettings() {
    setTestingContentflow(true);
    setContentflowTestResult(null);
    try {
      const body = { contentflow_api_url: contentflowApiUrl.trim() };
      if (contentflowApiKey.trim()) body.contentflow_api_key = contentflowApiKey.trim();
      const res = await fetch('/api/settings/test-contentflow', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body)
      });
      const data = await res.json();
      if (data.success) {
        setContentflowTestResult({ type: 'success', message: `🟢 ${data.message}` });
      } else {
        setContentflowTestResult({ type: 'error', message: `🔴 Test Gagal: ${data.error}` });
      }
    } catch (err) {
      setContentflowTestResult({ type: 'error', message: `🔴 Kesalahan Jaringan: ${err.message}` });
    } finally {
      setTestingContentflow(false);
    }
  }
  ...
  async function saveMinimaxSettings() {
    ...
    try {
      ...
      if (minimaxApiKey.trim()) {
        body.minimax_api_key = minimaxApiKey;
      }
      const res = await fetch('/api/settings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const data = await res.json();
      if (data.success) {
        showToast('MiniMax settings saved!');
        setMinimaxApiKey('');
  ...
  async function testConnection() {
    ...
    if (!keyToTest && !hasKey) { showToast('Masukkan API Key terlebih dahulu', 'error'); return; }
    setTesting(true); setTestResult(null);
    try {
      const res = await fetch('/api/settings/test-gemini', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ api_key: keyToTest, use_stored: !keyToTest }),
      });
      const data = await res.json();
      setTestResult(data);
      showToast(data.success ? 'Koneksi berhasil!' : 'Koneksi gagal: ' + data.message, data.success ? 'success' : 'error');
  ...
  async function saveGeminiSettings() {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        showToast('Pengaturan Tier API & Caching berhasil disimpan');
        fetchSettings();
      } else {
        showToast(data.error, 'error');
      }
  ...
  async function uploadFile(file) {
    ...
    formData.append('name', file.name.replace(/\.(md|txt|json)$/, ''));
    try {
      const res = await fetch('/api/kb', { method: 'POST', body: formData });
      const data = await res.json();
      if (data.success) { showToast(`KB "${data.data.name}" berhasil diupload`); fetchKBs(); }
  ...
  async function deleteKB(id, name) {
    if (!confirm(`Hapus Knowledge Base "${name}"?`)) return;
    const res = await fetch('/api/kb', {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
    ...
    const data = await res.json();
    if (data.success) { showToast(`"${name}" berhasil dihapus`); fetchKBs(); }
  }
  ...
  async function seedKBs() {
    ...
    try {
      ...
      const data = await res.json();
      showToast(data.message || 'KB loaded!');
      fetchKBs();
    } catch (e) { showToast(e.message, 'error'); }
    setSeeding(false);
  }
  ...
  function handleDrop(e) {
    ...
    files.forEach(f => { if (f.name.match(/\.(md|txt|json)$/)) uploadFile(f); });
  }
  ...
  async function fetchGoogleStatus() {
    try {
      ...
      const data = await res.json();
      if (data.success) setGoogleStatus(data.data);
    ...
    try {
      ...
      console.error('Failed to fetch scraper session status:', e);
    }
  }
    ...
    try {
      const res = await fetch('/api/settings', {
        ...
        body: JSON.stringify({
          ...
          scraper_use_cdp: 1,
          scraper_chrome_profile: scraperChromeProfile,
          ytdlp_cookies_from_browser: ytdlpCookiesFromBrowser
        })
      });
      const data = await res.json();
      if (data.success) {
        showToast('Pengaturan scraper berhasil disimpan');
        fetchSettings();
      } else {
        showToast(data.error, 'error');
      }
  ...
  async function testCdpConnection() {
    setTestingCdp(true);
    showToast('Menguji koneksi ke Google Chrome di port 9222...', 'success');
    try {
      const res = await fetch('/api/scraper/session/test-cdp');
      const data = await res.json();
      if (data.success) {
        showToast(data.message, 'success');
      } else {
        showToast(data.error || 'Gagal terhubung ke Chrome debugging.', 'error');
      }
    } catch (e) {
  ...
  async function saveGoogleCredentials() {
    if (!googleClientId.trim() || !googleClientSecret.trim()) {
      showToast('Client ID dan Client Secret wajib diisi', 'error');
      return;
    }
      ...
      <main className="main-content">
        <div className="page-container">
          <div className="page-header">
            <h2>⚙ Settings</h2>
            <p>Konfigurasi Knowledge Base dan Gemini API</p>
          </div>

          {/* Gemini API Config */}
          <div className="card" style={{ marginBottom: '24px' }}>
            <div className="card-title"><span className="icon">🔑</span> Gemini API Configuration</div>
            ...
            {/* Gemini API Settings (Tier & Caching) */}
          ...
          {/* MiniMax AI API Config */}
          <div className="card" style={{ marginBottom: '24px' }}>
            <div className="card-title"><span className="icon">🎙️</span> MiniMax AI Configuration</div>
          ...
          <div className="card" style={{ marginTop: '24px' }}>
            <div className="card-title"><span className="icon">🔗</span> Google Workspace Integration</div>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.82rem', marginBottom: '16px', lineHeight: 1.6 }}>
              Hubungkan Google Account untuk export hasil ke Google Sheets dan Google Docs.
            </p>
          ...
          <div className="card" style={{ marginTop: '24px' }}>
            <div className="card-title"><span className="icon">🌐</span> Webhook API (Veo / Nano Banana)</div>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.82rem', marginBottom: '16px', lineHeight: 1.6 }}>
              Koneksi ke local webhook server untuk generate image & video.
            </p>
            ...
            <div className="form-group" style={{ marginBottom: '16px' }}>
              ...
              <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                Metode pengiriman prompt ke G-Labs saat kampanye pertama kali dijalankan (Phase 1 AI Analyze).
              </div>
            </div>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    const data = await res.json();
                    showToast(data.success ? 'Webhook settings saved!' : data.error, data.success ? 'success' : 'error');
                  } catch (e) { showToast(e.message, 'error'); }
                  setSavingWebhook(false);
                }}
                disabled={savingWebhook}
              >
                {savingWebhook ? '⏳' : '💾'} Save Settings
              </button>
            </div>
          </div>
          ...
          <div className="card" style={{ marginTop: '24px' }}>
            <div className="card-title"><span className="icon">🛍️</span> Marketplace Scraper Settings</div>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.82rem', marginBottom: '16px', lineHeight: 1.6 }}>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '0.74rem', fontWeight: 'bold', color: 'var(--text-primary)', marginBottom: '4px', marginTop: '6px' }}>🐧 Linux / WSL2 (Windows Subsystem for Linux):</div>
              <pre style={{
                padding: '8px 12px', background: 'rgba(0,0,0,0.4)', color: '#00FFCC',
                borderRadius: '4px', fontSize: '0.72rem', overflowX: 'auto', border: '1px solid var(--border)',
                margin: '4px 0 6px 0', fontFamily: 'monospace'
              }}>
                {`google-chrome --headless --disable-gpu --remote-debugging-port=9222 --user-data-dir="~/.config/google-chrome" --profile-directory="${scraperChromeProfile}"`}
              </pre>

              <div style={{ fontSize: '0.68rem', color: 'var(--warning)', marginBottom: '10px', lineHeight: 1.4 }}>
                ⚠️ <b>Penting untuk WSL2:</b> Karena WSL2 berjalan di VM terpisah, Anda harus membuka port forwarding dari Windows ke WSL2. Jalankan perintah berikut di <b>Windows Command Prompt (cmd) sebagai Administrator</b>:
                <pre style={{
                  padding: '4px 8px', background: 'rgba(235,94,85,0.1)', color: 'var(--warning)',
                  borderRadius: '4px', fontSize: '0.68rem', overflowX: 'auto', border: '1px solid rgba(235,94,85,0.3)',
            ...
            <div style={{ marginTop: '16px', marginBottom: '16px' }}>
              ...
              <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '4px', lineHeight: 1.4 }}>
                Pilih browser yang memiliki sesi login aktif ke Facebook/YouTube agar yt-dlp dapat membaca session cookies saat mengunduh video referensi secara otomatis.
              </div>
            </div>
          ...
          <div className="card" style={{ marginTop: '24px' }}>
            <div className="card-title"><span className="icon">☁️</span> Cloud Storage Configuration</div>
            <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: '16px' }}>
              Pilih penyedia penyimpanan utama untuk menyimpan hasil video dan dokumen markdown.
            </p>
            ...
            {storageProvider === 'nextcloud' && (
              <div style={{ padding: '16px', background: 'var(--bg-glass)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)', marginBottom: '16px' }}>
                ...
                <div className="form-group" style={{ marginBottom: '12px' }}>
                  <label className="form-label">Default Target Folder</label>
            ...
            {/* Google Workspace Integration (V3) Configs */}
            <h4 style={{ margin: '16px 0 12px 0', fontSize: '0.85rem', color: 'var(--accent-light)' }}>{'>'} Google Workspace Folder & Sheet Setup</h4>
            <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: '16px' }}>
              Konfigurasi folder Google Drive dan Google Sheets untuk sinkronisasi otomatis.
            </p>

            <div className="form-group" style={{ marginBottom: '12px' }}>
              <label className="form-label">Drive Folder ID — Video Final (GLabs)</label>
              <input
                className="form-input"
                placeholder="Kosongkan untuk auto-create 'RE Videos'"
                id="drive_glabs_folder_id"
                value={driveGlabsFolderId}
                onChange={e => setDriveGlabsFolderId(e.target.value)}
              />
              <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                Folder khusus untuk video final hasil generate. Ambil ID dari URL: drive.google.com/drive/folders/<strong>ID_DISINI</strong>
              </div>
            </div>

            <div className="form-group" style={{ marginBottom: '12px' }}>
              <label className="form-label">Drive Folder ID — Markdown Export</label>
              <input
                className="form-input"
                placeholder="Kosongkan untuk auto-create 'MAKNA RE Markdown Results'"
                id="drive_re_markdown_folder_id"
                value={driveReMarkdownFolderId}
                onChange={e => setDriveReMarkdownFolderId(e.target.value)}
              />
            </div>

            <div className="form-group" style={{ marginBottom: '12px' }}>
              <label className="form-label">Master RE Sheet ID</label>
              <input
                className="form-input"
                placeholder="Kosongkan untuk auto-create 'MAKNA — Master RE'"
                id="master_re_sheet_id_input"
                value={masterReSheetId}
                onChange={e => setMasterReSheetId(e.target.value)}
              />
              <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                Ambil ID dari URL: docs.google.com/spreadsheets/d/<strong>ID_DISINI</strong>/edit
              </div>
            </div>

            <div className="form-group" style={{ marginBottom: '16px' }}>
              <label className="form-label">Folder Foto Produk Drive</label>
              <input
                className="form-input"
                placeholder="Folder Drive target foto produk (misal: _fotoproduk)"
                value={driveProductPhotoFolder}
                onChange={e => setDriveProductPhotoFolder(e.target.value)}
              />
              <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                Nama folder di Google Drive (di dalam folder root MAKNA Assets) tempat gambar produk diunggah saat ekspor.
              </div>
            </div>

            <div style={{ marginBottom: '16px' }}>
              <label className="form-label" style={{ marginBottom: '8px', display: 'block' }}>📋 Tab Mapping (Fixed)</label>
              <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                {['Riset RE', 'RE Results', 'Storyboard', 'Voiceover', 'Prompts', 'Captions'].map(tab => (
                  <span key={tab} style={{
                    fontSize: '0.72rem', fontFamily: 'var(--font-mono)',
                    background: 'var(--bg-glass)', padding: '4px 10px', borderRadius: '4px',
              ...
              onClick={async () => {
                ...
                try {
                  const body = {
                    ...
                    drive_glabs_folder_id: driveGlabsFolderId.trim(),
                    drive_re_markdown_folder_id: driveReMarkdownFolderId.trim(),
                    master_re_sheet_id: masterReSheetId.trim(),
                    drive_product_photo_folder: driveProductPhotoFolder.trim()
                  };
                  const res = await fetch('/api/settings', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
          ...
          <div className="card" style={{ marginTop: '24px' }}>
            <div className="card-title"><span className="icon">📘</span> Facebook Page Integration</div>
            <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: '16px' }}>
            ...
            <div className="form-group" style={{ marginBottom: '12px' }}>
              ...
              />
              <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                Domain publik server MAKNA Engine Anda. Digunakan oleh Meta untuk menarik file gambar/video dari server lokal Anda ke draf Facebook Page.
              </div>
            </div>
            ...
            <div className="form-group" style={{ marginBottom: '16px', marginTop: '12px' }}>
              ...
              />
              <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                Masukkan ID Halaman secara manual dipisahkan koma jika halaman tidak dapat terdeteksi otomatis dari token di atas akibat batasan App Developer Mode dari Meta.
              </div>
            </div>
          ...
          {/* Content Flow Direct Ingestion API Settings */}
          <div className="card" style={{ marginTop: '24px' }}>
            <div className="card-title"><span className="icon">⚡</span> Content Flow Direct Ingestion API</div>
            <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: '16px' }}>
              Konfigurasi API Ingestion untuk langsung meng-ingest hasil perencanaan (Content Planner) & video final (Strategic Campaign) ke Web App Content Flow.
            </p>

            <div className="form-group" style={{ marginBottom: '16px' }}>
              <label className="form-label">Content Flow Endpoint URL</label>
              <input
                className="form-input"
                placeholder="http://100.78.186.123:3001/api/v1/content/ingest"
                value={contentflowApiUrl}
                onChange={e => setContentflowApiUrl(e.target.value)}
              />
              <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                Base URL endpoint ingestion REST API Content Flow.
              </div>
            </div>

            <div className="form-group" style={{ marginBottom: '16px' }}>
              <label className="form-label">API Key (X-API-Key)</label>
              {hasContentflowKey && !editingContentflow ? (
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  <input className="form-input" value={maskedContentflowKey} readOnly disabled style={{ opacity: 0.7 }} />
                  <button className="btn btn-sm btn-secondary" onClick={() => setEditingContentflow(true)}>Ubah</button>
                </div>
              ) : (
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  <input
                    className="form-input"
                    placeholder="cf_live_XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
                    value={contentflowApiKey}
                    onChange={e => setContentflowApiKey(e.target.value)}
                  />
                  {hasContentflowKey && (
                    <button className="btn btn-sm btn-secondary" onClick={() => { setEditingContentflow(false); setContentflowApiKey(''); }}>Batal</button>
                  )}
                </div>
              )}
            </div>

            {contentflowTestResult && (
              <div style={{
                padding: '12px',
                borderRadius: 'var(--radius-sm)',
                marginBottom: '16px',
                fontSize: '0.8rem',
                background: contentflowTestResult.type === 'success' ? 'rgba(46, 204, 113, 0.15)' : 'rgba(231, 76, 60, 0.15)',
                border: `1px solid ${contentflowTestResult.type === 'success' ? 'rgba(46, 204, 113, 0.4)' : 'rgba(231, 76, 60, 0.4)'}`,
                color: contentflowTestResult.type === 'success' ? '#2ecc71' : '#e74c3c'
              }}>
                {contentflowTestResult.message}
              </div>
            )}

            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
              <button className="btn btn-secondary" onClick={testContentflowSettings} disabled={testingContentflow || (!hasContentflowKey && !contentflowApiKey)}>
                {testingContentflow ? '⏳ Testing...' : '🟢 Test Koneksi Content Flow'}
              </button>
              <button className="btn btn-primary" onClick={saveContentflowSettings} disabled={savingContentflow}>
                {savingContentflow ? '⏳ Menyimpan...' : '💾 Simpan Konfigurasi Content Flow'}
              </button>
            </div>
          </div>
          ...
          {/* Local Storage Configuration */}
          <div className="card" style={{ marginTop: '24px' }}>
            <div className="card-title"><span className="icon">💾</span> Local Storage Configuration</div>
            <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: '16px' }}>
              Simpan salinan hasil render secara lokal di server disk di samping upload cloud.
            </p>
            ...
            <div className="form-group" style={{ marginBottom: '16px' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '0.9rem', color: 'var(--text-primary)' }}>
                <input
                  ...
                  style={{ width: '16px', height: '16px', cursor: 'pointer' }}
                />
                <span>Simpan Hasil Render di Penyimpanan Lokal Server</span>
              </label>
            </div>

            {saveToLocalStorage && (
              <div className="form-group" style={{ marginBottom: '16px' }}>
                <label className="form-label">Direktori Penyimpanan Lokal</label>
                <input
                  className="form-input"
                  placeholder="renders"
                ...
                />
                <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                  Lokasi relatif atau absolut di mana video final akan disalin (contoh: <code>renders</code>).
                </div>
              </div>
            )}
              ...
              onClick={async () => {
                ...
                try {
                  const res = await fetch('/api/settings', {
                    ...
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                      save_to_local_storage: saveToLocalStorage ? 1 : 0,
                      local_storage_path: localStoragePath.trim() || 'renders'
                    }),
                  });
                  const data = await res.json();
                  showToast(data.success ? 'Local storage config saved!' : data.error, data.success ? 'success' : 'error');
                  fetchSettings();
                } catch (e) {
                  showToast(e.message, 'error');
                }
            ...
            >
              {savingLocalConfig ? '⏳' : '💾'} Save Local Config
            </button>
          </div>
        </div>
      </main>
