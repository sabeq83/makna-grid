  ...
  try {
    ...
    for (const vp of videoInputList) {
      if (video_source_type === 'database') {
        const videoEntry = getVideoById(vp);
        if (!videoEntry) {
        ...
        const db = getDb();
        const item = db.prepare('SELECT visual_clip_paths, ffmpeg_output_path, ffmpeg_status, visual_status FROM re_campaign_items WHERE id = ?').get(itemId);
    ...
    if (audio_source_type === 'database') {
      const audioEntry = getVideoById(audio_path);
      if (!audioEntry) {
    ...
    if (audio_source_type !== 'tts_studio') {
      ...
      const batchData = getTtsBatchWithClips(resolvedAudioPath);
      if (!batchData) {
    ...
    createFfmpegStudioJob(jobRecord);
