...
export async function GET() {
  try {
    ...
    const hubUrl = getSetting('cloud_hub_url') || process.env.CLOUD_HUB_URL || 'http://localhost:3001';
    const token = getSetting('secret_cloud_token') || process.env.SECRET_CLOUD_TOKEN || 'makna_cloud_secret_hub_token_2026';
    const interval = getSetting('cloud_sync_interval') || '60';
    
    const lastPollTime = getSetting('cloud_last_poll_time') || '';
    const lastSyncStatus = getSetting('cloud_last_sync_status') || 'idle';
    const lastSyncError = getSetting('cloud_last_sync_error') || '';
...
export async function POST(request) {
  try {
    ...
    if (cloud_sync_enabled !== undefined) {
      ...
      setSetting('cloud_sync_enabled', isEnabled);
    ...
    if (cloud_hub_url !== undefined) {
      setSetting('cloud_hub_url', cloud_hub_url.trim());
    }

    if (secret_cloud_token !== undefined) {
      setSetting('secret_cloud_token', secret_cloud_token.trim());
    }

    if (cloud_sync_interval !== undefined) {
      setSetting('cloud_sync_interval', String(cloud_sync_interval));
