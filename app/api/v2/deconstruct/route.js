...
export async function GET(request) {
  try {
    ...
    if (searchParams.get('assets') === 'true') {
      const db = getDb();
      const assets = db.prepare("SELECT * FROM re_deconstructed_assets WHERE status = 'deconstructed' ORDER BY created_at DESC").all();
      return NextResponse.json({ success: true, assets });
    }
    const batches = getDeconstructBatches();
...
export async function POST(request) {
  try {
    ...
    // Trigger scheduler
    createJob('re_deconstruct', { batch_id: batchId });
