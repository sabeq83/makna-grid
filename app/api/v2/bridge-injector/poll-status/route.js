...
export async function GET(request) {
  try {
    ...
    const db = getDb();
    const campaign = db.prepare('SELECT status FROM bridge_injector_campaigns WHERE id = ?').get(campaignId);

    if (!campaign) {
      return NextResponse.json({ success: false, error: 'Kampanye tidak ditemukan.' }, { status: 404 });
    }

    const output = db.prepare('SELECT * FROM bridge_injector_outputs WHERE campaign_id = ?').get(campaignId);
    let updated = false;
    ...
    const finalCampaign = updated ? db.prepare('SELECT * FROM bridge_injector_campaigns WHERE id = ?').get(campaignId) : campaign;
    const finalOutput = updated ? db.prepare('SELECT * FROM bridge_injector_outputs WHERE campaign_id = ?').get(campaignId) : output;
