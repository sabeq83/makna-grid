...
export async function GET() {
  try {
    ...
    const isSchedulerActiveSetting = getSetting('bridge_injector_scheduler_active');
...
export async function PATCH(request) {
  try {
    ...
    if (schedulerActive !== undefined) {
      setSetting('bridge_injector_scheduler_active', schedulerActive ? 'true' : 'false');
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      db.prepare('UPDATE bridge_injector_campaigns SET status = ? WHERE id = ?').run('failed', campaignId);
...
`;
    ...
    db.prepare('UPDATE bridge_injector_campaigns SET status = ? WHERE id = ?').run('waiting_t2i', campaignId);
