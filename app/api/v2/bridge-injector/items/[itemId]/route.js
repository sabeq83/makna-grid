...
export async function PUT(request, { params }) {
  try {
    ...
    const campaign = db.prepare('SELECT campaign_name FROM bridge_injector_campaigns WHERE id = ?').get(item.campaign_id);
    if (!campaign) {
...
`;
    ...
    db.prepare('UPDATE bridge_injector_items SET injected_script_md_path = ? WHERE id = ?').run(relativeMdPath, itemId);
...
export async function PATCH(request, { params }) {
  try {
    ...
    const item = db.prepare('SELECT id FROM bridge_injector_items WHERE id = ?').get(itemId);
    if (!item) {
      return NextResponse.json({ success: false, error: 'Item tidak ditemukan.' }, { status: 404 });
    }
