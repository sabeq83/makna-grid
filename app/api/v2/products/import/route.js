    ...
    db.transaction(() => {
      for (const product of products) {
        ...
        for (const url of urlsToCheck) {
          if (url) {
            const row = db.prepare('SELECT id FROM product_extractions WHERE input_source = ? OR source_url = ?').get(url, url);
            if (row) {
              exists = true;
              break;
            }
        ...
        if (!exists && product.id) {
          const row = db.prepare('SELECT id FROM product_extractions WHERE id = ?').get(product.id);
          if (row) {
            exists = true;
          }
        }
