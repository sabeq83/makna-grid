...
export async function POST(req) {
  try {
    ...
    const db = getDb();
    const allProducts = db.prepare('SELECT id, input_source, source_url, raw_photo_url, cleaned_photo_url, generated_photo_url, unique_selling_point, packaging_type FROM product_extractions').all();
      ...
      for (const row of uniqueCsvRows) {
        ...
        if (url.startsWith('http://') || url.startsWith('https://')) {
          ...
          if (existing) {
            ...
            db.prepare('UPDATE product_extractions SET affiliate_link = ? WHERE id = ?').run(affiliate, existing.id);
            updatedCount++;
