    ...
    db.transaction(() => {
      for (const row of rawRows) {
        ...
        if (rawLink && rawLink.trim().startsWith('http')) {
          const matched = db.prepare('SELECT id FROM product_extractions WHERE source_url = ? OR input_source = ?').get(rawLink.trim(), rawLink.trim());
          if (matched) exists = true;
        } else {
          const matched = db.prepare('SELECT id FROM product_extractions WHERE product_name = ?').get(rawName.trim());
          if (matched) exists = true;
        }
