...
export async function POST(req) {
  try {
    ...
    for (const id of ids) {
      const p = db.prepare('SELECT id, product_name, product_description, unique_selling_point, is_in_packaging, packaging_type FROM product_extractions WHERE id = ?').get(id);
      if (p) {
        productsArray.push({
          id: p.id,
    ...
    for (const item of results) {
      updateProductExtraction(item.id, {
