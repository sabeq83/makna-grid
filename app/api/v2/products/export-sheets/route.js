...
export async function POST(req) {
  try {
    ...
    if (ids && Array.isArray(ids) && ids.length > 0) {
      ...
      products = db.prepare(`SELECT * FROM product_extractions WHERE id IN (${placeholders}) ORDER BY created_at DESC`).all(...ids);
    } else {
      products = db.prepare('SELECT * FROM product_extractions ORDER BY created_at DESC').all();
    }
    ...
    const photoFolderName = getSetting('drive_product_photo_folder') || '_fotoproduk';
