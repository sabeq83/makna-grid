...
export async function POST(request, { params }) {
  try {
    ...
    const output = db.prepare('SELECT * FROM re_plus_recomm_outputs WHERE id = ?').get(id);
    if (!output) {
      return NextResponse.json({ success: false, error: 'Recommendation not found' }, { status: 404 });
    }

    // 2. Get parent job details
    const job = db.prepare('SELECT * FROM re_plus_recomm_jobs WHERE id = ?').get(output.recomm_job_id);
    ...
    createReCampaign(campaignOptions);

    // 6. Associate the competitor URL with the campaign
    addReCampaignItems(campaignId, [output.source_url]);

    // 7. Fetch the campaign item ID
    const item = db.prepare('SELECT id FROM re_campaign_items WHERE campaign_id = ? AND source_url = ? LIMIT 1')
