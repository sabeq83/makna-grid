...
import { getDb, updateInstantCampaignItem, updateInstantCampaign } from '@/lib/db';
import fs from 'fs';
...
export async function PATCH(request, { params }) {
  try {
    ...
    const item = db.prepare('SELECT * FROM instant_campaign_items WHERE id = ?').get(itemId);
    if (!item) {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      prodUrl = formData.get('product_image_url');
      ...
      if (file && typeof file !== 'string') {
        ...
        safeFilename = `if_manual_${item.campaign_id}_item${item.id}_${Date.now()}${ext}`;
      ...
      prodUrl = body.product_image_url;
    ...
    if (prodUrl !== null) creativePayload.product_image_url = prodUrl;
    ...
    if (resetStatus) {
      // JIT Sourcing: if product_image_url starts with http and product_ref_image_path is not yet downloaded, status = pending_sourcing
      let nextStatus = 'pending';
      const imgUrl = creativePayload.product_image_url;
      if (imgUrl && (imgUrl.startsWith('http://') || imgUrl.startsWith('https://')) && !creativePayload.product_ref_image_path) {
    ...
    updateInstantCampaignItem(itemId, updateFields);

    // Make sure campaign status is 'running' if it's currently paused and we did a reset
    if (resetStatus) {
      const campaign = db.prepare('SELECT * FROM instant_campaigns WHERE id = ?').get(item.campaign_id);
      if (campaign && campaign.status !== 'running') {
        updateInstantCampaign(item.campaign_id, { status: 'running' });
      }
    }
    ...
    console.error('[API Update Creative IFC] Error:', error);
