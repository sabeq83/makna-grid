...
export async function POST(request, { params }) {
  try {
    ...
    const campaign = getInstantCampaign(id);
    if (!campaign) {
    ...
    // Set to processing
    updateInstantCampaignStatus(id, 'processing');
    ...
    if (productUrl) {
      try {
        const db = getDb();
        const extracted = db.prepare('SELECT unique_selling_point FROM product_extractions WHERE input_source = ? LIMIT 1').get(productUrl);
        if (extracted) {
    ...
    let brandMandate = '';
    if (brandProfileId) {
      const brandConfig = getBrandProfile(brandProfileId);
      if (brandConfig) {
        brandMandate = `\n========================================================================
...

{
  ...
  "distribution_assets": {
    ...
    try {
      ...
      updateInstantCampaignStatus(id, 'failed');
      saveInstantCampaignOutput(uuidv4(), id, null, `JSON Parse Error: ${e.message}\nRaw Output: ${rawResponse}`);
    ...
    updateInstantCampaignStatus(id, 'completed');
    saveInstantCampaignOutput(uuidv4(), id, JSON.stringify(parsedJson));
