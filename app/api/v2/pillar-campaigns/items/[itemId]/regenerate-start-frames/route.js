...
async function runItemRegenerateStartFramesBackground(itemId, campaign, item, newVideoPlan) {
  ...
  try {
    ...
    if (campaign.product_ref_image_path) {
      ...
      // OPC Self-healing / row creative check if mass production
      ...
      if (t2iCompleted && t2iImageUrl) {
        ...
        if (imgResponse.ok) {
          ...
          const updateTransaction = db.transaction(() => {
            const currentItem = db.prepare("SELECT t2i_images_json FROM pillar_campaign_items WHERE id = ?").get(itemId);
