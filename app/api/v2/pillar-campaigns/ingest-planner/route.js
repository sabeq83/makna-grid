...
export async function POST(request) {
  try {
    ...
    const planner = db.prepare('SELECT * FROM content_planners WHERE id = ?').get(planner_id);
    if (!planner) {
    ...
    if (Array.isArray(selected_row_ids) && selected_row_ids.length > 0) {
      ...
      rows = db.prepare(`SELECT * FROM content_planner_rows WHERE planner_id = ? AND id IN (${placeholders}) ORDER BY sequence ASC`).all(planner_id, ...selected_row_ids);
    } else {
      rows = db.prepare('SELECT * FROM content_planner_rows WHERE planner_id = ? ORDER BY sequence ASC').all(planner_id);
    }
