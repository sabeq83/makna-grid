...
export async function GET(request, { params }) {
  try {
    ...
    const campaign = getPillarCampaign(id);
    ...
    if (campaign.is_bridging_active) {
      if (campaign.bridging_mode === 'select_existing' && campaign.target_product_id) {
        const product = db.prepare('SELECT * FROM product_extractions WHERE id = ?').get(campaign.target_product_id);
        if (product) {
    ...
    const items = listPillarCampaignItems(id);
...
export async function PATCH(request, { params }) {
  try {
    ...
    const campaign = getPillarCampaign(id);
    updatePillarCampaign(id, updates);
