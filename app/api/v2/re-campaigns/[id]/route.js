...
export async function GET(request, { params }) {
  try {
    ...
    const campaign = getReCampaign(id);
    if (!campaign) return NextResponse.json({ error: 'Campaign not found' }, { status: 404 });

    const items = listReCampaignItems(id);
    const db = getDb();
    for (const item of items) {
      if (item.tts_status === 'completed' && item.tts_batch_id) {
        item.tts_clips = db.prepare('SELECT * FROM tts_studio_clips WHERE batch_id = ? ORDER BY clip_index ASC').all(item.tts_batch_id);
      } else {
        item.tts_clips = [];
      }

      // Load G-Labs tasks for the item
      item.glabs_tasks = db.prepare('SELECT * FROM glabs_tasks WHERE campaign_id = ? AND item_id = ? ORDER BY clip_index ASC').all(id, item.id);

      // Load angle variants
      item.angle_variants = db.prepare('SELECT * FROM re_item_angle_variants WHERE re_item_id = ? ORDER BY created_at ASC').all(item.id);
      for (const variant of item.angle_variants) {
        if (variant.tts_status === 'completed' && variant.tts_batch_id) {
          variant.tts_clips = db.prepare('SELECT * FROM tts_studio_clips WHERE batch_id = ? ORDER BY clip_index ASC').all(variant.tts_batch_id);
        } else {
          variant.tts_clips = [];
        }
        ...
        if (variantTaskIds.length > 0) {
          ...
          variant.glabs_tasks = db.prepare(`SELECT * FROM glabs_tasks WHERE task_id IN (${placeholders})`).all(...variantTaskIds);
        } else {
          variant.glabs_tasks = [];
        }
    ...
    const stats = getReCampaignStats(id);
    const { getSetting } = await import('../../../../../lib/db');
    const storageProvider = getSetting('storage_provider') || 'gdrive';
    const nextcloudUrl = getSetting('nextcloud_url') || '';
