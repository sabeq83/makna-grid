...
export async function POST(request, { params }) {
  try {
    ...
    const oldCampaign = db.prepare('SELECT product_ref_image_path FROM re_campaigns WHERE id = ?').get(id);
    ...
    const items = db.prepare("SELECT id FROM re_campaign_items WHERE campaign_id = ?").all(id);
    ...
    if (fs.existsSync(startFramesDir) && itemIds.length > 0) {
      ...
      const variants = db.prepare(`SELECT id FROM re_item_angle_variants WHERE re_item_id IN (${placeholders})`).all(...itemIds);
