...
export async function PATCH(req, { params }) {
  try {
    ...
    const variant = db.prepare("SELECT * FROM re_item_angle_variants WHERE id = ?").get(id);
    if (!variant) {
      return NextResponse.json({ success: false, error: "Data variasi angle tidak ditemukan." }, { status: 404 });
    }

    // Update DB
    db.prepare("UPDATE re_item_angle_variants SET visual_tasks_json = ? WHERE id = ?").run(visual_tasks_json, id);
