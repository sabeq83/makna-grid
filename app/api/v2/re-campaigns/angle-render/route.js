...
export async function POST(req) {
  try {
    ...
    const variant = db.prepare("SELECT * FROM re_item_angle_variants WHERE id = ?").get(angle_variant_id);
    if (!variant) {
      return NextResponse.json({ success: false, error: "Data variasi angle tidak ditemukan." }, { status: 404 });
    }

    const parentItem = db.prepare("SELECT campaign_id FROM re_campaign_items WHERE id = ?").get(variant.re_item_id);
    const campaign = db.prepare("SELECT enable_tts FROM re_campaigns WHERE id = ?").get(parentItem.campaign_id);
