...
export async function GET() {
  try {
    const campaigns = listReCampaigns();
    const withStats = campaigns.map(c => ({ ...c, stats: getReCampaignStats(c.id) }));
    const isSchedulerActive = getSetting('re_campaigns_scheduler_active') !== 'false';
