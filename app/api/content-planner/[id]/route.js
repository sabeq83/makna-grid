...
export async function GET(request, { params }) {
  try {
    ...
    const planner = db.prepare('SELECT * FROM content_planners WHERE id = ?').get(id);
    if (!planner) {
      return NextResponse.json({ success: false, error: 'Planner tidak ditemukan.' }, { status: 404 });
    }

    const rows = db.prepare('SELECT * FROM content_planner_rows WHERE planner_id = ? ORDER BY sequence ASC').all(id);
...
export async function DELETE(request, { params }) {
  try {
    ...
    db.prepare('DELETE FROM content_planners WHERE id = ?').run(id);
