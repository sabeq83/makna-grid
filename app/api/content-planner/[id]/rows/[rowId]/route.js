...
export async function PATCH(request, { params }) {
  try {
    ...
    const existingRow = db.prepare('SELECT * FROM content_planner_rows WHERE id = ? AND planner_id = ?').get(rowId, plannerId);
    if (!existingRow) {
    ...
    if (updates.length > 0) {
      ...
      values.push(rowId);
      db.prepare(`UPDATE content_planner_rows SET ${updates.join(', ')} WHERE id = ?`).run(...values);
    }

    const updatedRow = db.prepare('SELECT * FROM content_planner_rows WHERE id = ?').get(rowId);
...
export async function DELETE(request, { params }) {
  try {
    ...
    db.prepare('DELETE FROM content_planner_rows WHERE id = ? AND planner_id = ?').run(rowId, plannerId);
