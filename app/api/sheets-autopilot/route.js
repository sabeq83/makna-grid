...
export async function GET(request) {
  try {
    ...
    if (id) {
      ...
      const campaign = getSheetsCampaign(id);
      if (!campaign) {
        return NextResponse.json({ success: false, error: 'Kampanye tidak ditemukan.' }, { status: 404 });
      }
      const jobs = getSheetsJobs(id);
      return NextResponse.json({ success: true, data: { campaign, jobs } });
    }

    const campaigns = getSheetsCampaigns();
    const isSchedulerActive = getSetting('sheets_autopilot_scheduler_active') !== 'false';
...
export async function POST(request) {
  try {
    ...
    createSheetsCampaign(newCampaign);
...
export async function PATCH(request) {
  try {
    ...
    if (schedulerStatus !== undefined) {
      setSetting('sheets_autopilot_scheduler_active', schedulerStatus ? 'true' : 'false');
    ...
    updateSheetsCampaignStatus(id, status);
