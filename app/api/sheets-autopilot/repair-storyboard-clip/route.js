...
export async function POST(request) {
  ...
  try {
    ...
    let product = db.prepare('SELECT * FROM product_extractions WHERE input_source = ? OR source_url = ?').get(productUrl, productUrl);
...
Respon JSON:
{
  ...
  "t2v_prompt": null
    ...
    db.prepare('UPDATE sheets_jobs SET storyboard = ?, voiceover = ?, prompts_json = ? WHERE id = ?')
    ...
    db.prepare('UPDATE sheets_jobs SET local_audio_path = ? WHERE id = ?').run(finalAudioPath, jobId);
