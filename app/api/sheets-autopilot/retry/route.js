...
export async function POST(request) {
  try {
    ...
    const db = getDb();
    const campaign = getSheetsCampaign(campaignId);
    if (!campaign) {
    ...
    if (force === true) {
      ...
      const jobs = db.prepare('SELECT id FROM sheets_jobs WHERE campaign_id = ? AND row_index = ?').all(campaignId, rowIndex);
      ...
      const deleteTransaction = db.transaction(() => {
        if (jobIds.length > 0) {
          ...
          db.prepare(`DELETE FROM glabs_tasks WHERE item_id IN (${placeholders})`).run(...jobIds);
        }
        db.prepare('DELETE FROM sheets_jobs WHERE campaign_id = ? AND row_index = ?').run(campaignId, rowIndex);
      });
      deleteTransaction();
    }
