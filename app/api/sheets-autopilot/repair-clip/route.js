...
export async function POST(request) {
  ...
  try {
    ...
    const db = getDb();
    const job = db.prepare('SELECT * FROM sheets_jobs WHERE id = ?').get(jobId);
    ...
    if (t2vObj && (!t2iObj || !i2vObj)) {
      ...
      if (productImageUrlOverride) {
        ...
        db.prepare("UPDATE product_extractions SET photo_url = ?, scraped_image_url = ? WHERE id = ?")
        ...
        if (!productBase64) {
          ...
          if (redownloadUrl && redownloadUrl.startsWith('http')) {
            ...
            db.prepare("UPDATE product_extractions SET raw_photo_url = ? WHERE id = ?").run(rawRelPath, product.id);
    ...
    const updateRes = db.prepare("UPDATE glabs_tasks SET video_url = ?, task_id = ?, status = 'completed' WHERE item_id = ? AND clip_index = ?")
    ...
    if (storageProvider === 'nextcloud') {
      ...
      db.prepare("UPDATE sheets_jobs SET gdrive_folder_url = ? WHERE id = ?").run(shareUrl, jobId);
