...
export async function POST(req) {
  try {
    ...
    if (batch_id && regenerate_clip_index !== undefined) {
      ...
      const db = getDb();
      const existing = db.prepare('SELECT id FROM tts_studio_clips WHERE batch_id = ? AND clip_index = ?').get(batch_id, i);

      let finalClipId = clipId;
      if (existing) {
        finalClipId = existing.id;
        db.prepare('UPDATE tts_studio_clips SET status = "processing", audio_path = ? WHERE id = ?').run(publicPath, finalClipId);
      } else {
        createTtsClip({
          id: clipId,
          batch_id: batchId,
      ...
      if (!modelQuality && batch_id) {
        const { getDb } = await import('@/lib/db');
        const db = getDb();
        const batchRecord = db.prepare('SELECT tts_model_quality FROM tts_studio_batches WHERE id = ?').get(batch_id);
        if (batchRecord) {
      ...
      try {
        ...
        updateTtsClip(finalClipId, { status: 'completed' });
        ...
        updateTtsClip(finalClipId, { status: 'failed' });
        return NextResponse.json({
          success: false,
          error: err.message,
          clip: {
