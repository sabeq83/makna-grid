...
function upsertRow(db, tableName, rowData) {
  if (!rowData) return;
  ...
  try {
    ...
    db.transaction(() => {
      ...
      if (dbPayload.brandProfile) {
        upsertRow(db, 'brand_profiles', dbPayload.brandProfile);
      }
      if (dbPayload.product) {
        ...
        upsertRow(db, 'product_extractions', product);
      }
      ...
      if (campaignType === 'OPC') {
        if (dbPayload.campaign) {
          upsertRow(db, 'pillar_campaigns', dbPayload.campaign);
        }
        if (dbPayload.items && Array.isArray(dbPayload.items)) {
          for (const item of dbPayload.items) {
            upsertRow(db, 'pillar_campaign_items', item);
          }
        }
      } else {
        // Mode RE Campaign
        if (dbPayload.campaign) {
          upsertRow(db, 're_campaigns', dbPayload.campaign);
        }
        if (dbPayload.items && Array.isArray(dbPayload.items)) {
          for (const item of dbPayload.items) {
            upsertRow(db, 're_campaign_items', item);
          }
        }
        ...
        if (dbPayload.angleVariants && Array.isArray(dbPayload.angleVariants)) {
          for (const variant of dbPayload.angleVariants) {
            upsertRow(db, 're_item_angle_variants', variant);
          }
        }
      }
    })();
