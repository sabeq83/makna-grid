...
export async function GET(req) {
  try {
    ...
    if (type === 'OPC') {
      campaign = db.prepare("SELECT * FROM pillar_campaigns WHERE id = ?").get(campaignId);
    ...
    const product = campaign.target_product_id 
      ? db.prepare("SELECT * FROM product_extractions WHERE id = ?").get(campaign.target_product_id) 
      : null;
