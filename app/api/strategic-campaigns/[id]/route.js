...
export async function GET(request, { params }) {
  try {
    ...
    const campaign = db.prepare('SELECT * FROM strategic_campaigns WHERE id = ?').get(id);
    if (!campaign) {
      return NextResponse.json({ success: false, error: 'Kampanye strategis tidak ditemukan.' }, { status: 404 });
    }

    const items = db.prepare('SELECT * FROM strategic_campaign_items WHERE campaign_id = ? ORDER BY sequence ASC').all(id);

    // Fetch scenes for each item
    const itemsWithScenes = items.map(item => {
      const scenes = db.prepare('SELECT * FROM strategic_campaign_scenes WHERE campaign_item_id = ? ORDER BY scene_number ASC').all(item.id);
      return {
        ...item,
        scenes
      };
...
export async function DELETE(request, { params }) {
  try {
    ...
    db.prepare('DELETE FROM strategic_campaigns WHERE id = ?').run(id);
...
export async function PATCH(request, { params }) {
  try {
    ...
    db.prepare('UPDATE strategic_campaigns SET workflow_config_json = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(
