...
export async function POST(req, { params }) {
  try {
    ...
    const scene = db.prepare("SELECT * FROM strategic_campaign_scenes WHERE campaign_item_id = ? AND scene_number = ?").get(itemId, Number(sceneNumber));
    if (!scene) {
    ...
    for (let poll = 0; poll < 40; poll++) {
      ...
      if (statusRes?.status === 'completed') {
        ...
        if (imgFile) {
          ...
          db.prepare('UPDATE strategic_campaign_scenes SET start_frame_path = ? WHERE id = ?').run(`/uploads/start_frames/${filename}`, scene.id);
          ...
          db.prepare('UPDATE strategic_campaign_scenes SET task_id = NULL, local_clip_path = NULL WHERE id = ?').run(scene.id);

          // Reset item visual & ffmpeg status so scheduler automatically regenerates fresh I2V video
          db.prepare("UPDATE strategic_campaign_items SET visual_status = 'pending', visual_tasks_json = NULL, visual_clip_paths = NULL, ffmpeg_status = 'pending', workflow_status = 'processing' WHERE id = ?").run(itemId);
        }
        break;
      }
    }
