...
export async function POST(request, { params }) {
  try {
    ...
    if (action === 'apply_safe_revisions') {
      const latestReview = db.prepare("SELECT * FROM content_compliance_reviews WHERE campaign_item_id = ? AND review_stage = 'creative' ORDER BY created_at DESC").get(itemId);
      ...
      db.prepare("UPDATE strategic_campaign_items SET workflow_status = 'compliance_passed', updated_at = CURRENT_TIMESTAMP WHERE id = ?").run(itemId);
