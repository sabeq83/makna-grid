...
export async function POST(req, { params }) {
  try {
    ...
    if (step === 'visuals') {
      ...
      let resetCount = 0;
      const clearTaskStmt = db.prepare("UPDATE strategic_campaign_scenes SET task_id = NULL WHERE id = ?");
      ...
      if (campaign && campaign.status !== 'running') {
        db.prepare("UPDATE strategic_campaigns SET status = 'running', updated_at = CURRENT_TIMESTAMP WHERE id = ?").run(campaignId);
      }
      ...
      db.prepare("UPDATE strategic_campaign_scenes SET start_frame_path = NULL WHERE campaign_item_id = ?").run(itemId);
