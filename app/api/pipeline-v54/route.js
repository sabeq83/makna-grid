...
export async function POST(request) {
  try {
    ...
    const product = getProductExtraction(product_id);
    if (!product) {
    ...
    if (action === 'run_pipeline') {
      ...
      updatePipelineAsset(pipeline_id, {
      ...
      try {
        ...
        updatePipelineAsset(pipeline_id, { status: 'error_stage3', raw_responses: JSON.stringify(rawResponses) });
      ...
      try {
        ...
        updatePipelineAsset(pipeline_id, { status: 'error_stage4', raw_responses: JSON.stringify(rawResponses) });
      ...
      try {
        ...
        updatePipelineAsset(pipeline_id, { status: 'error_stage5', raw_responses: JSON.stringify(rawResponses) });
    ...
    if (action === 'regenerate') {
      ...
      const existing = getPipelineAsset(pipeline_id);
      if (!existing) {
      ...
      if (stage === 'narration') {
        ...
        updatePipelineAsset(pipeline_id, { audio_blueprint: JSON.stringify(audioBlueprint) });
