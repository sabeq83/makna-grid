...
export async function POST(request) {
  try {
    ...
    createReverseResult(result);

    // Link RE result back to video library entry
    updateVideoLibraryEntry(video_id, { re_id: result.id, status: 'analyzed' });
    ...
    if (tokensStr) {
      try {
        ...
        let spreadsheetId = getSetting('master_re_sheet_id');
        if (spreadsheetId) {
        ...
        if (!spreadsheetId) {
          ...
          setSetting('master_re_sheet_id', spreadsheetId);
