...
export async function POST(request) {
  try {
    ...
    let spreadsheetId = getSetting('master_pipeline_sheet_id');
    let isNew = false;
    ...
    if (!spreadsheetId) {
      ...
      setSetting('master_pipeline_sheet_id', spreadsheetId);
      isNew = true;
    ...
    if (stage === 'ideation') {
      ...
      if (source === 'reverse') {
        ...
        idea = asset.idea_id ? getIdea(asset.idea_id) : null;
...
export async function GET() {
  try {
    const sheetId = getSetting('master_pipeline_sheet_id');
    return NextResponse.json({
      success: true,
      data: {
        exists: !!sheetId,
