...
export async function createStrategicCampaign(params) {
  ...
  if (targetProdId) {
    try {
      const prod = db.prepare('SELECT photo_url, active_photo, clean_photo_url, cleaned_photo_url, generated_photo_url, product_truth, geometric_truth FROM product_extractions WHERE id = ?').get(targetProdId);
      if (prod) {
