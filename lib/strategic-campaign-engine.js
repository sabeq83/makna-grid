...
import { DEMOGRAPHIC_PRESETS, WARDROBE_PRESETS, LIGHTING_PRESETS, MASCOT_UNIVERSES, MASCOT_ART_STYLES } from './prompts.js';
...
export async function createStrategicCampaign(params) {
  ...
  const targetProdId = workflow_config.target_product_id || (source_planner_id ? db.prepare('SELECT product_id FROM content_planners WHERE id = ?').get(source_planner_id)?.product_id : null);

  if (targetProdId) {
    try {
      const prod = db.prepare('SELECT photo_url, active_photo, clean_photo_url, cleaned_photo_url, generated_photo_url FROM product_extractions WHERE id = ?').get(targetProdId);
      if (prod) {
  ...
  const plannerObj = source_planner_id ? db.prepare('SELECT account_name, google_sheet_id FROM content_planners WHERE id = ?').get(source_planner_id) : null;
  ...
  db.prepare(`
    INSERT INTO strategic_campaigns (
      id, campaign_name, source_planner_id, input_mode, product_name, product_description, product_usp, product_ref_image, status, vso_config_json, workflow_config_json, target_spreadsheet_id, nextcloud_parent_folder, account_name
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    campaignId,
  ...
  if (source_planner_id && selected_row_ids.length > 0) {
    ...
    const plannerRows = db.prepare(`SELECT * FROM content_planner_rows WHERE id IN (${placeholders}) ORDER BY sequence ASC`).all(...selected_row_ids);
...
export async function executeCall1CreativeEngine(campaignItemId) {
  const db = getDb();
  const item = db.prepare('SELECT * FROM strategic_campaign_items WHERE id = ?').get(campaignItemId);
...
INSTRUKSI KONFIGURASI PROFIL PRODUK & KAMPAUAN:
- Produk: ${item.product}
...
Format Output WAJIB berupa JSON Object:
{
  ...
  "social_media_package": {
    "caption": "Naskah caption posting media sosial (TikTok, Instagram Reels, FB) yang persuasif dan relevan dengan naskah adegan...",
    "hashtags": ["#hashtag1", "#hashtag2", "#hashtag3", "#hashtag4", "#hashtag5"],
    "call_to_action": "Kalimat CTA penutup...",
    "marketing_angle": "Sudut pandang strategi..."
  },
  "video_dna": {
    "pilar_konten": "${item.pillar}",
    "hook_type": "Pertanyaan / Mitos / Problem-Solving",
    "visual_style": "${workflow.visual_style || 'Cinematic'}",
    "signature_moment": "Adegan estetik/ASMR paling menonjol",
    "camera_pace": "Dynamic Micro-Pacing (8s per clip)",
    "primary_emotion": "Penasaran / Empati / High Desire",
...
`;
  ...
  db.prepare('DELETE FROM strategic_campaign_scenes WHERE campaign_item_id = ?').run(campaignItemId);
  ...
  try {
    ...
    if (complianceReview.status === 'pass') {
      db.prepare("UPDATE strategic_campaign_items SET workflow_status = 'ready_for_review' WHERE id = ?").run(campaignItemId);
      ...
      if (Array.isArray(revisedPackage.storyboard)) {
        const updateSceneStmt = db.prepare('UPDATE strategic_campaign_scenes SET voice_over = ?, on_screen_text = ? WHERE campaign_item_id = ? AND scene_number = ?');
        revisedPackage.storyboard.forEach((sc, idx) => {
          updateSceneStmt.run(sc.voice_over || '', sc.on_screen_text || '', campaignItemId, idx + 1);
        });
      }
      ...
      db.prepare("UPDATE strategic_campaign_items SET workflow_status = 'compliance_blocked' WHERE id = ?").run(campaignItemId);
    } else if (complianceReview.status === 'human_review') {
      db.prepare("UPDATE strategic_campaign_items SET workflow_status = 'human_review_required' WHERE id = ?").run(campaignItemId);
    }
  } catch (compErr) {
...
export async function syncStrategicCampaignToContentFlow(campaignId) {
  const db = getDb();
  let campaign = db.prepare('SELECT * FROM pillar_campaigns WHERE id = ?').get(campaignId);
  let isPillar = true;
  if (!campaign) {
    campaign = db.prepare('SELECT * FROM re_campaigns WHERE id = ?').get(campaignId);
    isPillar = false;
  }
  if (!campaign) {
  ...
  let items = [];
  if (isPillar) {
    items = db.prepare('SELECT * FROM pillar_campaign_items WHERE campaign_id = ?').all(campaignId);
  } else {
    items = db.prepare('SELECT * FROM re_campaign_items WHERE campaign_id = ?').all(campaignId);
  }
...
export async function syncCampaignItemToContentFlow(campaign, item, assetUrl) {
  try {
    ...
    let caption = item.caption || item.hook || campaign.custom_hook || '';
    if (item.creative_package_json) {
      try {
        const creative = JSON.parse(item.creative_package_json);
        if (creative.social_package && creative.social_package.caption) {
