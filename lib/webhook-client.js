...
function getBaseUrl() {
  ...
  const role = (process.env.NODE_ROLE || '').toLowerCase();
  const defaultHost = role === 'gateway' ? '100.117.59.92' : '127.0.0.1';
  const host = process.env.WEBHOOK_HOST || getSetting('webhook_host') || defaultHost;
