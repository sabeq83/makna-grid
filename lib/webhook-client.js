import { getSetting, getDb, getGlabsTaskRoute } from './db.js';
...
function getBaseUrl(taskId = null) {
  if (taskId) {
    try {
      const route = getGlabsTaskRoute(taskId);
...
export async function webhookHealth(hostOverride, portOverride) {
  const host = hostOverride || getSetting('webhook_host') || process.env.WEBHOOK_HOST || '100.117.59.92';
...
export async function generateImage({ prompt, model, aspect_ratio, reference_images, upscale, webhookOverride }) {
  ...
  const host = webhookOverride?.webhook_host || getSetting('webhook_host') || process.env.WEBHOOK_HOST || '100.117.59.92';
...
export function getFileUrl(filename, taskId = null) {
  ...
  if (!host && filename) {
    try {
      const db = getDb();
      // Match route where task_id is a substring of the filename
      const route = db.prepare("SELECT * FROM glabs_task_routes WHERE ? LIKE '%' || task_id || '%' LIMIT 1").get(filename);
      if (route) {
        host = route.host;
        port = route.port;
      }
