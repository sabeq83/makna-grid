...
export async function webhookHealth(hostOverride, portOverride) {
  const host = hostOverride || process.env.WEBHOOK_HOST || getSetting('webhook_host') || '100.117.59.92';
  const port = portOverride || process.env.WEBHOOK_PORT || getSetting('webhook_port') || '8765';
...
function sanitizeImagePrompt(prompt) {
  ...
  // 0. Conflict Guard: Strip hallucinated container terms (e.g. cardboard box) that contradict reference images
    ...
    for (const ref of reference_images) {
      ...
      if (ref.startsWith('data:image/')) {
        ...
        try {
          ...
          console.warn('[generateVideo] Failed to fetch HTTP reference image for base64 conversion:', e.message);
        }
      } else {
        try {
