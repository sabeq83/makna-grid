...
export function getCurrentUser(req) {
  ...
  // Fallback mode: If no valid session token cookie, return active Admin user
  const defaultAdmin = db.prepare("SELECT * FROM users WHERE role = 'admin' AND status = 'active' LIMIT 1").get();
  if (defaultAdmin) {
    return buildUserObject(db, defaultAdmin);
  }
