...
import { executeWithKeyPool, uploadVideoToGemini, generateContentFlexible } from './gemini.js';
...
import { buildReverseEngineeringBridgePrompt, buildOrganicPillarPrompt, buildReverseEngineeringPrompt, DEMOGRAPHIC_PRESETS, WARDROBE_PRESETS, LIGHTING_PRESETS, getPackagingInstruction, getConceptInstruction } from './prompts.js';
...
export function logToAutopilot(message) {
  try {
    ...
    const logFile = path.join(logDir, 'autopilot_logs.txt');
      ...
      if (elapsed > TIMEOUT_LIMIT_MS) {
        ...
        try {
          ...
          if (!job.script_status || job.script_status === 'pending' || job.script_status === 'processing') {
            ...
            const tasks = db.prepare('SELECT * FROM glabs_tasks WHERE item_id = ?').all(job.id);
    ...
    try {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (reviewStatus === 'Approved' && pipelineStatus !== 'Completed' && pipelineStatus !== 'Processing') {
          ...
          const newJob = db.prepare('SELECT * FROM sheets_jobs WHERE id = ?').get(jobId);
          const mergedJob = {
            ...newJob,
...
async function processActiveJob(job) {
  ...
  if (!job.script_status || job.script_status === 'pending') {
    ...
    job = db.prepare('SELECT * FROM sheets_jobs WHERE id = ?').get(job.id);
  }
...
async function runScriptStage(job, sheets, headers, sheetName) {
  ...
  try {
    ...
    if (shouldSourceProduct) {
      ...
      product = db.prepare('SELECT * FROM product_extractions WHERE input_source = ? OR source_url = ?').get(productUrl, productUrl);
...
Hasilkan JSON dengan struktur:
{
  ...
  "i2v_action_prompt": "Image-to-Video action prompt dalam Bahasa Inggris yang mendeskripsikan gerakan cairan, tuangan, krim, atau interaksi mikro terhadap produk tersebut"
    ...
    if (rowWardrobeColor && rowWardrobeColor.trim().length > 0) {
      let wardrobeDesc = `dressed in ${rowWardrobeColor} color`;
      const normalizedInput = rowWardrobeColor.toLowerCase().replace(/[\s_-]+/g, '');
      const matchedKey = Object.keys(WARDROBE_PRESETS || {}).find(k => k.toLowerCase().replace(/[\s_-]+/g, '') === normalizedInput);
      if (matchedKey) {
        wardrobeDesc = WARDROBE_PRESETS[matchedKey];
      } else {
        wardrobeDesc = `dressed in ${rowWardrobeColor} color, modest clothing`;
      }
      rowVsoData.wardrobe_style = 'custom';
      rowVsoData.wardrobe_style_custom = wardrobeDesc;
      ...
      if (keys.length > 0) {
        ...
        const wardrobeDesc = WARDROBE_PRESETS[selectedPresetKey];
        rowVsoData.wardrobe_style = 'custom';
        rowVsoData.wardrobe_style_custom = wardrobeDesc;
      }
    }
  ...
  "yt_desc": "Deskripsi YouTube Shorts. In ${languageLabel}."
    ...
    if (campaign.enable_audio_segment === 1) {
      ...
      if (updatedVoiceCastJson) {
        ...
        db.prepare('UPDATE sheets_campaigns SET voice_cast_json = ? WHERE id = ?').run(updatedVoiceCastJson, campaign.id);
    ...
    updateSheetsJobStatus(jobId, {
      ...
      captions_json: JSON.stringify({
        tiktok_caption: storyboardResult.tiktok_caption || storyboardResult.analysis_summary?.tiktok_caption || '',
        ig_caption: storyboardResult.ig_caption || storyboardResult.analysis_summary?.ig_caption || '',
        yt_title: storyboardResult.yt_title || storyboardResult.analysis_summary?.yt_title || '',
        yt_desc: storyboardResult.yt_desc || storyboardResult.analysis_summary?.yt_desc || ''
      }),
    ...
    if (resolvedPhotoPath) {
      ...
      if (redownloadUrl && redownloadUrl.startsWith('http')) {
        ...
        try {
          ...
          db.prepare('UPDATE product_extractions SET raw_photo_url = ? WHERE id = ?').run(rawRelPath, effectiveProduct.id);
  ...
  try {
    for (let c = 1; c <= totalClips; c++) {
      ...
      if (isBridgeScene && productBase64) {
        ...
        const t2iBase64 = startFrameBuffer.toString('base64');
...
async function runFFmpegStage(job, sheets, headers, sheetName) {
  ...
  const tasks = db.prepare('SELECT * FROM glabs_tasks WHERE item_id = ? ORDER BY clip_index ASC').all(job.id);
  const videoClips = [];
...
export function buildMarkdownContent(parsed, batchId) {
  ...
  const tiktok = parsed.tiktok_caption || parsed.analysis_summary?.tiktok_caption || '';
  const ig = parsed.ig_caption || parsed.analysis_summary?.ig_caption || '';
  const ytTitle = parsed.yt_title || parsed.analysis_summary?.yt_title || '';
  const ytDesc = parsed.yt_desc || parsed.analysis_summary?.yt_desc || '';

  if (tiktok || ig || ytTitle || ytDesc) {
