...
export function logToMultiplier(message) {
  try {
    ...
    const logFile = path.join(logDir, 'multiplier_logs.txt');
...
export async function processNextMultiplierTask() {
  ...
  logToMultiplier(`[Multiplier Worker] Starting processing for task ${task.id} (Status: ${task.status})`);
  ...
  try {
    ...
    updateMultiplierTask(task.id, { status: 'failed' });
  }
}
...
async function handleProductResolution(task) {
  updateMultiplierTask(task.id, { status: 'resolving_product' });

  const db = getDb();
  const bridging = JSON.parse(task.bridging_config_json || '{}');
  let productDNA = {};

  if (bridging.bridgingMode === 'select_existing') {
    const product = db.prepare('SELECT * FROM product_extractions WHERE id = ?').get(bridging.targetProductId);
    if (!product) throw new Error(`Product ID ${bridging.targetProductId} not found in database`);
    productDNA = {
    ...
    const url = bridging.productUrl || task.target_product_url;
...
async function handleStoryboardRemake(task) {
  const db = getDb();
  const asset = db.prepare('SELECT * FROM re_deconstructed_assets WHERE id = ?').get(task.deconstruct_asset_id);
  ...
  const hasTts = audio.enableTts === 1 || audio.enableTts === true;
  const hasGlabs = audio.enableGlabs === 1 || audio.enableGlabs === true;
  const nextStatus = hasTts ? 'generating_audio' : (hasGlabs ? 'generating_visuals' : 'completed');

  updateMultiplierTask(task.id, {
    remake_storyboard_json: JSON.stringify(parsed.storyboard || []),
...
async function handleAudioGeneration(task) {
  updateMultiplierTask(task.id, { status: 'generating_audio' });
...
async function handleVisualGeneration(task) {
  ...
  if (!enableGlabs) {
    ...
    updateMultiplierTask(task.id, { status: 'completed' });
    return;
  }
  ...
  if (!task.glabs_task_ids) {
    ...
    updateMultiplierTask(task.id, { status: 'generating_visuals' });
    ...
    for (let c = 1; c <= prompts.length; c++) {
      ...
      if (isHybridLockClip) {
        // Double pass hybrid lock
        const t2iPromptText = promptObj.t2i_prompt || '';
        const i2vPromptText = promptObj.i2v_prompt || '';

        logToMultiplier(`[Multiplier Worker] Hybrid Lock Clip: Generating start frame image for clip ${c}`);
        let productBase64 = null;
        if (bridging.productRefImagePath) {
          productBase64 = fileToBase64(bridging.productRefImagePath);
        }
        ...
        if (!t2iResult?.task_id) {
          throw new Error(`T2I image generation failed for clip ${c}`);
        }
        ...
        for (let attempt = 0; attempt < 30; attempt++) {
          ...
          if (t2iStatusResult?.status?.toLowerCase() === 'completed') {
            ...
            throw new Error(`T2I generation failed on task ${t2iResult.task_id}`);
          }
        }

        if (!t2iImageUrl) {
          throw new Error(`T2I image generation timed out or failed to output an image`);
        }
      ...
      insertGlabsTask(finalVideoTaskId, task.id, c, prompts[c - 1].t2v_prompt || prompts[c - 1].prompt || '');
    }
  ...
  // Case B: Task IDs are already submitted, poll them
    ...
    for (let i = 0; i < taskIds.length; i++) {
      ...
      const cached = db.prepare('SELECT status, video_url FROM glabs_tasks WHERE task_id = ?').get(taskId);
...
async function handleFFmpegMuxing(task) {
  updateMultiplierTask(task.id, { status: 'ffmpeg_muxing' });
...
async function uploadCompletedTaskToNextcloud(taskId, localVideoPath) {
  ...
  try {
    const db = getDb();
    const task = db.prepare('SELECT * FROM re_multiplier_tasks WHERE id = ?').get(taskId);
    if (!task) return;

    const { uploadFileToNextcloud, uploadBufferToNextcloud } = await import('./nextcloud-helper');
    const targetFolder = getSettingVal('nextcloud_target_folder') || '/MAKNA_Video_Generations';
