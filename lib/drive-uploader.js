 ...
 */
export async function getOrCreateRootFolder(drive, rootFolderName = ROOT_FOLDER_NAME) {
  // If it's the default MAKNA Assets, we can use the cached setting
  let rootId = null;
 ...
 */
export async function getOrCreateCampaignFolder(campaignName, prefix = 'RE_Campaign_') {
  const auth = getAuthorizedClient();
  const drive = google.drive({ version: 'v3', auth });

  const rootId = await getOrCreateRootFolder(drive, ROOT_FOLDER_NAME);
