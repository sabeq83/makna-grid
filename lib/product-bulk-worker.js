...
export async function processProductBulkEnrichment() {
  ...
  // =========================================================================
  const pendingProducts = db.prepare("SELECT * FROM product_extractions WHERE extraction_status = 'pending' LIMIT 10").all();
...
3. ID produk wajib sesuai dengan data input.`;
  ...
  // =========================================================================
  const pendingImageProducts = db.prepare("SELECT * FROM product_extractions WHERE extraction_status = 'pending_image' LIMIT 5").all();
  ...
  for (const product of pendingImageProducts) {
    try {
      ...
      if (product.scraped_image_url && product.scraped_image_url.startsWith('http')) {
        try {
          ...
          if (downloadSuccess && fs.existsSync(refAbsPath)) {
            ...
            db.prepare("UPDATE product_extractions SET raw_photo_url = ? WHERE id = ?").run(localRelPath, product.id);
      ...
      if (res && res.task_id) {
        db.prepare("UPDATE product_extractions SET glabs_task_id = ?, extraction_status = 'generating_image' WHERE id = ?")
      ...
      db.prepare("UPDATE product_extractions SET extraction_status = 'failed' WHERE id = ?").run(product.id);
    }
  }
  ...
  // =========================================================================
  const activeImageTasks = db.prepare("SELECT * FROM product_extractions WHERE extraction_status = 'generating_image'").all();
