  ...
  try {
    const opcItems = db.prepare(`
      SELECT pci.*, pc.campaign_name, pc.brand_profile_id, pc.target_product_id,
             bp.account_name AS bp_account_name, bp.brand_name AS bp_brand_name,
             pe.product_name AS pe_product_name, pe.source_url AS pe_source_url
    ...
    for (const item of opcItems) {
      ...
      const accountName = item.bp_account_name || item.bp_brand_name || payload.account_name || 'Umum';
