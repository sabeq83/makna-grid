 ...
 */
export function scanAndSyncExistingCampaigns(targetCampaignId = null) {
  const db = getDb();
  let totalIngested = 0;
  ...
  try {
    ...
    const opcItems = db.prepare(`
      SELECT pci.*, pc.campaign_name, pc.brand_profile_id, pc.target_product_id,
             ...
             (SELECT cp.affiliate_url FROM content_planners cp WHERE cp.product_id = pc.target_product_id AND cp.affiliate_url IS NOT NULL AND cp.affiliate_url != '' LIMIT 1) AS cp_affiliate_url
  ...
  try {
    ...
    for (const item of scItems) {
      ...
      upsertContentFlowItem({
        ...
        link_affiliate: creative.affiliate_url || item.pe_affiliate_link || '',
        link_produk: productUrl,
        caption: caption,
        production_date: item.created_at,
        url_asset: urlAsset,
