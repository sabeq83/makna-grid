 ...
 */
function sanitizeAccountName(rawName) {
  if (!rawName) return 'umum';
  const name = rawName.toLowerCase();
  if (name.includes('dapurbotani')) return 'dapurbotani';
  if (name.includes('nutribake')) return 'nutribake';
  if (name.includes('siasatsehat')) return 'siasatsehat';
  if (name.includes('nutriblend')) return 'nutriblend';
  if (name.includes('mealprepid')) return 'mealprepid';
  if (name.includes('contohbrand')) return 'contohbrand';
  return name.trim();
}
...
export async function scanAndSyncExistingCampaigns(targetCampaignId = null) {
  ...
  try {
    ...
    for (const item of opcItems) {
      ...
      const accountName = sanitizeAccountName(item.bp_brand_name || payload.account_name || (item.campaign_name && item.campaign_name.includes('_') ? item.campaign_name.split('_')[0] : 'Umum'));
      ...
      if (productUrl && (!item.pe_product_name || !item.pe_affiliate_link)) {
        try {
          peFallback = db.prepare('SELECT product_name, affiliate_link FROM product_extractions WHERE source_url = ? LIMIT 1').get(productUrl);
        } catch (_) {}
      }
  ...
  try {
    ...
    const scQueryParams = [];
    if (targetCampaignId) {
      scWhereClause = 'WHERE sc.id = ?';
      scQueryParams.push(targetCampaignId);
    }
    ...
    for (const item of scItems) {
      ...
      const accountName = sanitizeAccountName(pubPkg.account_name || (item.campaign_name && item.campaign_name.includes('_') ? item.campaign_name.split('_')[0] : 'Umum'));
      if (!campaignSeqCounters[item.campaign_id]) {
        campaignSeqCounters[item.campaign_id] = 1;
      }
      const seqNum = campaignSeqCounters[item.campaign_id]++;
  ...
  try {
    ...
    const reQueryParams = [];
    if (targetCampaignId) {
      reWhereClause = 'WHERE rc.id = ?';
      reQueryParams.push(targetCampaignId);
    }

    const reItems = db.prepare(`
      SELECT rci.*, rc.campaign_name, rc.target_product_id,
    ...
    for (const item of reItems) {
      ...
      const accountName = sanitizeAccountName(item.account_name || (item.campaign_name && item.campaign_name.includes('_') ? item.campaign_name.split('_')[0] : 'Umum'));
  ...
  try {
    ...
    const singleBridgeCampaigns = db.prepare(`
    ...
    for (const camp of singleBridgeCampaigns) {
      ...
      if (isFailed) {
        db.prepare('DELETE FROM content_flow_items WHERE id = ? OR source_item_id = ?').run(cfId, String(camp.id));
        continue;
      }
...
export async function syncBridgeCampaignToContentFlow(campaignId) {
  ...
  const db = getDb();
  const cfItems = db.prepare("SELECT * FROM content_flow_items WHERE source_campaign_id = ?").all(campaignId);
