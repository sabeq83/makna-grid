...
export async function scanAndSyncExistingCampaigns(targetCampaignId = null) {
  ...
  try {
    ...
    for (const item of opcItems) {
      ...
      const accountName = item.bp_brand_name || payload.account_name || (item.campaign_name && item.campaign_name.includes('_') ? item.campaign_name.split('_')[0] : 'Umum');
      ...
      const productName = item.pe_product_name || (peFallback ? peFallback.product_name : '') || payload.product_name || 'Umum';
  ...
  try {
    ...
    for (const item of scItems) {
      ...
      const accountName = pubPkg.account_name || (item.campaign_name && item.campaign_name.includes('_') ? item.campaign_name.split('_')[0] : 'Umum');
      if (!campaignSeqCounters[item.campaign_id]) {
        campaignSeqCounters[item.campaign_id] = 1;
      }
      const seqNum = campaignSeqCounters[item.campaign_id]++;
  ...
  try {
    ...
    for (const item of reItems) {
      ...
      const accountName = item.account_name || (item.campaign_name && item.campaign_name.includes('_') ? item.campaign_name.split('_')[0] : 'Umum');
  ...
  try {
    ...
    for (const item of bridgeItems) {
      ...
      const urlAsset = (item.clip2_video_path && !item.clip2_video_path.includes('index.php')) ? item.clip2_video_path : rawNcUrl;
