/**
 * MAKNA v54.9 — 5-Stage Pipeline Prompt Engineering
 * 5 Agen Spesialis: Product → Ideation → Narration → Visual → Prompt Engineer
 * + Reverse Engineering (standalone tool)
 */
...
3. 3 FORMULA STORYTELLING OBJEKTIF & REALISTIS (GANTIKAN KARANGAN BOHONG):
   ...
   - Formula C (Fact-to-Solution Journey): Perjalanan dari masalah fisik ke fakta ilmiah & solusi produk (Contoh: "Daun jati cina dikenal efektif melancarkan pencernaan secara alami. Dengan Nezafit Teh Daun Jati Cina, rutinitas detox malam jadi jauh lebih praktis.").
========4. ATURAN JANGKAR KEBENARAN MANFAAT PRODUK (PRIMARY PRODUCT TRUTH ANCHOR & ANTI-OVERCLAIM):
   ...
   - Jika deskripsi mentah e-commerce/produk memuat klaim berlebihan ("detoks usus kotor", "tidak menyebabkan ketergantungan", "tanpa efek samping", "menyembuhkan penyakit"), Anda WAJIB MENGABAIKAN KLAIM OVERCLAIM TERSEBUT dan HANYA MENGGUNAKAN MANFAAT RESMI DARI DATA USP PRODUK YANG SAH.
================================================================
`;
...
`;
  ...
  if (campaignData.is_bridging_active && productData) {
    const productName = productData.product_name || "";
    const productDesc = productData.product_description || "";
    const productUsp = typeof productData.unique_selling_point === 'string' ? productData.unique_selling_point : JSON.stringify(productData.unique_selling_point || "");
...
Return ONLY valid JSON (no markdown code blocks, no explanation). Use this exact structure:
{
  ...
  "voiceover": [
    ...
    {
      "scene": ${targetClips},
      "narration": "Naskah audio penutup klip terakhir.${campaignData.custom_instruction ? ` MANDATORY: Akhiran/penutup kalimat naskah Klip Terakhir ini WAJIB mengakhiri dengan kalimat: \\"${campaignData.custom_instruction}\\"` : ''}",
