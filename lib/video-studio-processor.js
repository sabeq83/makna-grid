...
export async function processVideoMuxing(jobConfig) {
  ...
  return new Promise((resolve, reject) => {
    command
      .save(resolvedOutput)
      .on('start', (cmd) => {
        console.log('[FFmpeg Video Studio] Executing Multi-Concat:', cmd);
      })
      .on('end', () => {
        console.log('[FFmpeg Video Studio] Rendering complete:', resolvedOutput);
        resolve(resolvedOutput);
      })
      .on('error', (err) => {
        console.error('[FFmpeg Video Studio] FFmpeg rendering error:', err);
        reject(err);
      });
  });
}
...
export async function processTtsStudioMuxing(config) {
  ...
  console.log(`[FFmpeg Video Studio] Paired ${pairCount} clips from batch ${batchId} for rendering.`);
  ...
  try {
    ...
    for (let i = 0; i < pairCount; i++) {
      ...
      console.log(`[FFmpeg Video Studio] Segment ${i + 1}/${pairCount}: Muxing video ${videoPaths[i]} with TTS audio ${completedClips[i].audio_path}`);
      ...
      if (syncOption === 'smart_sync') {
        ...
        console.log(`[FFmpeg Video Studio] Segment ${i + 1}/${pairCount}: Smart Sync resolved option to '${resolvedSyncOption}'`);
      }
    ...
    console.log(`[FFmpeg Video Studio] Concatenating ${tempClipPaths.length} segments into final output: ${outputPath}`);
    ...
    await new Promise((resolve, reject) => {
      concatCommand
        .save(resolvedOutput)
        .on('start', (cmd) => {
          console.log('[FFmpeg Video Studio] Executing final segment concat:', cmd);
        })
        .on('end', () => {
          console.log('[FFmpeg Video Studio] Segment concat complete:', resolvedOutput);
          resolve(resolvedOutput);
        })
        .on('error', (err) => {
          console.error('[FFmpeg Video Studio] Segment concat error:', err);
          reject(err);
        });
    });
