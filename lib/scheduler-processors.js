...
export function getCampaignNextcloudTargetFolder(campaign, fallbackS) {
  ...
  if (!parentFolder && campaign?.account_name) {
    try {
      const db = getDb();
      const bp = db.prepare('SELECT nextcloud_target_folder FROM brand_profiles WHERE LOWER(brand_name) = LOWER(?)').get(campaign.account_name);
...
export async function processAnalyzer(payload, job) {
  ...
  try {
    ...
    const result = await executeWithKeyPool(1, async (apiKey) => {
      ...
      const kbs = getAllKnowledgeBasesWithContent();
    ...
    try {
      ...
      try {
        ...
        const campaign = getReCampaign(reId);
...
export async function processReScraper(payload, job) {
  ...
  if (itemId) {
    ...
    const db = getDb();
    item = db.prepare('SELECT * FROM re_campaign_items WHERE id = ?').get(itemId);
  } else {
...
export async function processReAnalyzer(payload, job) {
  ...
  const campaign = getReCampaign(item.campaign_id);
  ...
  try {
    if (campaign.is_bridging_active && item.product_url && item.product_url.trim() !== '') {
      const product = db.prepare('SELECT * FROM product_extractions WHERE input_source = ? OR source_url = ?').get(item.product_url, item.product_url);
      if (product) {
    ...
    if (campaign.brand_profile_id) {
      ...
      brandConfig = getBrandProfile(campaign.brand_profile_id);
    }
      ...
      try {
        ...
        if (visualOverrides) {
          ...
          if (visualOverrides.wardrobe_style === 'random') {
            ...
            const siblingItems = db.prepare('SELECT id FROM re_campaign_items WHERE campaign_id = ? ORDER BY id ASC').all(campaign.id);
    ...
    const result = await executeWithKeyPool(1, async (apiKey) => {
      ...
      if (campaign.is_bridging_active) {
        ...
        const productData = await resolveProductData(campaign);
    ...
    const applyReplacements = (promptStr) => {
      ...
      if (visualOverrides) {
        ...
        if (!isMascotUniverse) {
          s = s.replace(/\(Biometric Anchor: [^\)]+\)/gi, `(Biometric Anchor: ${targetCharacter})`);
          s = s.replace(/\(Anchor: [^\)]+\)/gi, `(Anchor: ${targetCharacter})`);
          s = s.replace(/\(Wardrobe: [^\)]+\)/gi, `(Wardrobe: ${targetWardrobe})`);
          s = s.replace(/\(Lighting: [^\)]+\)/gi, `(Lighting: ${targetLighting})`);
        }
      }
      return s;
    };
    ...
    if (campaign.enable_audio_segment === 1) {
      ...
      if (updatedVoiceCastJson) {
        ...
        db.prepare('UPDATE re_campaigns SET voice_cast_json = ? WHERE id = ?').run(updatedVoiceCastJson, campaign.id);
    ...
    if (campaign.enable_vo_audit !== 1) { // RE Campaign forces Double-Pass T2I + I2V for all clips only when VO audit is disabled
      ...
      const brandProfile = db.prepare('SELECT * FROM brand_profiles WHERE LOWER(brand_name) = LOWER(?)').get(campaign.account_name || '');
      const fs = await import('fs');
      const path = await import('path');
    ...
    try {
      ...
      const existingIds = (existingRes.data?.values || []).flat().filter(v => v && v.startsWith('RE-'));
      const batchId = generateREBatchId(existingIds, campaign.campaign_name);
      ...
      try {
        ...
        const storageProvider = getS('storage_provider') || 'gdrive';
        const { getReCampaign } = await import('./db');
        const dbCampaign = getReCampaign(campaign.id) || campaign;
...
export async function resolveProductData(campaignRow) {
  if (!campaignRow.is_bridging_active) {
    return {
      product_name: '',
      product_description: '',
  ...
  if (bridging_mode === 'select_existing') {
    const db = getDb();
    const product = db.prepare('SELECT * FROM product_extractions WHERE id = ?').get(target_product_id);
    if (!product) {
  ...
  if (bridging_mode === 'url_extract') {
    ...
    const existingProduct = db.prepare('SELECT * FROM product_extractions WHERE input_source = ?').get(targetUrl);
    if (existingProduct) {
      console.log(`[resolveProductData] Found existing product data in database for URL: ${targetUrl}`);
      
      const isRe = db.prepare('SELECT 1 FROM re_campaigns WHERE id = ?').get(campaignRow.id);
      const isBridge = db.prepare('SELECT 1 FROM bridge_injector_campaigns WHERE id = ?').get(campaignRow.id);

      if (isRe) {
        db.prepare("UPDATE re_campaigns SET bridging_mode = 'select_existing', target_product_id = ? WHERE id = ?")
          .run(existingProduct.id, campaignRow.id);
      } else if (isBridge) {
        db.prepare("UPDATE bridge_injector_campaigns SET bridging_mode = 'select_existing', target_product_id = ? WHERE id = ?")
          .run(existingProduct.id, campaignRow.id);
      } else {
        db.prepare("UPDATE pillar_campaigns SET bridging_mode = 'select_existing', target_product_id = ? WHERE id = ?")
          .run(existingProduct.id, campaignRow.id);
      }
    ...
    if (isRe) {
      if (localImagePath && safeFilename) {
        db.prepare("UPDATE re_campaigns SET bridging_mode = 'select_existing', target_product_id = ?, product_ref_image_path = ?, product_filename_declare = ? WHERE id = ?")
      ...
      if (localImagePath && safeFilename) {
        db.prepare("UPDATE pillar_campaigns SET bridging_mode = 'select_existing', target_product_id = ?, product_ref_image_path = ?, product_filename_declare = ? WHERE id = ?")
          .run(newProductId, localImagePath, safeFilename, campaignRow.id);
      } else {
        db.prepare("UPDATE pillar_campaigns SET bridging_mode = 'select_existing', target_product_id = ? WHERE id = ?")
          .run(newProductId, campaignRow.id);
...
export async function processReTts(payload, job) {
  ...
  try {
    ...
    const batchId = `ttsb_re_${Date.now()}`;
    ...
    try {
      ...
      const parentFolderName = getCampaignParentFolderName(campaign, 'RE');
      if (storageProvider === 'nextcloud') {
        const { syncReCampaignAssetsToNextcloud } = await import('./nextcloud-sync-helper');
        const targetFolder = getCampaignNextcloudTargetFolder(campaign);
        const parentFolder = `${targetFolder}/${parentFolderName}`.replace(/\/+/g, '/');
        await syncReCampaignAssetsToNextcloud(campaign, [item], parentFolder);
      } else {
        const { syncReCampaignAssetsToDrive } = await import('./drive-sync-helper');
        const { getOrCreateCampaignFolder } = await import('./drive-uploader');
        const campaignFolderId = await getOrCreateCampaignFolder(parentFolderName, '');
        await syncReCampaignAssetsToDrive(campaign, [item], campaignFolderId);
      }
    } catch (uploadErr) {
      console.error('[RE TTS Cloud Sync] Failed to sync audio files to cloud:', uploadErr.message);
    }
...
export async function processReGlabs(payload, job) {
  ...
  if (payload?.target_type === 'angle_variant') {
    ...
    const variant = db.prepare('SELECT * FROM re_item_angle_variants WHERE id = ?').get(variantId);
    if (!variant) {
      throw new Error(`Angle variant ${variantId} not found`);
    }

    const parentItem = db.prepare('SELECT * FROM re_campaign_items WHERE id = ?').get(variant.re_item_id);
    const campaign = db.prepare('SELECT * FROM re_campaigns WHERE id = ?').get(parentItem.campaign_id);
    const brandProfile = db.prepare('SELECT * FROM brand_profiles WHERE LOWER(brand_name) = LOWER(?)').get(campaign.account_name || '');
    ...
    if (enableTts && variant.tts_status !== 'completed') {
      db.prepare("UPDATE re_item_angle_variants SET tts_status = 'processing' WHERE id = ?").run(variantId);
      try {
        ...
        if (provider !== 'minimax' && clips.length > 1) {
          try {
            ...
            db.prepare('DELETE FROM tts_studio_clips WHERE batch_id = ?').run(ttsBatchId);
          }
        }
        ...
        db.prepare("UPDATE re_item_angle_variants SET tts_status = 'completed', tts_batch_id = ? WHERE id = ?").run(ttsBatchId, variantId);
      } catch (ttsErr) {
        db.prepare("UPDATE re_item_angle_variants SET tts_status = 'failed' WHERE id = ?").run(variantId);
        const { logSystemError } = await import('./error-logger');
        logSystemError('RE G-Labs Angle Variant TTS', ttsErr, campaign.id);
        throw ttsErr;
      }
    } else if (!enableTts) {
      db.prepare("UPDATE re_item_angle_variants SET tts_status = 'skipped' WHERE id = ?").run(variantId);
    }
    ...
    db.prepare("UPDATE re_item_angle_variants SET visual_status = 'processing' WHERE id = ?").run(variantId);
    try {
      const clips = JSON.parse(variant.visual_tasks_json || '[]');
      ...
      for (let i = 0; i < clips.length; i++) {
        const clip = clips[i];
        const cNum = i + 1;
        const isHybridLockClip = true; // RE Campaign forces Double-Pass T2I + I2V for all clips
        ...
        if (existingTaskId) {
          let existingTask = db.prepare('SELECT * FROM glabs_tasks WHERE task_id = ?').get(existingTaskId);
          if (existingTask) {
              ...
              try {
                ...
                if (taskStatus === 'completed') {
                  ...
                  if (videoFile) {
                    ...
                    existingTask = db.prepare('SELECT * FROM glabs_tasks WHERE task_id = ?').get(existingTaskId);
                  }
                } else if (taskStatus === 'failed') {
          ...
          if (startFrameExists) {
            ...
            for (let attempt = 0; attempt < maxT2iAttempts; attempt++) {
              ...
              if (t2iStatus === 'completed') {
                ...
                throw new Error(`T2I task ${t2iTaskId} failed on GLabs`);
              }
            }
      ...
      db.prepare("UPDATE re_item_angle_variants SET glabs_task_ids = ? WHERE id = ?").run(JSON.stringify(glabsTaskIds), variantId);
    } catch (glabsSubmitErr) {
      db.prepare("UPDATE re_item_angle_variants SET visual_status = 'failed' WHERE id = ?").run(variantId);
    ...
    while (attempts < maxAttempts) {
      ...
      for (let i = 0; i < glabsTaskIds.length; i++) {
        ...
        try {
          ...
          const cachedTask = db.prepare('SELECT status, video_url FROM glabs_tasks WHERE task_id = ?').get(taskId);
      ...
      for (const taskId of glabsTaskIds) {
        const t = db.prepare('SELECT status FROM glabs_tasks WHERE task_id = ?').get(taskId);
    ...
    db.prepare("UPDATE re_item_angle_variants SET visual_status = 'completed', visual_clip_paths = ? WHERE id = ?").run(JSON.stringify(localPaths), variantId);
    ...
    if (enableFfmpeg) {
      db.prepare("UPDATE re_item_angle_variants SET ffmpeg_status = 'processing' WHERE id = ?").run(variantId);
      try {
        ...
        db.prepare("UPDATE re_item_angle_variants SET ffmpeg_status = 'completed', ffmpeg_output_path = ?, upload_status = 'uploading' WHERE id = ?").run(`/temp/${finalVideoFileName}`, variantId);
      } catch (ffmpegMuxErr) {
        db.prepare("UPDATE re_item_angle_variants SET ffmpeg_status = 'failed' WHERE id = ?").run(variantId);
        throw ffmpegMuxErr;
      }
    } else {
      db.prepare("UPDATE re_item_angle_variants SET ffmpeg_status = 'skipped', upload_status = 'uploading' WHERE id = ?").run(variantId);
    }
    ...
    try {
      ...
      const itemIndex = campaignItems.findIndex(i => i.id === parentItem.id);
      const sequenceNumber = itemIndex !== -1 ? itemIndex + 1 : 1;
      const paddedIndex = String(sequenceNumber).padStart(3, '0');

      const safeCampaignName = campaign.campaign_name.replace(/[^a-zA-Z0-9_]/g, '_');
      const safeAngleName = variant.angle_name.replace(/[^a-zA-Z0-9_]/g, '_');
      const batchFolderName = `RE-${campaign.campaign_name}-${paddedIndex}-Angle-${variant.angle_name}`;

      if (storageProvider === 'nextcloud') {
        const { uploadBufferToNextcloud } = await import('./nextcloud-helper');
        const targetFolder = campaign?.nextcloud_parent_folder ? `/${campaign.nextcloud_parent_folder.replace(/^\/+/, '')}` : (getSetting('nextcloud_target_folder') || '/MAKNA_Video_Generations');
        const basePath = `${targetFolder}/RE_Campaign_${safeCampaignName}/${batchFolderName}`.replace(/\/+/g, '/');
        ...
        for (let i = 0; i < localPaths.length; i++) {
          ...
          if (fs.existsSync(startFrameLocalPath)) {
            ...
            const cloudStartFrameName = `RE-${campaign.campaign_name}-${paddedIndex}-Angle-${variant.angle_name}_start_frame_${i + 1}.png`;
            await uploadBufferToNextcloud(startFrameBuffer, `${basePath}/${cloudStartFrameName}`);
          }
        }

        if (enableFfmpeg && finalVideoPath) {
          const finalFileName = `RE-${campaign.campaign_name}-${paddedIndex}-Angle-${variant.angle_name}_video_final.mp4`;
          ...
          for (let i = 0; i < localPaths.length; i++) {
            ...
            const clipFileName = `RE-${campaign.campaign_name}-${paddedIndex}-Angle-${variant.angle_name}_video_clip_${i + 1}.mp4`;
        ...
        const campaignFolderName = `RE_Campaign_${safeCampaignName}`;
        ...
        if (searchSub.data.files?.length) {
          ...
          const createdSub = await drive.files.create({
            requestBody: { name: campaignFolderName, mimeType: 'application/vnd.google-apps.folder', parents: [targetFolderId] },
            fields: 'id',
          });
      ...
      const subPath = `RE_Campaign_${campaign.id}/${batchFolderName}`;
      if (enableFfmpeg && finalVideoPath) {
        const finalFileName = `RE-${campaign.campaign_name}-${paddedIndex}-Angle-${variant.angle_name}_video_final.mp4`;
      ...
      db.prepare("UPDATE re_item_angle_variants SET upload_status = 'completed', drive_link = ? WHERE id = ?").run(driveUrl, variantId);
      console.log(`[RE+AM Render] Successfully completed all rendering and upload steps for variant: ${variant.angle_name}`);
    } catch (uploadErr) {
      db.prepare("UPDATE re_item_angle_variants SET upload_status = 'failed' WHERE id = ?").run(variantId);
      const { logSystemError } = await import('./error-logger');
  ...
  if (item) {
    ...
    const campaign = db.prepare('SELECT * FROM re_campaigns WHERE id = ?').get(item.campaign_id);
    ...
    for (let i = 0; i < taskIds.length; i++) {
      ...
      try {
        ...
        if (taskResult?.error) {
          ...
          anyFailed = true;
          errorDetail = `Task ${taskId} failed on GLabs`;
    ...
    if (allDone) {
      ...
      try {
        const storageProvider = getSetting('storage_provider') || 'gdrive';
        const campaignItems = db.prepare('SELECT id FROM re_campaign_items WHERE campaign_id = ? ORDER BY id ASC').all(item.campaign_id);
...
export async function processReFfmpeg(payload, job) {
  ...
  try {
    ...
    const batchId = getReBatchId(campaign, item, db);
    ...
    if (hasNextcloud) {
      try {
        ...
        const finalVideoNextcloudPath = `${nextcloudBaseFolder}/${batchId}_video_final.mp4`;
        const fileBuffer = fs.readFileSync(finalVideoPath);
        ...
        const markdownNextcloudPath = `${nextcloudBaseFolder}/${batchId}_naskah.md`;
        ...
        for (let i = 0; i < clips.length; i++) {
          ...
          if (fs.existsSync(clipPath)) {
            const clipBuffer = fs.readFileSync(clipPath);
            const targetVideoClipPath = `${nextcloudBaseFolder}/${batchId}_video_clip_${i + 1}.mp4`;
        ...
        for (let i = 0; i < clips.length; i++) {
          ...
          if (fs.existsSync(startFrameLocalPath)) {
            const startFrameBuffer = fs.readFileSync(startFrameLocalPath);
            const cloudStartFrameName = `${batchId}_start_frame_${i + 1}.png`;
          ...
          for (const clip of ttsClips) {
            if (clip.audio_path) {
              ...
              if (fs.existsSync(audioLocalPath)) {
                ...
                const targetAudioClipPath = `${nextcloudBaseFolder}/${batchId}_audio_clip_${clip.clip_index + 1}.${extension}`;
                const audioBuffer = fs.readFileSync(audioLocalPath);
    ...
    if (storageProvider === 'nextcloud' && !hasNextcloud) {
      ...
      const uploaded = await uploadVideoToFolder(finalVideoPath, finalVideoFileName, batchFolderId);
      uploadLink = `https://drive.google.com/drive/folders/${batchFolderId}`;
      ...
      for (let i = 0; i < clips.length; i++) {
        ...
        const clipPath = path.join(process.cwd(), 'public', clipRelativePath);
        if (fs.existsSync(clipPath)) {
          const clipFileName = `${batchId}_video_clip_${i + 1}.mp4`;
        ...
        try {
          ...
          for (const clip of ttsClips) {
            if (clip.audio_path) {
              const audioLocalPath = path.join(process.cwd(), 'public', clip.audio_path);
              if (fs.existsSync(audioLocalPath)) {
                const audioFileName = `${batchId}_audio_clip_${clip.clip_index + 1}.mp3`;
                await uploadLocalFileToFolder(audioLocalPath, audioFileName, batchFolderId, 'audio/mpeg');
              }
            }
          }
    ...
    // Save copy to local storage
    await saveFileToLocalStorage(finalVideoPath, `RE_Campaign_${campaign.id}/${batchFolderName}`, finalVideoFileName);
    ...
    try {
      const { syncCampaignItemToContentFlow } = await import('./strategic-campaign-engine');
      await syncCampaignItemToContentFlow(campaign, item, uploadLink);
    } catch (cfErr) {
...
export async function processItemMultiAngle(itemId, angleCount) {
  ...
  if (item.is_bridging_active && item.target_product_id) {
    product = db.prepare("SELECT * FROM product_extractions WHERE id = ?").get(item.target_product_id);
  }
...
export async function processRowProductSourcing(payload, job) {
  ...
  let item;
  if (itemId) {
    item = db.prepare('SELECT * FROM pillar_campaign_items WHERE id = ?').get(itemId);
  } else {
  ...
  if (
    ...
    if (checkUrl && checkUrl.trim() !== '') {
      try {
        const existing = db.prepare('SELECT * FROM product_extractions WHERE input_source = ? OR source_url = ?').get(checkUrl, checkUrl);
        if (existing) {
    ...
    try {
      ...
      const existing = db.prepare('SELECT id FROM product_extractions WHERE product_name = ? AND (source_url = ? OR input_source = ?)').get(creativePayload.product_name, url, url);
      if (!existing) {
  ...
  try {
    const existing = db.prepare('SELECT * FROM product_extractions WHERE input_source = ? OR source_url = ?').get(url, url);
    if (existing) {
      ...
      creativePayload.product_ref_image_path = creativePayload.product_ref_image_path || existing.photo_url;
      creativePayload.packaging_type = creativePayload.packaging_type || existing.packaging_type || '';
      creativePayload.is_in_packaging = creativePayload.is_in_packaging !== undefined ? creativePayload.is_in_packaging : (existing.is_in_packaging || 0);
      creativePayload.t2i_prompt = creativePayload.t2i_prompt || existing.t2i_prompt || '';
      creativePayload.i2v_action_prompt = creativePayload.i2v_action_prompt || existing.i2v_action_prompt || '';

      // Extract filename from photo_url path if not present
      if (!creativePayload.product_filename_declare && existing.photo_url) {
        creativePayload.product_filename_declare = path.basename(existing.photo_url);
      }
  ...
  try {
    ...
    try {
      const existing = db.prepare('SELECT id FROM product_extractions WHERE input_source = ?').get(url);
      if (!existing) {
...
export async function processPillarGenerator(payload, job) {
  ...
  try {
    ...
    const isMassProd = campaign.is_mass_production === 1;
      ...
      try {
        ...
        if (visualOverrides) {
          ...
          if (visualOverrides.wardrobe_style === 'random') {
            ...
            const rowNum = rowPayload.row_number || 1;
            const idx = (rowNum - 1) % wardrobePresets.length;
            visualOverrides.wardrobe_style = wardrobePresets[idx];
          }
    ...
    if (isMassProd) {
      productData = {
        ...
        packaging_type: rowPayload.packaging_type || '',
        is_in_packaging: rowPayload.is_in_packaging !== undefined ? rowPayload.is_in_packaging : 0,
        product_truth: rowPayload.product_truth || '',
        geometric_truth: rowPayload.geometric_truth || '',
      ...
      if (visualOverrides) {
        ...
        if (!isMascotUniverse) {
          if (!/\(Wardrobe:/i.test(s)) {
            s = s.replace(/\(Anchor: ([^\)]+)\)/i, `(Anchor: $1), (Wardrobe: ${targetWardrobe})`);
          } else {
            s = s.replace(/\(Wardrobe: [^\)]+\)/gi, `(Wardrobe: ${targetWardrobe})`);
          }
          s = s.replace(/\(Biometric Anchor: [^\)]+\)/gi, `(Biometric Anchor: ${targetCharacter})`);
    ...
    if (campaign.enable_audio_segment === 1) {
      ...
      if (updatedVoiceCastJson) {
        ...
        db.prepare('UPDATE pillar_campaigns SET voice_cast_json = ? WHERE id = ?').run(updatedVoiceCastJson, campaign.id);
        campaign.voice_cast_json = updatedVoiceCastJson;
      }
    }
    ...
    updatePillarCampaignItem(item.id, {
      ...
      video_dna_json: JSON.stringify(videoDna),
      t2i_images_json: JSON.stringify(t2iImagePaths),
      workflow_status: 'ready_for_review' // Trigger human-in-the-loop review pause
    });
    ...
    if (uploadSpreadsheet || uploadMarkdown) {
      try {
        ...
        const batchId = getOpcBatchId(campaign, item, db);
        const campaignItems = db.prepare('SELECT id FROM pillar_campaign_items WHERE campaign_id = ? ORDER BY id ASC').all(campaign.id);
        const itemIndex = campaignItems.findIndex(i => i.id === item.id);
        const sequenceNumber = itemIndex !== -1 ? itemIndex + 1 : 1;
        const paddedIndex = String(sequenceNumber).padStart(3, '0');
...
export async function processPillarTts(payload, job) {
  ...
  if (itemId) {
    ...
    item = getNextPendingPillarTtsItem();
  }
  ...
  try {
    ...
    const batchId = `ttsb_opc_${Date.now()}`;
    ...
    try {
      ...
      const parentFolderName = getCampaignParentFolderName(campaign, 'OPC');
      if (storageProvider === 'nextcloud') {
        const { syncOpcCampaignAssetsToNextcloud } = await import('./nextcloud-sync-helper');
        const targetFolder = getCampaignNextcloudTargetFolder(campaign);
        const parentFolder = `${targetFolder}/${parentFolderName}`.replace(/\/+/g, '/');
        await syncOpcCampaignAssetsToNextcloud(campaign, [item], parentFolder);
      } else {
        const { syncOpcCampaignAssetsToDrive } = await import('./drive-sync-helper');
        const { getOrCreateCampaignFolder } = await import('./drive-uploader');
        const campaignFolderId = await getOrCreateCampaignFolder(parentFolderName, '');
        await syncOpcCampaignAssetsToDrive(campaign, [item], campaignFolderId);
      }
    } catch (uploadErr) {
      console.error('[Pillar TTS Cloud Sync] Failed to sync audio files to cloud:', uploadErr.message);
    }
...
export async function processPillarGlabs(payload, job) {
  ...
  if (item) {
    const campaign = db.prepare('SELECT * FROM pillar_campaigns WHERE id = ?').get(item.campaign_id);
    const taskIds = JSON.parse(item.visual_tasks_json || '[]');
    if (taskIds.length === 0) {
    ...
    if (allDone) {
      ...
      try {
        const storageProvider = getSetting('storage_provider') || 'gdrive';
        const campaignItems = db.prepare('SELECT id FROM pillar_campaign_items WHERE campaign_id = ? ORDER BY id ASC').all(item.campaign_id);
        const itemIndex = campaignItems.findIndex(i => i.id === item.id);
        const sequenceNumber = itemIndex !== -1 ? itemIndex + 1 : 1;
        const paddedIndex = String(sequenceNumber).padStart(3, '0');
      ...
      if (isHybridLockClip) {
        ...
        if (startFrameExists) {
          ...
          if (campaign?.target_product_id) {
            try {
              const activeProd = db.prepare('SELECT clean_photo_url, cleaned_photo_url, photo_url FROM product_extractions WHERE id = ?').get(campaign.target_product_id);
          ...
          const runT2I = async (useRefImage) => {
            const t2iResult = await generateImage({
              ...
              model: imageModel,
              aspect_ratio: campaign.aspect_ratio || '9:16',
              reference_images: (useRefImage && productBase64) ? [productBase64] : undefined,
              webhookOverride: brandProfile
            });
          ...
          try {
            t2iImageUrl = await runT2I(isBridge);
            t2iCompleted = true;
          } catch (err) {
            if (isBridge && productBase64) {
              console.warn(`[Pillar Glabs] T2I task for clip ${c} failed with reference image: ${err.message}. Retrying without reference image...`);
              t2iImageUrl = await runT2I(false);
              t2iCompleted = true;
            } else {
              throw err;
            }
...
export async function processPillarFfmpeg(payload, job) {
  ...
  if (itemId) {
    ...
    item = getNextPendingPillarFfmpegItem();
  }
  ...
  try {
    ...
    // Save copy to local storage
    const subPath = `OPC_Campaign_${campaign.id}/${batchId}`;
    const finalFileNameLocal = `${batchId}_video_final.mp4`;
    ...
    try {
      const { scanAndSyncExistingCampaigns } = await import('./contentflow-ingest');
      scanAndSyncExistingCampaigns(item.campaign_id);
    } catch (cfErr) {
...
export async function processInstantRowProductSourcing(payload, job) {
  ...
  let item;
  if (itemId) {
    item = db.prepare('SELECT * FROM instant_campaign_items WHERE id = ?').get(itemId);
  } else {
...
export async function processInstantGenerator(payload, job) {
  ...
  const campaign = getInstantCampaign(item.campaign_id);
  if (!campaign) return { skip: true, reason: `Campaign ${item.campaign_id} not found` };
...
export async function processInstantTts(payload, job) {
  ...
  try {
    ...
    const batchId = `ttsb_ifc_${Date.now()}`;
    const config = campaign.config || {};
    ...
    if (allCompleted && completedUrls.filter(Boolean).length === taskIds.length) {
      ...
      try {
        ...
        if (campaign) {
          overrideInstantCampaignIfMassProd(campaign, item);
          const brandProfile = db.prepare('SELECT * FROM brand_profiles WHERE LOWER(brand_name) = LOWER(?)').get(campaign.account_name || '');
          const campaignItems = db.prepare('SELECT id FROM instant_campaign_items WHERE campaign_id = ? ORDER BY id ASC').all(item.campaign_id);
          const itemIndex = campaignItems.findIndex(i => i.id === item.id);
          const sequenceNumber = itemIndex !== -1 ? itemIndex + 1 : 1;
          const paddedIndex = String(sequenceNumber).padStart(3, '0');
  ...
  try {
    ...
    try {
      ...
      if (storageProvider === 'nextcloud') {
        ...
        const batchId = getIfcBatchId(campaign, item, db);
        const targetFolder = getCampaignNextcloudTargetFolder(campaign);
        const nextcloudBaseFolder = `/${targetFolder}/${parentFolderName}/${batchId}`.replace(/\/+/g, '/');
        ...
        try {
          if (item.result_json) {
            ...
            const { buildMarkdownContent } = await import('./export-builder');
            const markdownContent = buildMarkdownContent(parsedResult, batchId);
            await uploadBufferToNextcloud(Buffer.from(markdownContent, 'utf-8'), `${nextcloudBaseFolder}/${batchId}_naskah.md`);
            await uploadBufferToNextcloud(Buffer.from(markdownContent, 'utf-8'), `${nextcloudBaseFolder}/naskah.md`);
          }
        } catch (mdErr) {
          ...
          try {
            ...
            for (const clip of ttsClips) {
              if (clip.audio_path) {
                const audioLocalPath = path.join(process.cwd(), 'public', clip.audio_path);
                if (fs.existsSync(audioLocalPath)) {
                  const audioFileName = `${batchId}_audio_${clip.clip_index + 1}.mp3`;
                  const audioTargetPath = `${nextcloudBaseFolder}/${audioFileName}`;
                  const audioBuffer = fs.readFileSync(audioLocalPath);
                  await uploadBufferToNextcloud(audioBuffer, audioTargetPath);
                }
        ...
        const campaignFolderId = await getOrCreateCampaignFolder(campaign.product_name, 'IFC_Campaign_');

        let targetUploadFolderId = campaignFolderId;
        if (campaign.is_mass_production === 1) {
          const subfolderName = `IFC_${campaign.product_name.replace(/[^a-zA-Z0-9_]/g, '_')}_${paddedIndex}`;
          targetUploadFolderId = await getOrCreateFolderInFolder(subfolderName, campaignFolderId);
        }
    ...
    // Save copy to local storage
    let subPath = `IFC_Campaign_${campaign.id}`;
    let finalFileNameLocal = `${campaign.product_name}_${paddedIndex}.mp4`;
    if (campaign.is_mass_production === 1) {
      const subfolderName = `IFC_${campaign.product_name.replace(/[^a-zA-Z0-9_]/g, '_')}_${paddedIndex}`;
      subPath = `${subPath}/${subfolderName}`;
      finalFileNameLocal = `IFC-${campaign.product_name.replace(/[^a-zA-Z0-9_]/g, '_')}-${paddedIndex}.mp4`;
    }
    await saveFileToLocalStorage(finalVideoPath, subPath, finalFileNameLocal);
...
export async function processProductScraper(payload, job) {
  ...
  const allProducts = db.prepare('SELECT id, input_source, source_url, raw_photo_url, cleaned_photo_url, generated_photo_url, unique_selling_point, packaging_type, scraped_image_url, raw_description, t2i_prompt, i2v_action_prompt, product_name FROM product_extractions').all();
...
export async function processRecipeGenerator(payload, job) {
  ...
  if (payload?.item_id) {
    item = db.prepare("SELECT * FROM recipe_items WHERE id = ?").get(payload.item_id);
  } else {
    item = db.prepare("SELECT * FROM recipe_items WHERE status = 'pending_gemini' ORDER BY created_at ASC LIMIT 1").get();
  }
  ...
  try {
    const campaign = getRecipeCampaignById(item.campaign_id);
    if (!campaign) {
    ...
    if (campaign.source_deconstruct_asset_id) {
      deconstructAsset = db.prepare("SELECT * FROM re_deconstructed_assets WHERE id = ?").get(campaign.source_deconstruct_asset_id);
    }
...
`;
    ...
    if (isVideoCampaign) {
      ...
      if (campaign.brand_profile_id) {
        brandProfile = getBrandProfile(campaign.brand_profile_id);
      }
      let config = null;
...
${promptsSchema}
    ...
    const rawResponse = await executeWithKeyPool(1, async (apiKey) => {
      ...
      let res;
      try {
        res = await tryGenerate('gemini-3.5-flash');
      } catch (err) {
        console.warn(`[Recipe Gemini Fallback] gemini-3.5-flash failed (${err.message}). Swapping to gemini-flash-latest...`);
        try {
          res = await tryGenerate('gemini-flash-latest');
        } catch (err2) {
          console.warn(`[Recipe Gemini Fallback] gemini-flash-latest failed (${err2.message}). Swapping to gemini-2.5-flash...`);
          res = await tryGenerate('gemini-flash-latest');
        }
      }
      return res.response.text();
    ...
    if (isVideoCampaign) {
      ...
      if (!isGlabsEnabled) {
        ...
        const remaining = db.prepare("SELECT COUNT(*) as cnt FROM recipe_items WHERE campaign_id = ? AND status != 'completed'").get(campaign.id);
        if (remaining.cnt === 0) {
...
export async function processRecipeGLabs(payload, job) {
  ...
  if (payload?.item_id) {
    ...
    item = db.prepare("SELECT * FROM recipe_items WHERE status = 'pending_glabs' ORDER BY created_at ASC LIMIT 1").get();
  }
  ...
  const brandProfile = campaign ? db.prepare('SELECT * FROM brand_profiles WHERE LOWER(brand_name) = LOWER(?)').get(campaign.account_name || '') : null;
...
export async function processRecipeExporter(payload, job) {
  ...
  if (payload?.item_id) {
    ...
    item = db.prepare("SELECT * FROM recipe_items WHERE status = 'pending_export' ORDER BY created_at ASC LIMIT 1").get();
  }
...
async function downloadAndUploadPillarClip(item, campaign, clipIndex, url, localPaths, db) {
  ...
  db.prepare('UPDATE pillar_campaign_items SET visual_clip_paths = ? WHERE id = ?').run(JSON.stringify(localPaths), item.id);
...
async function downloadAndUploadReClip(item, campaign, clipIndex, url, localPaths, db, isAngleVariant = false, variant = null) {
  ...
  localPaths[clipIndex] = localRelativePath;
  if (isAngleVariant) {
    db.prepare('UPDATE re_item_angle_variants SET visual_clip_paths = ? WHERE id = ?').run(JSON.stringify(localPaths), variant.id);
  } else {
    db.prepare('UPDATE re_campaign_items SET visual_clip_paths = ? WHERE id = ?').run(JSON.stringify(localPaths), item.id);
  }
...
export function getCampaignParentFolderName(campaign, type = 'OPC') {
  ...
  return `${type.toLowerCase()}_${productSlug}_${campaignHash}`;
}
...
export function buildStrategicMarkdownContent(campaign, item, scenes, publishingPkg) {
  const lines = [];
  const batchId = item.video_id || `SC-${item.id}`;

  lines.push(`# Strategic Campaign Script — ${batchId}`);
  lines.push('');
  lines.push(`**Campaign Name:** ${campaign.campaign_name || '-'}`);
  lines.push(`**Pillar:** ${item.pillar || '-'}`);
  lines.push(`**Product:** ${item.product || '-'}`);
  lines.push(`**Narrative Mode:** ${item.narrative_mode || 'Storytelling'}`);
  lines.push(`**Target Clips Count:** ${item.target_clips_count || scenes.length || 4}`);
  lines.push(`**Created At:** ${item.created_at || new Date().toISOString()}`);
  lines.push('');
  ...
  lines.push(`- **Strategic Angle:** ${item.strategic_angle || '-'}`);
  lines.push(`- **Hook:** ${item.hook || '-'}`);
  lines.push(`- **Visual Action:** ${item.visual_action || '-'}`);
  lines.push(`- **VFO:** ${item.vfo || '-'}`);
  lines.push('');
  ...
  scenes.forEach((sc, idx) => {
    lines.push(`### Scene ${sc.scene_number || (idx + 1)} — ${sc.duration_seconds || 8}s (${sc.scene_function || 'continuation'})`);
    lines.push(`- **Visual Action:** ${sc.visual_action || '-'}`);
    lines.push(`- **Voice Over / Narration:** *"${sc.voice_over || ''}"*`);
    lines.push(`- **On-Screen Text (OST):** \`${sc.on_screen_text || ''}\``);
    if (sc.t2i_prompt) {
      lines.push(`- **T2I Prompt:**`);
      lines.push('  ```');
      lines.push(`  ${sc.t2i_prompt}`);
      lines.push('  ```');
    }
    if (sc.i2v_prompt) {
      lines.push(`- **I2V Prompt:**`);
      lines.push('  ```');
      lines.push(`  ${sc.i2v_prompt}`);
      lines.push('  ```');
    }
    lines.push('');
  });

  if (publishingPkg && publishingPkg.publishing_assets) {
    lines.push('---');
    lines.push('## 📱 Social Media Package');
    lines.push('');
    const assets = publishingPkg.publishing_assets;
    if (assets.tiktok) {
      lines.push('### 🎵 TikTok');
      lines.push(`> **Caption:** ${assets.tiktok.caption || '-'}`);
      lines.push(`> **CTA:** ${assets.tiktok.cta || '-'}`);
      if (Array.isArray(assets.tiktok.hashtags)) {
        lines.push(`> **Hashtags:** ${assets.tiktok.hashtags.join(' ')}`);
      }
      lines.push('');
    }
    if (assets.instagram) {
      lines.push('### 📷 Instagram');
      lines.push(`> **Caption:** ${assets.instagram.caption || '-'}`);
      lines.push(`> **CTA:** ${assets.instagram.cta || '-'}`);
      if (Array.isArray(assets.instagram.hashtags)) {
        lines.push(`> **Hashtags:** ${assets.instagram.hashtags.join(' ')}`);
      }
      lines.push('');
    }
    if (assets.facebook) {
      lines.push('### 📘 Facebook');
      lines.push(`> **Caption:** ${assets.facebook.caption || '-'}`);
      lines.push(`> **CTA:** ${assets.facebook.cta || '-'}`);
      lines.push('');
    }
    if (assets.youtube) {
      lines.push('### 🎥 YouTube');
      lines.push(`> **Title:** ${assets.youtube.title || '-'}`);
      lines.push(`> **Description:** ${assets.youtube.description || '-'}`);
      if (Array.isArray(assets.youtube.hashtags)) {
        lines.push(`> **Hashtags:** ${assets.youtube.hashtags.join(' ')}`);
      }
      lines.push('');
    }
  }
...
// ==========================================
// STRATEGIC CAMPAIGNS (SC) PROCESSORS
// ==========================================

export async function processStrategicGenerator(payload, job) {
  const { getDb, updateStrategicCampaignItem } = await import('./db.js');
  const { executeCall1CreativeEngine, executeCall2PublishingEngine } = await import('./strategic-campaign-engine.js');
  const db = getDb();
  const itemId = payload?.item_id;

  if (!itemId) return { skip: true, reason: 'No item_id provided' };

  const item = db.prepare('SELECT * FROM strategic_campaign_items WHERE id = ?').get(itemId);
  if (!item) return { skip: true, reason: `Item ${itemId} not found` };

  updateStrategicCampaignItem(itemId, { generation_status: 'processing' });

  try {
    console.log(`[Strategic Scheduler] Generating creative script, storyboard & captions (Call 1) for item #${itemId}...`);
    await executeCall1CreativeEngine(itemId);

    updateStrategicCampaignItem(itemId, { 
      generation_status: 'completed',
      workflow_status: 'ready_for_review'
    });
    return { success: true, item_id: itemId, message: 'Strategic Generator completed' };
  } catch (err) {
    console.error(`[Strategic Scheduler] Error in processStrategicGenerator for item #${itemId}:`, err.message);
    updateStrategicCampaignItem(itemId, { generation_status: 'failed', error_message: err.message });
    throw err;
  }
}

/**
 * Dedicated Start Frame T2I Generator
 * ONLY renders T2I Start Frame images for existing scenes to Webhook G-Labs.
 * DOES NOT call Gemini AI. DOES NOT modify storyboard naskah / text.
 */
export async function processStrategicStartFrames(itemId) {
  const { getDb, updateStrategicCampaignItem } = await import('./db.js');
  const db = getDb();
  const item = db.prepare('SELECT * FROM strategic_campaign_items WHERE id = ?').get(itemId);
  if (!item) return { skip: true, reason: `Item ${itemId} not found` };

  const scenes = db.prepare('SELECT * FROM strategic_campaign_scenes WHERE campaign_item_id = ? ORDER BY scene_number ASC').all(itemId);
  if (scenes.length === 0) {
    return { skip: true, reason: `No scenes found for item #${itemId}` };
  }

  const campaign = db.prepare('SELECT * FROM strategic_campaigns WHERE id = ?').get(item.campaign_id);
  const brandProfile = campaign ? db.prepare('SELECT * FROM brand_profiles WHERE LOWER(brand_name) = LOWER(?)').get(campaign.account_name || '') : null;
  ...
  // Product reference image Base64 for bridging clips
  let productBase64 = null;
  if (workflow.product_ref_image_path) {
    try {
      const productAbsPath = path.join(process.cwd(), 'public', workflow.product_ref_image_path.replace(/^\//, ''));
      if (fs.existsSync(productAbsPath)) {
        const buf = fs.readFileSync(productAbsPath);
        const ext = path.extname(productAbsPath).toLowerCase();
  ...
  console.log(`[Strategic Start Frame] 🖼️ Merender gambar Start Frame (T2I) untuk Item #${itemId}... (Naskah & Storyboard TIDAK DIUBAH)`);

  for (const sc of scenes) {
    if (sc.start_frame_path && sc.start_frame_path.trim() !== '') {
      continue; // Already has start frame
    }

    if (!sc.t2i_prompt) continue;

    const isBridge = workflow.is_bridging_active && (sc.scene_number >= bridgeAt && sc.scene_number <= bridgeEnd);
    const refImages = (isBridge && productBase64) ? [productBase64] : undefined;

    console.log(`[Strategic Start Frame] Sending T2I request to G-Labs for Scene #${sc.scene_number}...`);
    
    try {
      const t2iRes = await generateImage({
        prompt: sc.t2i_prompt,
        model: 'nano_banana_pro',
        aspect_ratio: '9:16',
      ...
      if (t2iRes?.task_id) {
        for (let poll = 0; poll < 40; poll++) {
          ...
          if (statusRes?.status === 'completed') {
            ...
            if (imgFile) {
              const downloadUrl = getFileUrl(imgFile, t2iRes.task_id);
              const imgBuf = await fetch(downloadUrl).then(r => r.arrayBuffer());
              const filename = `sc_start_frame_${itemId}_scene_${sc.scene_number}.png`;
              const absPath = path.join(startFrameDir, filename);
              fs.writeFileSync(absPath, Buffer.from(imgBuf));
              const relPath = `/uploads/start_frames/${filename}`;

              db.prepare('UPDATE strategic_campaign_scenes SET start_frame_path = ? WHERE id = ?').run(relPath, sc.id);
              console.log(`[Strategic Start Frame] Saved Start Frame for Scene #${sc.scene_number} -> ${relPath}`);
            }
            break;
          }
        }
      ...
      console.warn(`[Strategic Start Frame] T2I failed for Scene #${sc.scene_number}:`, t2iErr.message);
    }
  }
  ...
  return { success: true, item_id: itemId, message: 'Strategic Start Frame T2I completed' };
}

export async function processStrategicTts(payload, job) {
  const { getDb, updateStrategicCampaignItem } = await import('./db.js');
  const db = getDb();
  const itemId = payload?.item_id;
  ...
  const workflow = JSON.parse(campaign.workflow_config_json || '{}');
  if (workflow.enable_tts === 0) {
    updateStrategicCampaignItem(itemId, { tts_status: 'skipped' });
    return { processed: true, item_id: itemId, message: 'Skipped TTS stage' };
  }

  updateStrategicCampaignItem(itemId, { tts_status: 'processing' });

  try {
    const scenes = db.prepare('SELECT * FROM strategic_campaign_scenes WHERE campaign_item_id = ? ORDER BY scene_number ASC').all(itemId);
    if (scenes.length === 0) {
      updateStrategicCampaignItem(itemId, { tts_status: 'completed' });
      return { processed: true, item_id: itemId, message: 'No scenes found for TTS' };
    }

    const { generateMinimaxVO } = await import('./minimax-tts.js');
    const { callGeminiTtsApi } = await import('./gemini.js');
    const path = await import('path');
    const fs = await import('fs');
    ...
    const provider = workflow.voice_provider || 'minimax';
    const persona = workflow.voice_persona || 'Indonesian_casual_reporter_vv2';
    const speed = workflow.voice_speed !== undefined ? Number(workflow.voice_speed) : 1.0;

    for (const scene of scenes) {
      if (!scene.voice_over || scene.voice_over.trim() === '') continue;

      const ext = provider === 'minimax' ? 'mp3' : 'wav';
      const outputFilename = `tts_strat_${itemId}_scene_${scene.scene_number}.${ext}`;
      const localAbsPath = path.join(tempDir, outputFilename);
      const localRelPath = `/temp/${outputFilename}`;

      if (provider === 'minimax') {
        await generateMinimaxVO(scene.voice_over, persona, localAbsPath, { speed });
      } else {
        await callGeminiTtsApi(scene.voice_over, persona, localAbsPath);
      }

      db.prepare('UPDATE strategic_campaign_scenes SET voice_over_audio_path = ? WHERE id = ?').run(localRelPath, scene.id);
    }

    updateStrategicCampaignItem(itemId, { tts_status: 'completed' });
    return { processed: true, item_id: itemId, message: 'Strategic TTS completed successfully' };
  } catch (err) {
    console.error(`[Strategic TTS Error] item #${itemId}:`, err.message);
    updateStrategicCampaignItem(itemId, { tts_status: 'failed', error_message: err.message });
    throw err;
  }
}

export async function processStrategicGlabs(payload, job) {
  const { getDb, updateStrategicCampaignItem, getSetting } = await import('./db.js');
  const { generateVideo, getTaskStatus, getFileUrl } = await import('./webhook-client.js');
  const db = getDb();
  const itemId = payload?.item_id;
  ...
  if (item.workflow_status === 'ready_for_review') {
    return { skip: true, reason: `Item #${itemId} is in ready_for_review state. Waiting for manual approval before sending G-Labs prompts.` };
  }
  ...
  const workflow = JSON.parse(campaign.workflow_config_json || '{}');
  if (workflow.enable_glabs === 0) {
    updateStrategicCampaignItem(itemId, { visual_status: 'skipped' });
    return { processed: true, item_id: itemId, message: 'Skipped G-Labs visual stage' };
  }

  const scenes = db.prepare('SELECT * FROM strategic_campaign_scenes WHERE campaign_item_id = ? ORDER BY scene_number ASC').all(itemId);
  if (scenes.length === 0) {
    updateStrategicCampaignItem(itemId, { visual_status: 'completed' });
    return { processed: true, item_id: itemId, message: 'No scenes to process visuals' };
  }
  ...
  // PHASE A: Submission if visual_status !== 'processing'
  if (item.visual_status !== 'processing') {
    let taskIds = [];
    try {
      taskIds = JSON.parse(item.visual_tasks_json || '[]');
    } catch (_) {}

    if (taskIds.length === 0 || taskIds.length !== scenes.length) {
      taskIds = Array(scenes.length).fill(null);
    }

    for (let i = 0; i < scenes.length; i++) {
      const sc = scenes[i];

      // Check if scene already has a valid local clip file downloaded
      if (sc.local_clip_path) {
        const absClip = path.join(process.cwd(), 'public', sc.local_clip_path);
        if (fs.existsSync(absClip)) {
          if (!taskIds[i]) taskIds[i] = sc.task_id || `completed_${sc.id}`;
          continue; // Preserve completed scene!
        }
      }

      // If scene already has an active task_id from database, keep it
      if (sc.task_id && !taskIds[i]) {
        taskIds[i] = sc.task_id;
        continue;
      }

      if (taskIds[i]) continue;

      const clipNum = sc.scene_number || (i + 1);

      const startFrameBase64 = fileToBase64(sc.start_frame_path);
      if (!startFrameBase64) {
        throw new Error(`Gambar Start Frame belum tersedia untuk Adegan #${sc.scene_number}. Silakan regenerasi Start Frame terlebih dahulu.`);
      }

      const { sanitizeI2vPrompt } = await import('./prompts.js');
      const rawI2vPrompt = sc.i2v_prompt || sc.visual_action || sc.t2i_prompt;
      const sanitizedPrompt = sanitizeI2vPrompt(rawI2vPrompt);

      console.log(`[Strategic G-Labs] Submitting I2V visual generation task for Scene #${sc.scene_number} (Clip ${clipNum})...`);

      const res = await generateVideo({
        prompt: sanitizedPrompt,
        model: workflow.video_model || 'veo_31_lite',
        aspect_ratio: workflow.aspect_ratio || '9:16',
        mode: 'image_to_video',
        reference_images: [startFrameBase64],
        webhookOverride: brandProfile
      });
      ...
      if (res && res.task_id) {
        taskIds[i] = res.task_id;
        db.prepare('UPDATE strategic_campaign_scenes SET task_id = ? WHERE id = ?').run(res.task_id, sc.id);
      } else {
        throw new Error(`G-Labs did not return a task_id for Scene #${sc.scene_number}`);
      }
    }
    ...
    return { processed: true, item_id: itemId, message: 'Submitted G-Labs visual tasks' };
  }
  ...
  for (let i = 0; i < taskIds.length; i++) {
    if (localPaths[i]) {
      const localFile = path.join(process.cwd(), 'public', localPaths[i]);
    ...
    try {
      ...
      if (taskStatus === 'completed') {
        ...
        if (videoFile) {
          const downloadUrl = getFileUrl(videoFile, taskId);
          const localRelPath = `/temp/strat_clip_${itemId}_scene_${i + 1}.mp4`;
          const localAbsPath = path.join(tempDir, `strat_clip_${itemId}_scene_${i + 1}.mp4`);

          const res = await fetch(downloadUrl);
          if (res.ok) {
            const buf = Buffer.from(await res.arrayBuffer());
            fs.writeFileSync(localAbsPath, buf);
            localPaths[i] = localRelPath;
            db.prepare('UPDATE strategic_campaign_scenes SET local_clip_path = ? WHERE id = ?').run(localRelPath, scenes[i].id);
          } else {
            allCompleted = false;
          }
        } else {
        ...
        anyFailed = true;
        errorDetail = `Task ${taskId} failed on G-Labs for scene #${i + 1}`;
      } else {
        allCompleted = false;
      }
    } catch (pollErr) {
      allCompleted = false;
      console.error(`[Strategic G-Labs Polling Error] task ${taskId}:`, pollErr.message);
    }
  }

  updateStrategicCampaignItem(itemId, { visual_clip_paths: JSON.stringify(localPaths) });

  if (anyFailed) {
    updateStrategicCampaignItem(itemId, { visual_status: 'failed', error_message: errorDetail });
    throw new Error(`Strategic visual generation failed: ${errorDetail}`);
  }

  if (allCompleted && localPaths.every(Boolean)) {
    updateStrategicCampaignItem(itemId, { visual_status: 'completed' });
    return { processed: true, item_id: itemId, message: 'Strategic G-Labs visual stage completed' };
  }

  return { processed: true, item_id: itemId, message: 'Strategic G-Labs polling in progress' };
}



export async function processStrategicFfmpeg(payload, job) {
  const { getDb, updateStrategicCampaignItem, getSetting } = await import('./db.js');
  const { processVideoMuxing, resolveLocalPath } = await import('./video-studio-processor.js');
  ...
  const workflow = JSON.parse(campaign.workflow_config_json || '{}');
  if (workflow.enable_ffmpeg === 0) {
    updateStrategicCampaignItem(itemId, { ffmpeg_status: 'skipped' });
    return { processed: true, item_id: itemId, message: 'Skipped FFmpeg stage' };
  }

  updateStrategicCampaignItem(itemId, { ffmpeg_status: 'processing' });
  console.log(`[Strategic FFmpeg] Starting FFmpeg processing for Item #${itemId}...`);

  try {
    let visualPaths = [];
    try {
      visualPaths = JSON.parse(item.visual_clip_paths || '[]');
    } catch (_) {}

    const scenes = db.prepare('SELECT * FROM strategic_campaign_scenes WHERE campaign_item_id = ? ORDER BY scene_number ASC').all(itemId);
    if (visualPaths.length === 0 && scenes.length === 0) {
      throw new Error("No visual video clips or scenes found for FFmpeg muxing.");
    }

    const pairCount = Math.max(visualPaths.length, scenes.length);
    const tempDir = path.join(process.cwd(), 'public', 'temp');
    if (!fs.existsSync(tempDir)) fs.mkdirSync(tempDir, { recursive: true });

    const outputDir = path.join(process.cwd(), 'public', 'uploads', 'strategic-campaigns', `cmp_${campaign.id}_${itemId}`);
    fs.mkdirSync(outputDir, { recursive: true });
    const outputAbsPath = path.join(outputDir, 'final_video.mp4');
    const outputRelPath = `/uploads/strategic-campaigns/cmp_${campaign.id}_${itemId}/final_video.mp4`;

    const syncOpt = workflow.ffmpeg_sync_option || 'smart_sync';
    const scale = workflow.ffmpeg_video_scale !== undefined ? Number(workflow.ffmpeg_video_scale) : 1.0;
    const sfxVol = workflow.ffmpeg_sfx_volume !== undefined ? Number(workflow.ffmpeg_sfx_volume) : 0.0;
    const bgmVol = workflow.ffmpeg_bgm_volume !== undefined ? Number(workflow.ffmpeg_bgm_volume) : 0.0;
    const bridgeAtClip = workflow.bridge_at_clip !== undefined ? Number(workflow.bridge_at_clip) : 2;

    const tempClipPaths = [];

    // 1. Process muxing for each segment individually (Segmental Muxing & Smart Sync per Scene)
    for (let i = 0; i < pairCount; i++) {
      const vPath = visualPaths[i] || '';
      const sc = scenes[i] || {};
      const aPath = sc.voice_over_audio_path || '';

      const tempClipPath = path.join(tempDir, `temp_sc_muxed_${itemId}_clip_${i + 1}.mp4`);
      console.log(`[Strategic FFmpeg] Segment ${i + 1}/${pairCount}: Muxing video ${vPath || '(none)'} with TTS audio ${aPath || '(none)'}...`);

      let resolvedSyncOption = syncOpt;
      if (syncOpt === 'smart_sync' && vPath && aPath) {
        try {
          ...
          console.log(`[Strategic FFmpeg] Segment ${i + 1}/${pairCount}: Smart Sync resolved option to '${resolvedSyncOption}'`);
        } catch (_) {}
      }

      await processVideoMuxing({
        videoPath: JSON.stringify([vPath]),
        audioPath: aPath,
    ...
    // 2. Concatenate all segment video files together using fluent-ffmpeg
    console.log(`[Strategic FFmpeg] Concatenating ${tempClipPaths.length} segments into final output: ${outputAbsPath}...`);
    ...
    await new Promise((resolve, reject) => {
      ...
      const hasBgm = workflow.bgm_path && fs.existsSync(resolveLocalPath(workflow.bgm_path));
      if (hasBgm) {
        concatCommand.input(resolveLocalPath(workflow.bgm_path));
      }
      ...
      concatCommand
        ...
        .outputOptions(['-map [v_joined]', `-map [${finalAudioLabel}]`])
        .videoCodec('libx264')
        .audioCodec('aac')
        .outputOptions([
          '-preset slow',
        ...
        .on('start', (cmdline) => console.log(`[Strategic FFmpeg Concat] Running ffmpeg: ${cmdline}`))
        .on('end', () => {
          console.log(`[Strategic FFmpeg Concat] Final video muxing finished: ${outputAbsPath}`);
          resolve();
        })
        .on('error', (err) => {
          console.error(`[Strategic FFmpeg Concat Error]:`, err.message);
          reject(err);
        })
        .save(outputAbsPath);
    });
    ...
    updateStrategicCampaignItem(itemId, {
      ffmpeg_status: 'completed',
      ffmpeg_output_path: outputRelPath,
      upload_status: 'uploading'
    });

    // 3. CLOUD UPLOAD & ASSET VAULT SYNC (Final Video, Video Clips, Start Frames, Voiceovers)
    console.log(`[Strategic FFmpeg] Uploading video and complete asset vault to Cloud Storage...`);
    let driveUrl = '';
    const batchId = getScBatchId(campaign, item, db);
    ...
    try {
      ...
      if (storageProvider === 'nextcloud') {
        const { uploadBufferToNextcloud, getOrCreatePublicShareLink } = await import('./nextcloud-helper.js');
        const parentFolderName = getCampaignParentFolderName(campaign, 'SC');
        const targetFolder = campaign.nextcloud_parent_folder || workflow.nextcloud_parent_folder || getSetting('nextcloud_target_folder') || 'MAKNA_Strategic_Campaigns';
        const nextcloudBaseFolder = `/${targetFolder.replace(/^\/+/, '')}/${parentFolderName}/${batchId}`.replace(/\/+/g, '/');
        ...
        // Upload naskah.md untuk Strategic Campaign
        try {
          const publishingPkg = item.publishing_package_json ? JSON.parse(item.publishing_package_json) : null;
          const markdownContent = buildStrategicMarkdownContent(campaign, item, scenes, publishingPkg);
          await uploadBufferToNextcloud(Buffer.from(markdownContent, 'utf-8'), `${nextcloudBaseFolder}/${batchId}_naskah.md`);
          await uploadBufferToNextcloud(Buffer.from(markdownContent, 'utf-8'), `${nextcloudBaseFolder}/naskah.md`);
        } catch (mdErr) {
          console.error('[Strategic FFmpeg nextcloud] Failed to upload naskah.md:', mdErr.message);
        }

        // Upload individual video clips
        for (let i = 0; i < visualPaths.length; i++) {
          const clipRelativePath = visualPaths[i];
          const clipPath = path.join(process.cwd(), 'public', clipRelativePath);
          if (fs.existsSync(clipPath)) {
            const clipBuffer = fs.readFileSync(clipPath);
            await uploadBufferToNextcloud(clipBuffer, `${nextcloudBaseFolder}/${batchId}_video_clip_${i + 1}.mp4`);
          }
        }

        // Upload start frame PNG images
        for (let i = 0; i < scenes.length; i++) {
          const sc = scenes[i];
          if (sc.start_frame_path) {
            const sfPath = path.join(process.cwd(), 'public', sc.start_frame_path);
            if (fs.existsSync(sfPath)) {
              const sfBuffer = fs.readFileSync(sfPath);
              await uploadBufferToNextcloud(sfBuffer, `${nextcloudBaseFolder}/${batchId}_start_frame_${i + 1}.png`);
            }
          }
        }

        // Upload voiceover MP3 audio files
        for (let i = 0; i < scenes.length; i++) {
          const sc = scenes[i];
          if (sc.voice_over_audio_path) {
            const voPath = path.join(process.cwd(), 'public', sc.voice_over_audio_path);
            if (fs.existsSync(voPath)) {
              const voBuffer = fs.readFileSync(voPath);
              await uploadBufferToNextcloud(voBuffer, `${nextcloudBaseFolder}/${batchId}_voiceover_${i + 1}.mp3`);
            }
          }
        }
      } else {
        const { uploadVideoToFolder, uploadLocalFileToFolder, getOrCreateCampaignFolder, getOrCreateFolderInFolder } = await import('./drive-uploader.js');
        ...
        const finalVideoFileNameCloud = `${batchId}_video_final.mp4`;
        await uploadVideoToFolder(outputAbsPath, finalVideoFileNameCloud, targetUploadFolderId);
        driveUrl = `https://drive.google.com/drive/folders/${targetUploadFolderId}`;
        ...
        for (let i = 0; i < visualPaths.length; i++) {
          ...
          const clipPath = path.join(process.cwd(), 'public', clipRelativePath);
          if (fs.existsSync(clipPath)) {
            await uploadLocalFileToFolder(clipPath, `${batchId}_video_clip_${i + 1}.mp4`, targetUploadFolderId, 'video/mp4');
          }
        }
        ...
        for (let i = 0; i < scenes.length; i++) {
          ...
          if (sc.start_frame_path) {
            const sfPath = path.join(process.cwd(), 'public', sc.start_frame_path);
            if (fs.existsSync(sfPath)) {
              await uploadLocalFileToFolder(sfPath, `${batchId}_start_frame_${i + 1}.png`, targetUploadFolderId, 'image/png');
            }
          }
        }
        ...
        for (let i = 0; i < scenes.length; i++) {
          ...
          if (sc.voice_over_audio_path) {
            const voPath = path.join(process.cwd(), 'public', sc.voice_over_audio_path);
            if (fs.existsSync(voPath)) {
              await uploadLocalFileToFolder(voPath, `${batchId}_voiceover_${i + 1}.mp3`, targetUploadFolderId, 'audio/mpeg');
            }
          }
        }
      }
    } catch (uploadErr) {
      console.error(`[Strategic Cloud Upload Error]:`, uploadErr.message);
    }

    // 4. GOOGLE SHEETS SYNC ENGINE (TAB: Account Name, Row Match by video_id)
    const targetSpreadsheetId = campaign.target_spreadsheet_id || workflow.target_spreadsheet_id;
    if (targetSpreadsheetId && driveUrl) {
      try {
        const { getAuthorizedClient } = await import('./google-auth.js');
        const { google } = await import('googleapis');
        const { updateCell } = await import('./sheets-autopilot-worker.js');
        
        const auth = getAuthorizedClient();
        const sheets = google.sheets({ version: 'v4', auth });
        const sheetName = campaign.account_name || workflow.account_name || 'CAMPAIGN_SC';

        try {
          const getHeaders = await sheets.spreadsheets.values.get({ spreadsheetId: targetSpreadsheetId, range: `'${sheetName}'!A1:Z1` });
          const scHeaders = (getHeaders.data.values || [[]])[0].map(h => h.trim().toLowerCase());
          
          // Row matching by video_id first, fallback to hook
          let resolvedRowNum = -1;
          const videoIdColIdx = scHeaders.findIndex(h => h === 'video_id' || h === 'video id' || h === 'id_video' || h === 'id video' || h === 'id_video');
          
          if (videoIdColIdx !== -1 && item.video_id) {
            try {
              const fullData = await sheets.spreadsheets.values.get({ spreadsheetId: targetSpreadsheetId, range: `'${sheetName}'!A2:Z500` });
              const rows = fullData.data.values || [];
              for (let i = 0; i < rows.length; i++) {
                const val = (rows[i][videoIdColIdx] || '').trim();
                if (val && val === item.video_id.trim()) {
                  resolvedRowNum = i + 2; // 1-indexed header is row 1
                  break;
                }
              }
            } catch (_) {}
          ...
          if (resolvedRowNum === -1) {
            resolvedRowNum = await findOpcRowByHook(sheets, targetSpreadsheetId, sheetName, scHeaders, item.hook, -1);
          }

          if (resolvedRowNum !== -1) {
            console.log(`[Strategic Sheets Sync] Updating row ${resolvedRowNum} (video_id: ${item.video_id || '-'}) in tab "${sheetName}"...`);
            
            // Extract Single Line Caption
            let captionText = '';
            try {
              if (item.publishing_package_json) {
                const pubPkg = JSON.parse(item.publishing_package_json);
                captionText = pubPkg.publishing_assets?.tiktok?.caption || pubPkg.caption || '';
              }
              if (!captionText && item.creative_package_json) {
                const crPkg = JSON.parse(item.creative_package_json);
                captionText = crPkg.social_media_package?.caption || '';
              }
            } catch (_) {}
            const singleLineCaption = (captionText || '').replace(/[\r\n]+/g, ' ').replace(/\s+/g, ' ').trim();

            // Production Date YYYYMMDD
            const now = new Date();
            const prodDateStr = `${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}`;

            // Helper cell updater
            const updateColIfFound = async (possibleHeaders, value) => {
              for (const headerName of possibleHeaders) {
                const idx = scHeaders.indexOf(headerName);
                if (idx !== -1) {
                  await updateCell(sheets, targetSpreadsheetId, sheetName, idx, resolvedRowNum, value);
                  break;
                }
              }
            };
            ...
            // 1. URL Asset
            await updateColIfFound(['url asset', 'asset_url', 'url_asset', 'link asset', 'asset link'], driveUrl);

            // 2. Caption (Single Line)
            await updateColIfFound(['caption', 'naskah caption', 'post caption'], singleLineCaption);

            // 3. Pipeline Status (Completed)
            await updateColIfFound(['pipeline status', 'pipeline_status'], 'Completed');

            // 4. Status (Produced)
            await updateColIfFound(['status'], 'Produced');

            // 5. Production Date (YYYYMMDD)
            await updateColIfFound(['production date', 'production_date', 'tanggal produksi', 'prod_date'], prodDateStr);

            // Processed At timestamp
            await updateColIfFound(['processed_at', 'processed at'], new Date().toISOString());

          } else {
            console.warn(`[Strategic Sheets Sync] No matching row found in tab "${sheetName}" for video_id: "${item.video_id}" or hook: "${item.hook.slice(0, 30)}..."`);
          }
        } catch (tabErr) {
          console.warn(`[Strategic Sheets Sync] Error accessing tab "${sheetName}":`, tabErr.message);
        }
      } catch (sheetsErr) {
        console.warn(`[Strategic Sheets Sync] Error:`, sheetsErr.message);
      }
    }

    // 5. AUTO INGEST TO CONTENT FLOW
    try {
      const { syncCampaignItemToContentFlow } = await import('./strategic-campaign-engine.js');
      await syncCampaignItemToContentFlow(campaign, item, driveUrl);
    } catch (cfErr) {
      console.warn('[Strategic FFmpeg Content Flow Sync] Warning:', cfErr.message);
    }
    ...
    console.log(`[Strategic FFmpeg] Strategic FFmpeg stage completed successfully for Item #${itemId}`);
    return { processed: true, item_id: itemId, message: 'Strategic FFmpeg stage completed with Cloud upload & Sheets sync' };
  } catch (err) {
    console.error(`[Strategic FFmpeg Error] item #${itemId}:`, err.message);
    updateStrategicCampaignItem(itemId, { ffmpeg_status: 'failed', error_message: err.message });
    throw err;
  }
}

export async function processStrategicSocialPoster(payload, job) {
  const { getDb, updateStrategicCampaignItem } = await import('./db.js');
  const db = getDb();
  const itemId = payload?.item_id;
  ...
  if (!actualNeedsSocial) {
    updateStrategicCampaignItem(itemId, { social_post_status: 'skipped' });
    return { processed: true, item_id: itemId, message: 'Skipped Social Post stage' };
  }

  updateStrategicCampaignItem(itemId, { social_post_status: 'completed' });
  return { processed: true, item_id: itemId, message: 'Strategic Social Post stage completed' };
}

export async function syncStrategicCampaignItemAssets(itemId) {
  const { getDb, updateStrategicCampaignItem, getSetting } = await import('./db.js');
  const path = await import('path');
  const fs = await import('fs');
  const db = getDb();
  ...
  if (storageProvider === 'nextcloud') {
    ...
    // 1. Final video if available
    if (item.ffmpeg_output_path) {
      const outputAbsPath = path.join(process.cwd(), 'public', item.ffmpeg_output_path.replace(/^\//, ''));
      if (fs.existsSync(outputAbsPath)) {
        const fileBuffer = fs.readFileSync(outputAbsPath);
        await uploadBufferToNextcloud(fileBuffer, `${nextcloudBaseFolder}/${batchId}_video_final.mp4`);
        // Backup copy for backward compatibility
        try {
          await uploadBufferToNextcloud(fileBuffer, `${nextcloudBaseFolder}/video_final.mp4`);
        } catch (_) {}
    ...
    // Upload naskah.md untuk Nextcloud
    try {
      const publishingPkg = item.publishing_package_json ? JSON.parse(item.publishing_package_json) : null;
      const markdownContent = buildStrategicMarkdownContent(campaign, item, scenes, publishingPkg);
      await uploadBufferToNextcloud(Buffer.from(markdownContent, 'utf-8'), `${nextcloudBaseFolder}/${batchId}_naskah.md`);
      ...
      console.error('[Strategic Sync nextcloud] Failed to upload naskah.md:', mdErr.message);
    }

    // 2. Scene video clips, audio voiceovers, and start frames
    for (let i = 0; i < scenes.length; i++) {
      const sc = scenes[i];
      const sceneNum = sc.scene_number || (i + 1);

      if (sc.local_clip_path) {
        const clipPath = path.join(process.cwd(), 'public', sc.local_clip_path.replace(/^\//, ''));
        if (fs.existsSync(clipPath)) {
          const clipBuffer = fs.readFileSync(clipPath);
          await uploadBufferToNextcloud(clipBuffer, `${nextcloudBaseFolder}/${batchId}_video_clip_${sceneNum}.mp4`);
          totalFiles++;
        }
      }

      if (sc.voice_over_audio_path) {
        const audioPath = path.join(process.cwd(), 'public', sc.voice_over_audio_path.replace(/^\//, ''));
        if (fs.existsSync(audioPath)) {
          const audioBuffer = fs.readFileSync(audioPath);
          await uploadBufferToNextcloud(audioBuffer, `${nextcloudBaseFolder}/${batchId}_audio_scene_${sceneNum}.mp3`);
          totalFiles++;
        }
      }

      if (sc.start_frame_path) {
        const sfPath = path.join(process.cwd(), 'public', sc.start_frame_path.replace(/^\//, ''));
        if (fs.existsSync(sfPath)) {
          const sfBuffer = fs.readFileSync(sfPath);
          await uploadBufferToNextcloud(sfBuffer, `${nextcloudBaseFolder}/${batchId}_start_frame_${sceneNum}.png`);
          totalFiles++;
        }
      }
    }
    ...
    if (item.ffmpeg_output_path) {
      const outputAbsPath = path.join(process.cwd(), 'public', item.ffmpeg_output_path.replace(/^\//, ''));
      if (fs.existsSync(outputAbsPath)) {
        await uploadVideoToFolder(outputAbsPath, `${batchId}_video_final.mp4`, targetUploadFolderId);
        // Backup copy for backward compatibility
        try {
          await uploadVideoToFolder(outputAbsPath, 'video_final.mp4', targetUploadFolderId);
        } catch (_) {}
        totalFiles++;
      }
    }

    // Upload naskah.md untuk Google Drive
    try {
      const publishingPkg = item.publishing_package_json ? JSON.parse(item.publishing_package_json) : null;
      const markdownContent = buildStrategicMarkdownContent(campaign, item, scenes, publishingPkg);
      const { uploadMarkdownToCampaignFolder } = await import('./drive-uploader.js');
      await uploadMarkdownToCampaignFolder(markdownContent, `${batchId}_naskah.md`, targetUploadFolderId);
      await uploadMarkdownToCampaignFolder(markdownContent, 'naskah.md', targetUploadFolderId);
      totalFiles += 2;
    } catch (mdErr) {
      console.error('[Strategic Sync drive] Failed to upload naskah.md:', mdErr.message);
    }
    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      if (sc.local_clip_path) {
        const clipPath = path.join(process.cwd(), 'public', sc.local_clip_path.replace(/^\//, ''));
        if (fs.existsSync(clipPath)) {
          await uploadLocalFileToFolder(clipPath, `${batchId}_video_clip_${sceneNum}.mp4`, targetUploadFolderId, 'video/mp4');
          totalFiles++;
        }
      }
      ...
      if (sc.voice_over_audio_path) {
        const audioPath = path.join(process.cwd(), 'public', sc.voice_over_audio_path.replace(/^\//, ''));
        if (fs.existsSync(audioPath)) {
          await uploadLocalFileToFolder(audioPath, `${batchId}_audio_scene_${sceneNum}.mp3`, targetUploadFolderId, 'audio/mpeg');
          totalFiles++;
        }
      }
      ...
      if (sc.start_frame_path) {
        const sfPath = path.join(process.cwd(), 'public', sc.start_frame_path.replace(/^\//, ''));
        if (fs.existsSync(sfPath)) {
          await uploadLocalFileToFolder(sfPath, `${batchId}_start_frame_${sceneNum}.png`, targetUploadFolderId, 'image/png');
          totalFiles++;
        }
      }
    }
  ...
  return { success: true, totalFiles, driveUrl, storageProvider };
}
