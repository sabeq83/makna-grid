...
export async function processReGlabs(payload, job) {
  ...
  if (payload?.target_type === 'angle_variant') {
    ...
    try {
      ...
      const baseBatchId = getReBatchId(campaign, parentItem, db);
    ...
    try {
      ...
      if (storageProvider === 'nextcloud') {
        ...
        for (let i = 0; i < visualPaths.length; i++) {
          ...
          if (fs.existsSync(clipPath)) {
            const clipBuffer = fs.readFileSync(clipPath);
            await uploadBufferToNextcloud(clipBuffer, `${nextcloudBaseFolder}/${filePrefix}_video_clip_${i + 1}.mp4`);
          }
        }
        ...
        for (let i = 0; i < scenes.length; i++) {
          ...
          if (sc.start_frame_path) {
            ...
            if (fs.existsSync(sfPath)) {
              ...
              await uploadBufferToNextcloud(sfBuffer, `${nextcloudBaseFolder}/${filePrefix}_start_frame_${i + 1}.png`);
            }
          }
        }
        ...
        for (let i = 0; i < scenes.length; i++) {
          ...
          if (sc.voice_over_audio_path) {
            ...
            if (fs.existsSync(voPath)) {
              ...
              await uploadBufferToNextcloud(voBuffer, `${nextcloudBaseFolder}/${filePrefix}_voiceover_${i + 1}.mp3`);
            }
          }
        }
      } else {
        ...
        for (let i = 0; i < visualPaths.length; i++) {
          ...
          const clipPath = path.join(process.cwd(), 'public', clipRelativePath);
          if (fs.existsSync(clipPath)) {
            await uploadLocalFileToFolder(clipPath, `${filePrefix}_video_clip_${i + 1}.mp4`, targetUploadFolderId, 'video/mp4');
          }
        }
        ...
        for (let i = 0; i < scenes.length; i++) {
          ...
          if (sc.start_frame_path) {
            const sfPath = path.join(process.cwd(), 'public', sc.start_frame_path);
            if (fs.existsSync(sfPath)) {
              await uploadLocalFileToFolder(sfPath, `${filePrefix}_start_frame_${i + 1}.png`, targetUploadFolderId, 'image/png');
            }
          }
        }
        ...
        for (let i = 0; i < scenes.length; i++) {
          ...
          if (sc.voice_over_audio_path) {
            const voPath = path.join(process.cwd(), 'public', sc.voice_over_audio_path);
            if (fs.existsSync(voPath)) {
              await uploadLocalFileToFolder(voPath, `${filePrefix}_voiceover_${i + 1}.mp3`, targetUploadFolderId, 'audio/mpeg');
            }
          }
        }
      }
  ...
  if (storageProvider === 'nextcloud') {
    ...
    if (item.ffmpeg_output_path) {
      ...
      if (fs.existsSync(outputAbsPath)) {
        const fileBuffer = fs.readFileSync(outputAbsPath);
        await uploadBufferToNextcloud(fileBuffer, `${nextcloudBaseFolder}/${filePrefix}_video_final.mp4`);
        // Backup copy for backward compatibility
        try {
          await uploadBufferToNextcloud(fileBuffer, `${nextcloudBaseFolder}/video_final.mp4`);
        } catch (_) {}
    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      if (sc.local_clip_path) {
        ...
        if (fs.existsSync(clipPath)) {
          const clipBuffer = fs.readFileSync(clipPath);
          await uploadBufferToNextcloud(clipBuffer, `${nextcloudBaseFolder}/${filePrefix}_video_clip_${sceneNum}.mp4`);
          totalFiles++;
        }
      }
      ...
      if (sc.voice_over_audio_path) {
        ...
        if (fs.existsSync(audioPath)) {
          ...
          await uploadBufferToNextcloud(audioBuffer, `${nextcloudBaseFolder}/${filePrefix}_audio_scene_${sceneNum}.mp3`);
          totalFiles++;
        }
      }
      ...
      if (sc.start_frame_path) {
        ...
        if (fs.existsSync(sfPath)) {
          const sfBuffer = fs.readFileSync(sfPath);
          await uploadBufferToNextcloud(sfBuffer, `${nextcloudBaseFolder}/${filePrefix}_start_frame_${sceneNum}.png`);
          totalFiles++;
        }
      }
    }
    ...
    if (item.ffmpeg_output_path) {
      const outputAbsPath = path.join(process.cwd(), 'public', item.ffmpeg_output_path.replace(/^\//, ''));
      if (fs.existsSync(outputAbsPath)) {
        await uploadVideoToFolder(outputAbsPath, `${filePrefix}_video_final.mp4`, targetUploadFolderId);
        // Backup copy for backward compatibility
        try {
    ...
    try {
      ...
      await uploadMarkdownToCampaignFolder(markdownContent, `${filePrefix}_naskah.md`, targetUploadFolderId);
    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      if (sc.local_clip_path) {
        const clipPath = path.join(process.cwd(), 'public', sc.local_clip_path.replace(/^\//, ''));
        if (fs.existsSync(clipPath)) {
          await uploadLocalFileToFolder(clipPath, `${filePrefix}_video_clip_${sceneNum}.mp4`, targetUploadFolderId, 'video/mp4');
          totalFiles++;
        }
      }
      ...
      if (sc.voice_over_audio_path) {
        const audioPath = path.join(process.cwd(), 'public', sc.voice_over_audio_path.replace(/^\//, ''));
        if (fs.existsSync(audioPath)) {
          await uploadLocalFileToFolder(audioPath, `${filePrefix}_audio_scene_${sceneNum}.mp3`, targetUploadFolderId, 'audio/mpeg');
          totalFiles++;
        }
      }
      ...
      if (sc.start_frame_path) {
        const sfPath = path.join(process.cwd(), 'public', sc.start_frame_path.replace(/^\//, ''));
        if (fs.existsSync(sfPath)) {
          await uploadLocalFileToFolder(sfPath, `${filePrefix}_start_frame_${sceneNum}.png`, targetUploadFolderId, 'image/png');
          totalFiles++;
        }
      }
    }
