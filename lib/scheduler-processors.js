...
export function resolveProductBase64(campaign, productData = null, rowPayload = null) {
  try {
    ...
    // MANDATE: Prioritaskan Foto Studio Bersih Produk Asli (clean_photo_url / raw_photo_url)
    if (productData) {
      const studioPhoto = productData.clean_photo_url || productData.cleaned_photo_url || productData.raw_photo_url || productData.photo_url || productData.generated_photo_url;
    ...
    if (campaign?.target_product_id || campaign?.product_id) {
      try {
        ...
        if (p) {
          const studioPhoto = p.clean_photo_url || p.cleaned_photo_url || p.raw_photo_url || p.photo_url || p.generated_photo_url;
          const b64 = fileToBase64(studioPhoto);
          if (b64) return b64;
        }
      } catch (e) {}
