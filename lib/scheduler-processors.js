...
export function getCampaignNextcloudTargetFolder(campaign, fallbackS) {
  ...
  const baseFolder = campaign?.nextcloud_parent_folder || getS('nextcloud_target_folder') || '/MAKNA_Video_Generations';
              ...
              if (t2iResult?.task_id) {
                ...
                for (let attempt = 0; attempt < maxAttempts; attempt++) {
                  ...
                  if (taskStatus === 'completed') {
                    ...
                    if (imageFile) {
                      const completedUrl = getFileUrl(imageFile);
            ...
            for (const task of submittedTasks) {
              ...
              try {
                ...
                if (t2iStatus === 'completed') {
                  ...
                  if (imageFile) {
                    task.imageUrl = getFileUrl(imageFile);
                    task.completed = true;
        ...
        if (taskStatus === 'completed') {
          ...
          try {
            ...
            if (videoFile) {
              ...
              const fileUrl = getFileUrl(videoFile);
      ...
      for (let i = 0; i < glabsTaskIds.length; i++) {
        ...
        try {
          ...
          if (taskStatus === 'completed') {
            ...
            if (videoFile) {
              completedUrls[i] = getFileUrl(videoFile);
          ...
          const runT2I = async (useRefImage) => {
            ...
            for (let poll = 0; poll < maxPoll; poll++) {
              ...
              if (t2iStatus?.status?.toLowerCase() === 'completed') {
                ...
                if (imgFile) {
                  t2iImageUrl = getFileUrl(imgFile.split('/').pop());
                  t2iCompleted = true;
                  break;
                }
          ...
          if (glabsResult?.task_id) {
            ...
            for (let poll = 0; poll < 40; poll++) {
              ...
              if (statusRes?.status === 'completed') {
                ...
                if (videoOrImageFile) {
                  const downloadUrl = getGlabsFileUrl(videoOrImageFile);
