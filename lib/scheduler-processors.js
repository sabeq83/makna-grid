  ...
  if (item) {
    ...
    for (let i = 0; i < taskIds.length; i++) {
      ...
      try {
        ...
        // BUG FIX: Jika file video sudah expired di Worker (404), reset ke pending agar
        // scheduler dapat submit ulang task render baru secara otomatis
        const is404Expired = pollErr.message && (
          pollErr.message.includes('Failed to download visual clip from URL') ||
          pollErr.statusCode === 404
        );
        if (is404Expired) {
          console.warn(`[RE Glabs] Task ${taskId} video file expired on worker (404). Resetting item #${item.id} visual_status → pending for re-render.`);
          updateReCampaignItem(item.id, {
            visual_status: 'pending',
            visual_tasks_json: '[]',
            visual_clip_paths: null,
          });
          return { processed: true, message: `Item #${item.id} reset to pending: expired video file on G-Labs worker` };
        }
      }
    }
        ...
        try {
          ...
          if (dbImg?.clean_photo_url) {
            ...
            if (require('fs').existsSync(imgPath)) {
              ...
              db.prepare('UPDATE pillar_campaigns SET product_ref_image_path = ? WHERE id = ?').run(dbImg.clean_photo_url, campaign.id);
  ...
  if (item) {
    ...
    for (let i = 0; i < taskIds.length; i++) {
      ...
      try {
        ...
        if (is404Expired) {
          console.warn(`[Pillar Glabs] Task ${taskId} video file expired on worker (404). Resetting item #${item.id} visual_status → pending for re-render.`);
          updatePillarCampaignItem(item.id, {
            visual_status: 'pending',
            visual_tasks_json: '[]',
            visual_clip_paths: null,
...
async function downloadAndUploadPillarClip(item, campaign, clipIndex, url, localPaths, db) {
  ...
  const response = await fetch(url);
  if (!response.ok) {
    const err = new Error(`Failed to download visual clip from URL: ${url}`);
