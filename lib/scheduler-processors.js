...
export async function processReAnalyzer(payload, job) {
  ...
  updateReCampaignItem(item.id, { analyze_status: 'processing' });

  const auth = getAuthorizedClient();
  const sheets = google.sheets({ version: 'v4', auth });

  // Ensure campaign has a Google Sheet
  ...
  if (!spreadsheetId) {
    try {
      ...
      console.error('[RE Analyzer] Failed to create campaign spreadsheet:', err.message);
      throw err;
    }
  }
  ...
  try {
    ...
    try {
      ...
      console.error('[RE Analyzer] Sheet export failed:', sheetErr.message);
    }
...
export async function processReFfmpeg(payload, job) {
  ...
  try {
    ...
    const batchId = `RE-${campaignCode}-${dateStr}-${paddedIndex}`;
    ...
    if (hasNextcloud) {
      try {
        ...
        const finalVideoNextcloudPath = `${nextcloudBaseFolder}/video_final.mp4`;
        const fileBuffer = fs.readFileSync(finalVideoPath);
        ...
        const markdownNextcloudPath = `${nextcloudBaseFolder}/naskah.md`;
  ...
  try {
    ...
    if (campaign.enable_vo_audit === 1) {
      try {
        ...
        console.log(`[OPC Generator] Item #${item.id}: Compliance audit finished with verdict ${auditResult.verdict}`);
      } catch (err) {
    ...
    if (campaign.visual_mode === 'hybrid_lock' && campaign.enable_vo_audit !== 1) {
      ...
      try {
        ...
        if (tempCampaign.product_ref_image_path) {
          productBase64 = fileToBase64(tempCampaign.product_ref_image_path);
        }
...
export async function processPillarFfmpeg(payload, job) {
  ...
  try {
    ...
    // Auto-ingest completed video item to Content Flow API
    try {
      const { syncCampaignItemToContentFlow } = await import('./strategic-campaign-engine');
...

{
  ...
  "distribution_assets": {
    ...
    updateInstantCampaignItem(item.id, {
      generation_status: 'completed',
      result_json: JSON.stringify(adaptedJson)
    });
    ...
    try {
      ...
      if (storageProvider === 'nextcloud') {
        ...
        if (campaign.is_mass_production === 1) {
          ...
          const finalFileName = `IFC-${campaign.product_name.replace(/[^a-zA-Z0-9_]/g, '_')}-${paddedIndex}.mp4`;
          targetPath = `${targetFolder}/IFC_Campaign_${campaign.id}/${subfolderName}/${finalFileName}`.replace(/\/+/g, '/');
        } else {
          targetPath = `${targetFolder}/IFC_Campaign_${campaign.id}/${campaign.product_name}_${paddedIndex}.mp4`.replace(/\/+/g, '/');
        }
            ...
            for (const clip of ttsClips) {
              if (clip.audio_path) {
                ...
                if (fs.existsSync(audioLocalPath)) {
                  ...
                  if (campaign.is_mass_production === 1) {
                    const subfolderName = `IFC_${campaign.product_name.replace(/[^a-zA-Z0-9_]/g, '_')}_${paddedIndex}`;
                    audioTargetPath = `${targetFolder}/IFC_Campaign_${campaign.id}/${subfolderName}/${audioFileName}`.replace(/\/+/g, '/');
                  } else {
                    audioTargetPath = `${targetFolder}/IFC_Campaign_${campaign.id}/${audioFileName}`.replace(/\/+/g, '/');
                  }
                  const audioBuffer = fs.readFileSync(audioLocalPath);
                  await uploadBufferToNextcloud(audioBuffer, audioTargetPath);
                }
...
${promptsSchema}
    ...
    const rawResponse = await executeWithKeyPool(1, async (apiKey) => {
      ...
      try {
        ...
        console.warn(`[Recipe Gemini Fallback] gemini-3.5-flash failed (${err.message}). Swapping to gemini-2.5-flash...`);
        try {
          res = await tryGenerate('gemini-2.5-flash');
        } catch (err2) {
          console.warn(`[Recipe Gemini Fallback] gemini-2.5-flash failed (${err2.message}). Swapping to gemini-flash-latest...`);
...
export function getCampaignParentFolderName(campaign, type = 'OPC') {
  const paddedId = String(campaign.id).padStart(10, '0');
  const safeName = (campaign.campaign_name || '').replace(/[^a-zA-Z0-9_]/g, '_');
  return `${type}_${safeName}_${paddedId}`;
}
...
export async function processStrategicGenerator(payload, job) {
  ...
  try {
    ...
    // Automatically render T2I Start Frames for scenes
    await processStrategicStartFrames(itemId);
...
export function getScBatchId(campaign, item, db) {
  ...
  const cleanName = (campaign.campaign_name || '').replace(/[^a-zA-Z0-9]/g, '').slice(0, 4).toUpperCase();
  const campaignCode = cleanName || 'STRAT';
  const itemDate = new Date(item.created_at || Date.now());
  const dateStr = itemDate.toISOString().slice(0, 10).replace(/-/g, '');
  return `SC-${campaignCode}-${dateStr}-${paddedIndex}`;
}
