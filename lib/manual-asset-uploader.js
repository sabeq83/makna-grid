...
export async function syncItemAssetsToCloud(campaignType, itemId) {
  ...
  const item = db.prepare(`SELECT * FROM ${itemTable} WHERE id = ?`).get(itemId);
  if (!item) {
    throw new Error(`Item ID ${itemId} tidak ditemukan pada tabel ${itemTable}.`);
  }

  const campaign = db.prepare(`SELECT * FROM ${campaignTable} WHERE id = ?`).get(item.campaign_id);
  ...
  if (storageProvider === 'nextcloud') {
    ...
    db.prepare(`UPDATE ${itemTable} SET nextcloud_folder_url = ? WHERE id = ?`).run(folderShareUrl, itemId);
  } else {
    // Google Drive
    ...
    db.prepare(`UPDATE ${itemTable} SET drive_folder_id = ? WHERE id = ?`).run(itemFolder.folderId, itemId);
  }
 ...
 */
export function getLocalItemAssetsManifest(campaignType, itemId) {
  const db = getDb();
  const isRE = campaignType === 're';
  const itemTable = isRE ? 're_campaign_items' : 'pillar_campaign_items';
  const item = db.prepare(`SELECT * FROM ${itemTable} WHERE id = ?`).get(itemId);
