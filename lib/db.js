...
const DB_PATH = path.join(process.cwd(), 'data', 'makna_grid.db');
...
function initSchema() {
  ...
  db.exec(`
    ...
    try { db.prepare('ALTER TABLE content_flow_items ADD COLUMN catatan TEXT').run(); } catch (_) {}
  ...
  // Safe Migration for strategic_campaign_items & strategic_campaigns pipeline tracking
  try {
  ...
  const migrateBridgeInjectorBulk = () => {
    try {
      ...
      if (!cols.find(c => c.name === 'voice_provider')) {
        db.prepare("ALTER TABLE bridge_injector_campaigns ADD COLUMN voice_provider TEXT DEFAULT 'gemini'").run();
        console.log("[DB Migration] Added column voice_provider to bridge_injector_campaigns");
      }
      if (!cols.find(c => c.name === 'voice_persona')) {
        db.prepare("ALTER TABLE bridge_injector_campaigns ADD COLUMN voice_persona TEXT DEFAULT 'Kore'").run();
...
export function getReCampaign(id) {
  const db = getDb();
  return db.prepare('SELECT * FROM re_campaigns WHERE id = ?').get(id);
}

export function listReCampaigns() {
  const db = getDb();
  return db.prepare('SELECT * FROM re_campaigns ORDER BY created_at DESC').all();
}
...
export function createBrandProfile({ id, brand_name, tone_of_voice, visual_signature, color_palette, forbidden_elements, brand_slogan_or_cta, raw_guideline_text, guideline_filename }) {
  const db = getDb();
  return db.prepare(
    `INSERT INTO brand_profiles (id, brand_name, tone_of_voice, visual_signature, color_palette, forbidden_elements, brand_slogan_or_cta, raw_guideline_text, guideline_filename)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(id, brand_name, tone_of_voice || 'Kasual/Gaul', visual_signature, color_palette || '', forbidden_elements || '', brand_slogan_or_cta || '', raw_guideline_text || '', guideline_filename || '');
}

export function getAllBrandProfiles() {
  const db = getDb();
  return db.prepare('SELECT id, brand_name, tone_of_voice, visual_signature, color_palette, forbidden_elements, brand_slogan_or_cta, guideline_filename, created_at FROM brand_profiles ORDER BY created_at DESC').all();
}
...
export function updateBrandProfile(id, data) {
  const db = getDb();
  const allowedFields = ['brand_name', 'tone_of_voice', 'visual_signature', 'color_palette', 'forbidden_elements', 'brand_slogan_or_cta', 'raw_guideline_text', 'guideline_filename'];
  const fields = [];
  const values = [];
  for (const [key, val] of Object.entries(data)) {
...
export function getPillarCampaign(id) {
  const db = getDb();
  return db.prepare('SELECT * FROM pillar_campaigns WHERE id = ?').get(id);
}

export function listPillarCampaigns() {
  const db = getDb();
  return db.prepare('SELECT * FROM pillar_campaigns ORDER BY created_at DESC').all();
}
...
export function upsertContentFlowItem(item) {
  ...
  const stmt = db.prepare(`
    ...
    ON CONFLICT(id) DO UPDATE SET
      account_name = CASE WHEN EXCLUDED.account_name IS NOT NULL AND EXCLUDED.account_name != '' AND EXCLUDED.account_name != 'Umum' THEN EXCLUDED.account_name ELSE content_flow_items.account_name END,
      video_id = CASE WHEN EXCLUDED.video_id IS NOT NULL AND EXCLUDED.video_id != '' THEN EXCLUDED.video_id ELSE content_flow_items.video_id END,
      campaign_title = CASE WHEN EXCLUDED.campaign_title IS NOT NULL AND EXCLUDED.campaign_title != '' THEN EXCLUDED.campaign_title ELSE content_flow_items.campaign_title END,
      hook = CASE WHEN EXCLUDED.hook IS NOT NULL AND EXCLUDED.hook != '' THEN EXCLUDED.hook ELSE content_flow_items.hook END,
      nama_produk = CASE WHEN EXCLUDED.nama_produk IS NOT NULL AND EXCLUDED.nama_produk != '' AND EXCLUDED.nama_produk != 'Umum' THEN EXCLUDED.nama_produk ELSE content_flow_items.nama_produk END,
      link_affiliate = CASE WHEN EXCLUDED.link_affiliate IS NOT NULL AND EXCLUDED.link_affiliate != '' THEN EXCLUDED.link_affiliate ELSE content_flow_items.link_affiliate END,
      link_produk = CASE WHEN EXCLUDED.link_produk IS NOT NULL AND EXCLUDED.link_produk != '' THEN EXCLUDED.link_produk ELSE content_flow_items.link_produk END,
      caption = CASE WHEN EXCLUDED.caption IS NOT NULL AND EXCLUDED.caption != '' THEN EXCLUDED.caption ELSE content_flow_items.caption END,
      url_asset = CASE WHEN EXCLUDED.url_asset IS NOT NULL AND EXCLUDED.url_asset != '' THEN EXCLUDED.url_asset ELSE content_flow_items.url_asset END,
      drive_link = CASE WHEN EXCLUDED.drive_link IS NOT NULL AND EXCLUDED.drive_link != '' THEN EXCLUDED.drive_link ELSE content_flow_items.drive_link END,
      nextcloud_url = CASE WHEN EXCLUDED.nextcloud_url IS NOT NULL AND EXCLUDED.nextcloud_url != '' THEN EXCLUDED.nextcloud_url ELSE content_flow_items.nextcloud_url END,
      pipeline_status = CASE WHEN EXCLUDED.pipeline_status IS NOT NULL AND EXCLUDED.pipeline_status != '' THEN EXCLUDED.pipeline_status ELSE content_flow_items.pipeline_status END,
  ...
  // Auto-sync to PostgreSQL Storage Node 3 (non-blocking)
  try {
    const { pgQuery } = require('./db-pg.js');
    pgQuery(`
      INSERT INTO content_flow_items (
        id, source_type, source_campaign_id, source_item_id, account_name, video_id,
        campaign_title, hook, nama_produk, link_affiliate, link_produk, caption,
    ...
    ]).catch(err => console.warn('[DB PG Sync Warning]', err.message));
  } catch (_) {}
...
export function getContentFlowItems(filters = {}) {
  ...
  if (filters.allowedAccounts && Array.isArray(filters.allowedAccounts)) {
    if (filters.allowedAccounts.length > 0) {
      const placeholders = filters.allowedAccounts.map(() => '?').join(',');
