...
export function getStats() {
  ...
  const draftCount = db.prepare("SELECT COUNT(*) as count FROM ideas WHERE status = 'Draft'").get().count;
  const generatedCount = db.prepare("SELECT COUNT(*) as count FROM ideas WHERE status = 'Generated'").get().count;
  const reverseCount = db.prepare('SELECT COUNT(*) as count FROM reverse_results').get().count;
  const productCount = db.prepare('SELECT COUNT(*) as count FROM product_extractions').get().count;
  const pipelineCount = db.prepare('SELECT COUNT(*) as count FROM pipeline_assets').get().count;
  return { kbCount, ideaCount, assetCount, draftCount, generatedCount, reverseCount, productCount, pipelineCount };
}
...
export function upsertContentFlowItem(item) {
  ...
  const stmt = db.prepare(`
    ...
    ON CONFLICT(id) DO UPDATE SET
      account_name = COALESCE(EXCLUDED.account_name, content_flow_items.account_name),
      video_id = COALESCE(EXCLUDED.video_id, content_flow_items.video_id),
      campaign_title = COALESCE(EXCLUDED.campaign_title, content_flow_items.campaign_title),
      hook = COALESCE(EXCLUDED.hook, content_flow_items.hook),
      nama_produk = COALESCE(EXCLUDED.nama_produk, content_flow_items.nama_produk),
      link_affiliate = COALESCE(EXCLUDED.link_affiliate, content_flow_items.link_affiliate),
      link_produk = COALESCE(EXCLUDED.link_produk, content_flow_items.link_produk),
      caption = COALESCE(EXCLUDED.caption, content_flow_items.caption),
      url_asset = COALESCE(EXCLUDED.url_asset, content_flow_items.url_asset),
      drive_link = COALESCE(EXCLUDED.drive_link, content_flow_items.drive_link),
      nextcloud_url = COALESCE(EXCLUDED.nextcloud_url, content_flow_items.nextcloud_url),
      pipeline_status = COALESCE(EXCLUDED.pipeline_status, content_flow_items.pipeline_status),
