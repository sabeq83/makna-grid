 ...
 */
function makeModelResilient(model, apiKey, modelName = 'gemini-3.6-flash', isExplicitKey = false) {
  const originalGenerateContent = model.generateContent.bind(model);
  
  model.generateContent = async function(request) {
    if (isExplicitKey) {
      const primaryModel = modelName;
      const fallbackModel = primaryModel === 'gemini-3.6-flash' ? 'gemini-3.5-flash' : (primaryModel === 'gemini-3.5-flash' ? 'gemini-flash-latest' : null);
      const maxRetries = 4;
...
export async function getGeminiModel(apiKey = null) {
  ...
  return makeModelResilient(model, key, 'gemini-3.6-flash', apiKey !== null);
}
