...
import { getSetting, setSetting, getAvailableApiKey, incrementKeyUsage, markKeyExhausted } from './db.js';
 ...
 */
function makeModelResilient(model, apiKey, modelName = 'gemini-3.5-flash', isExplicitKey = false) {
  const originalGenerateContent = model.generateContent.bind(model);
  
  model.generateContent = async function(request) {
    if (isExplicitKey) {
      const primaryModel = modelName;
      const fallbackModel = primaryModel === 'gemini-3.5-flash' ? 'gemini-flash-latest' : (primaryModel === 'gemini-flash-latest' ? null : 'gemini-3.5-flash');
      const maxRetries = 4;
    ...
    return await executeWithKeyPool(1, async (rotatedKey) => {
      const primaryModel = modelName;
      const fallbackModel = primaryModel === 'gemini-2.5-flash' ? 'gemini-flash-latest' : (primaryModel === 'gemini-flash-latest' ? null : 'gemini-2.5-flash');
      const maxRetries = 4;
...
export async function getGeminiModel(apiKey = null) {
  ...
  return makeModelResilient(model, key, 'gemini-3.5-flash', apiKey !== null);
}
...
export async function generateWithVideo(fileUri, mimeType, prompt, apiKey = null) {
  ...
  const resilientModel = makeModelResilient(model, key, 'gemini-3.5-flash', apiKey !== null);
...
export async function testGeminiConnection(apiKey) {
  try {
    ...
    const resilientModel = makeModelResilient(model, apiKey, 'gemini-3.5-flash', true);
...
export async function executeWithKeyPool(cost, executorFn) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      // For any other error (auth, validation, etc.), throw immediately
      throw error;
    }
  }
 ...
 */
export async function refreshGeminiContextCache(apiKey = null, modelName = 'gemini-3.5-flash') {
  const key = resolveApiKey(apiKey);
...
async function executeContentGeneration(resolvedKey, params) {
  const { prompt, modelName = 'gemini-3.5-flash', videoPayload = null, useSearch = false, timeoutMs: customTimeoutMs = null } = params;
  ...
  while (true) {
    ...
    try {
      const cacheableModels = ['gemini-2.5-flash', 'gemini-3.5-flash'];
