import { getDb, listReCampaignItems, listPillarCampaignItems, listStrategicCampaignItems, getSetting, updateReCampaignItem, updatePillarCampaignItem, updateStrategicCampaignItem, updateInstantCampaignItem, listInstantCampaignItems } from './db.js';
import { 
  processReScraper, 
  processReAnalyzer, 
  processReTts, 
  ...
  processStrategicGenerator,
  processStrategicTts,
  processStrategicGlabs,
  processStrategicFfmpeg,
  processStrategicSocialPoster,
...
import { runSyncWorker } from './sheets-autopilot-worker';
...
export function startCampaignScheduler() {
  ...
  tickCampaignScheduler().catch(err => console.error('[Campaign Scheduler] Initial tick error:', err));
...
function isStrategicItemFinished(item, campaign) {
  const actualNeedsSocial = campaign.enable_social_post === 1 || campaign.post_facebook_draft === 1 || campaign.post_youtube_draft === 1 || campaign.post_tiktok_draft === 1;
...
export async function tickCampaignScheduler() {
  ...
  const bridgeSchedulerActive = getSetting('bridge_injector_scheduler_active') !== 'false';
  if (bridgeSchedulerActive) {
    try {
      const activeBridgeCampaigns = db.prepare("SELECT id FROM bridge_injector_campaigns WHERE status IN ('polling_t2i', 'generating_i2v')").all();
  ...
  // --- STRATEGIC CAMPAIGNS TICKING ENGINE ---
  await tickStrategicCampaigns(db);
  
  // Ambil semua kampanye running (RE Campaign) jika skeduler aktif
  const reSchedulerActive = getSetting('re_campaigns_scheduler_active') !== 'false';
  const campaigns = reSchedulerActive
    ? db.prepare("SELECT * FROM re_campaigns WHERE status = 'running'").all()
    : [];
  const reLogFile = path.join(process.cwd(), 'public', 're_campaign_logs.txt');
  logContextStorage.run(reLogFile, () => {
    for (const campaign of campaigns) {
      const items = listReCampaignItems(campaign.id);
      ...
      const { updateReCampaignItem } = require('./db');
  ...
  const opcSchedulerActive = getSetting('opc_campaigns_scheduler_active') !== 'false';
  const pillarCampaigns = opcSchedulerActive
    ? db.prepare("SELECT * FROM pillar_campaigns WHERE status = 'running'").all()
    : [];
  const opcLogFile = path.join(process.cwd(), 'public', 'opc_logs.txt');
  logContextStorage.run(opcLogFile, () => {
    for (const campaign of pillarCampaigns) {
      const items = listPillarCampaignItems(campaign.id);
      const pauseAt = campaign.scheduler_pause_at || null;
      let hasActiveItem = false;
      ...
      for (const currentItem of items) {
        if (currentItem.workflow_status === 'production_processing' && isPillarItemFinished(currentItem, campaign)) {
          updatePillarCampaignItem(currentItem.id, { workflow_status: 'completed' });
  ...
  const instantCampaigns = db.prepare("SELECT * FROM instant_campaigns WHERE status = 'running' AND local_scheduler = 1").all();
  const instantLogFile = path.join(process.cwd(), 'public', 'instant_factory_logs.txt');
  logContextStorage.run(instantLogFile, () => {
    for (const campaign of instantCampaigns) {
      const items = listInstantCampaignItems(campaign.id);
      const pauseAt = campaign.scheduler_pause_at || null;
  ...
  const isSchedulerActive = getSetting('sheets_autopilot_scheduler_active') === 'true';
  ...
  const isMultiplierActive = getSetting('multiplier_scheduler_active') === 'true';
  ...
  if (now - state.lastMultiplierSyncTime >= 10_000 && !state.isMultiplierSyncRunning && isMultiplierActive) {
    ...
    const { processNextMultiplierTask, logToMultiplier } = require('./re-multiplier-worker');
  ...
  const recipeSchedulerActive = getSetting('recipe_scheduler_active') === 'true';
  if (recipeSchedulerActive) {
    const recipeCampaigns = db.prepare("SELECT * FROM recipe_campaigns WHERE status = 'processing'").all();
    const recipeLogFile = path.join(process.cwd(), 'public', 'recipe_logs.txt');
    
    if (recipeCampaigns.length > 0) {
      logContextStorage.run(recipeLogFile, () => {
        for (const campaign of recipeCampaigns) {
          const items = db.prepare("SELECT * FROM recipe_items WHERE campaign_id = ?").all(campaign.id);
...
async function runStep(itemId, name, processor, taskKey = itemId) {
  ...
  if (name.startsWith('re_')) {
    logFile = path.join(process.cwd(), 'public', 're_campaign_logs.txt');
  } else if (name.startsWith('pillar_')) {
    logFile = path.join(process.cwd(), 'public', 'opc_logs.txt');
  } else if (name.startsWith('instant_')) {
    logFile = path.join(process.cwd(), 'public', 'instant_factory_logs.txt');
  } else if (name.startsWith('recipe_')) {
    logFile = path.join(process.cwd(), 'public', 'recipe_logs.txt');
  } else if (name.startsWith('bridge_')) {
    logFile = path.join(process.cwd(), 'public', 'bridge_injector_logs.txt');
  } else if (name.startsWith('strategic_')) {
    logFile = path.join(process.cwd(), 'public', 'strategic_campaign_logs.txt');
  }

  const execute = async () => {
    console.log(`🚀 [Campaign Scheduler] Starting step [${name}] for item #${itemId}...`);
    try {
      const payload = { item_id: itemId };
      const mockJob = { id: -1 };
      const result = await processor(payload, mockJob);
      console.log(`✅ [Campaign Scheduler] Finished step [${name}] for item #${itemId}:`, result);
    } catch (e) {
      console.error(`❌ [Campaign Scheduler] Error in step [${name}] for item #${itemId}:`, e.message);
    } finally {
...
async function wrapProcessor(itemId, db, processorFn) {
  try {
    ...
    db.prepare("UPDATE bridge_injector_items SET workflow_status = 'failed', error_message = ? WHERE id = ?").run(err.message, itemId);
  }
}

async function processBridgeBulkDownload(itemId, db) {
  const item = db.prepare("SELECT * FROM bridge_injector_items WHERE id = ?").get(itemId);
  if (!item) return;
  ...
  if (isNextcloudFolder) {
    ...
    if (matchedClip1File) {
      ...
      db.prepare("UPDATE bridge_injector_items SET original_clip1_filename = ? WHERE id = ?").run(originalBase, itemId);
    }
  ...
  const matchedProduct = db.prepare(`
...
async function processBridgeBulkGemini(itemId, db) {
  ...
  const campaign = db.prepare("SELECT * FROM bridge_injector_campaigns WHERE id = ?").get(item.campaign_id);
  if (!campaign) return;

  console.log(`[Bridge Bulk Scheduler] Running Gemini script injection for item #${itemId}...`);
  
  const productData = db.prepare("SELECT * FROM product_extractions WHERE id = ?").get(item.target_product_id);
  if (!productData) {
    throw new Error("Product data not found in DB even after resolution");
  }

  const { getGeminiModel } = await import('./gemini.js');
  const { buildProductBridgingInjectorPrompt } = await import('./prompts.js');
...
async function processBridgeBulkT2i(itemId, db) {
  ...
  if (res && res.task_id) {
    db.prepare("UPDATE bridge_injector_items SET clip2_t2i_task_id = ?, t2i_status = 'processing' WHERE id = ?").run(res.task_id, itemId);
...
async function processBridgeBulkI2vPoll(itemId, db) {
  ...
  if (status === 'completed') {
    ...
    if (videoFile) {
      ...
      if (item.enable_tts === 1) {
        nextStatus = 'rendering_tts';
      } else if (item.enable_ffmpeg === 1) {
...
async function processBridgeBulkTts(itemId, db) {
  ...
  if (item.enable_tts === 0) {
    db.prepare("UPDATE bridge_injector_items SET tts_status = 'skipped' WHERE id = ?").run(itemId);
    console.log(`[Bridge Bulk TTS] TTS is disabled for item ${item.id}. Skipping.`);
    return;
  }

  db.prepare("UPDATE bridge_injector_items SET tts_status = 'processing' WHERE id = ?").run(itemId);
  ...
  try {
    ...
    db.prepare("UPDATE bridge_injector_items SET tts_status = 'failed', error_message = ? WHERE id = ?").run(err.message, itemId);
    throw err;
  }
}
...
async function processBridgeBulkFfmpeg(itemId, db) {
  ...
  if (item.enable_ffmpeg === 0) {
    db.prepare("UPDATE bridge_injector_items SET ffmpeg_status = 'skipped' WHERE id = ?").run(itemId);
    console.log(`[Bridge Bulk FFmpeg] FFmpeg is disabled for item ${item.id}. Skipping.`);
    return;
  }

  db.prepare("UPDATE bridge_injector_items SET ffmpeg_status = 'processing' WHERE id = ?").run(itemId);
  ...
  try {
    ...
    const clip1Video = resolveLocal(item.local_clip1_path);
    const clip1Audio = resolveLocal(item.local_audio_clip1_path);
    const clip2Video = resolveLocal(item.clip2_video_path);
    const clip3Video = resolveLocal(item.local_clip2_path);
    const clip4Video = resolveLocal(item.local_clip3_path);
    ...
    if (clip2Video && fs.existsSync(clip2Video)) {
      ...
      if (item.enable_tts === 1 && fs.existsSync(clip2Audio)) {
    ...
    if (clip3Video && fs.existsSync(clip3Video)) {
      ...
      if (item.enable_tts === 1 && fs.existsSync(clip3Audio)) {
        let resolvedSync = item.ffmpeg_sync_option || 'smart_sync';
        if (resolvedSync === 'smart_sync') {
          const { determineSmartSyncOption } = await import('./smart-sync-engine.js');
          resolvedSync = await determineSmartSyncOption({
    ...
    if (clip4Video && fs.existsSync(clip4Video)) {
      ...
      if (item.enable_tts === 1 && fs.existsSync(clip4Audio)) {
        let resolvedSync = item.ffmpeg_sync_option || 'smart_sync';
        if (resolvedSync === 'smart_sync') {
          const { determineSmartSyncOption } = await import('./smart-sync-engine.js');
          resolvedSync = await determineSmartSyncOption({
    ...
    db.prepare("UPDATE bridge_injector_items SET ffmpeg_status = 'failed', error_message = ? WHERE id = ?").run(err.message, itemId);
    throw err;
  }
}
...
async function processBridgeBulkSync(itemId, db) {
  ...
  console.log(`[Bridge Bulk Scheduler] Syncing files to Nextcloud folder: ${item.nextcloud_folder}...`);
  const fs = await import('fs');
  const path = await import('path');
  ...
  // Deteksi Nextcloud folder share token asal
  const downloadUrl = item.original_script_url.trim();
  const isNextcloudFolder = (downloadUrl.includes('/s/') || downloadUrl.includes('/index.php/s/')) && !downloadUrl.endsWith('/download');

  let uploadClient = null;
  let uploadRoot = '';
  let usingShareToken = false;
  ...
  if (isNextcloudFolder) {
    try {
      ...
      if (tokenMatch) {
        ...
        const webdavUrl = `${baseUrl.replace(/\/+$/, '')}/public.php/webdav`;
        const { createClient } = await import('webdav');
        uploadClient = createClient(webdavUrl, {
          username: token,
          password: ''
        });
        uploadRoot = '/';
        usingShareToken = true;
        console.log(`[Bridge Bulk Scheduler] Initialized WebDAV upload client using public share token for item #${itemId}.`);
      }
    } catch (err) {
      console.warn(`[Bridge Bulk Scheduler] Gagal inisiasi upload client via share token: ${err.message}. Fallback ke client global.`);
    }
  }

  if (!uploadClient) {
    const { getNextcloudClient, checkAndCreateFolder } = await import('./nextcloud-helper.js');
    uploadClient = getNextcloudClient();
    uploadRoot = '/' + (item.nextcloud_folder || 'MAKNA_Bridge_Bulk').replace(/^\/+/, '').replace(/\/+$/, '') + '/';
    await checkAndCreateFolder(uploadRoot);
  }

  // Helper asinkron untuk upload file dengan fallback otomatis
  const uploadFile = async (localPath, remoteName) => {
    ...
    try {
      ...
      console.warn(`[Bridge Bulk Scheduler] Gagal mengunggah ${remoteName} ke path ${targetPath}: ${err.message}`);
      if (usingShareToken) {
        console.log(`[Bridge Bulk Scheduler] Mencoba fallback upload menggunakan client Nextcloud global...`);
        try {
          const { getNextcloudClient, checkAndCreateFolder } = await import('./nextcloud-helper.js');
          const globalClient = getNextcloudClient();
          const fallbackRoot = '/' + (item.nextcloud_folder || 'MAKNA_Bridge_Bulk').replace(/^\/+/, '').replace(/\/+$/, '') + '/';
          await checkAndCreateFolder(fallbackRoot);
          const fallbackPath = `${fallbackRoot}${remoteName}`.replace(/\/+/g, '/');
          const fallbackStream = fs.createReadStream(localPath);
          await globalClient.putFileContents(fallbackPath, fallbackStream, { overwrite: true });
          console.log(`[Bridge Bulk Scheduler] Fallback sukses mengunggah ${remoteName} ke Nextcloud global: ${fallbackPath}`);
        } catch (fallbackErr) {
          console.error(`[Bridge Bulk Scheduler] Fallback upload juga gagal untuk ${remoteName}:`, fallbackErr.message);
          throw fallbackErr;
        }
      } else {
        throw err;
  ...
  if (item.injected_script_md_path) {
    ...
    await uploadFile(absMdPath, 'naskah_baru.md');
  }
  ...
  if (item.clip2_t2i_image_path) {
    ...
    await uploadFile(absImagePath, `start_frame_clip2_baru.${imageExt}`);
  }

  if (item.clip2_video_path) {
    const absVideoPath = path.join(process.cwd(), 'public', item.clip2_video_path);
    await uploadFile(absVideoPath, 'video_clip_2_baru.mp4');
  }

  const absFinalVideoPath = path.join(itemDir, 'bridging_video_final.mp4');
  const originalName = item.original_clip1_filename || 'bridging_video';
  const targetVideoName = `${originalName}_bridging_inject_final.mp4`;
  await uploadFile(absFinalVideoPath, targetVideoName);
  ...
  if (item.enable_tts === 1) {
    ...
    for (const cNum of [2, 3, 4]) {
      ...
      await uploadFile(absAudioPath, `audio_clip_${cNum}_baru.${ext}`);
    }
  }

  db.prepare(`
    UPDATE bridge_injector_items 
    SET sync_status = 'completed', workflow_status = 'completed' 
    WHERE id = ?
  `).run(itemId);

  console.log(`[Bridge Bulk Scheduler] Nextcloud sync completed for item #${itemId}.`);
...
async function tickBridgeBulkCampaigns(db) {
  const activeBulkCampaigns = db.prepare("SELECT * FROM bridge_injector_campaigns WHERE campaign_type = 'bulk' AND status = 'running'").all();
  
  for (const campaign of activeBulkCampaigns) {
    try {
      const items = db.prepare("SELECT * FROM bridge_injector_items WHERE campaign_id = ? ORDER BY id ASC").all(campaign.id);
      ...
      if (allFinished) {
        ...
        db.prepare("UPDATE bridge_injector_campaigns SET status = ? WHERE id = ?").run(finalStatus, campaign.id);
      ...
      if (activePhase1Item) {
        ...
        if (!state.activeTasks.has(taskKey)) {
          if (item.workflow_status === 'pending') {
            db.prepare("UPDATE bridge_injector_items SET workflow_status = 'processing' WHERE id = ?").run(item.id);
...
async function tickStrategicCampaigns(db) {
  const isStrategicActive = getSetting('strategic_campaigns_scheduler_active') !== 'false';
  if (!isStrategicActive) return;

  try {
    const activeStrategicCampaigns = db.prepare("SELECT * FROM strategic_campaigns WHERE status = 'running'").all();
    if (activeStrategicCampaigns.length === 0) return;

    const stratLogFile = path.join(process.cwd(), 'public', 'strategic_campaign_logs.txt');

    await logContextStorage.run(stratLogFile, async () => {
      for (const campaign of activeStrategicCampaigns) {
        const items = listStrategicCampaignItems(campaign.id);
        if (items.length === 0) continue;
        ...
        // Transition finished items to 'completed' workflow_status
        for (const currentItem of items) {
          if (currentItem.workflow_status === 'production_processing' && isStrategicItemFinished(currentItem, campaign)) {
            updateStrategicCampaignItem(currentItem.id, { workflow_status: 'completed' });
            currentItem.workflow_status = 'completed';
          }
        }

        for (const currentItem of items) {
          if (isStrategicItemFinished(currentItem, campaign)) {
            continue;
          }

          hasActiveItem = true;
          const taskKey = `strategic_${currentItem.id}`;
          if (state.activeTasks.has(taskKey)) {
            continue;
          }

          // 1. Generation Stage (Call 1 Creative Script & Storyboard + Call 2 Publishing + Start Frame T2I)
          if (currentItem.generation_status === 'pending' || currentItem.workflow_status === 'draft' || currentItem.workflow_status === 'ready_for_call_1') {
            // Check if all previous items have completed Fase 1
            const itemIdx = items.findIndex(item => item.id === currentItem.id);
            const prevItemsInFase1 = items.slice(0, itemIdx).some(prevItem => 
              prevItem.generation_status !== 'completed'
            );

            if (prevItemsInFase1) {
              continue; // Wait for previous items to complete Fase 1
            }
            runStep(currentItem.id, 'strategic_generator', processStrategicGenerator, taskKey);
            continue;
          }
          ...
          if (currentItem.generation_status === 'completed' && currentItem.tts_status === 'pending') {
            ...
            if (pauseAt === 'tts') {
              console.log(`⛔ [Strategic Scheduler] Paused before TTS for item #${currentItem.id} — campaign "${campaign.campaign_name}". Menunggu approval manual.`);
              continue;
            }
            runStep(currentItem.id, 'strategic_tts', processStrategicTts, taskKey);
            continue;
          }

          // 3. Visuals Stage — Phase A: Submission & Phase B: Polling
          if ((currentItem.tts_status === 'completed' || currentItem.tts_status === 'skipped') && currentItem.visual_status === 'pending') {
            if (currentItem.workflow_status === 'ready_for_review') {
              continue;
            }
            if (pauseAt === 'visuals') {
              console.log(`⛔ [Strategic Scheduler] Paused before G-Labs for item #${currentItem.id} — campaign "${campaign.campaign_name}". Review storyboard & prompt.`);
              continue;
            }
            runStep(currentItem.id, 'strategic_glabs', processStrategicGlabs, taskKey);
            continue;
          }
          if (currentItem.visual_status === 'processing') {
            runStep(currentItem.id, 'strategic_glabs_poll', processStrategicGlabs, taskKey);
          }

          // 4. FFmpeg Muxing Stage
          if ((currentItem.visual_status === 'completed' || currentItem.visual_status === 'skipped') && currentItem.ffmpeg_status === 'pending') {
            if (pauseAt === 'ffmpeg') {
              console.log(`⛔ [Strategic Scheduler] Paused before FFmpeg for item #${currentItem.id} — campaign "${campaign.campaign_name}". Review visual clip.`);
              continue;
            }
            runStep(currentItem.id, 'strategic_ffmpeg', processStrategicFfmpeg, taskKey);
            continue;
          }
          ...
          if ((currentItem.ffmpeg_status === 'completed' || currentItem.ffmpeg_status === 'skipped') && currentItem.social_post_status === 'pending') {
            if (pauseAt === 'social') {
              console.log(`⛔ [Strategic Scheduler] Paused before Social Post for item #${currentItem.id} — campaign "${campaign.campaign_name}". Konfirmasi sebelum posting.`);
              continue;
            }
            runStep(currentItem.id, 'strategic_social_poster', processStrategicSocialPoster, taskKey);
            continue;
          }
        }

        if (!hasActiveItem) {
          // Auto-retry pass for failed items
          const needsSocial = campaign.post_youtube_draft || campaign.post_tiktok_draft || campaign.post_facebook_draft;
          const actualNeedsSocial = needsSocial && campaign.enable_social_post !== 0;
          ...
          if (failedItem) {
            ...
            if (failedStep) {
              updateStrategicCampaignItem(failedItem.id, updateFields);
              console.log(`🔄 [Strategic Scheduler] Campaign "${campaign.campaign_name}": Memulai Auto-Retry #${updateFields.retry_count} untuk Item #${failedItem.id} pada langkah ${failedStep.toUpperCase()}.`);
              continue;
            }
          }
        }
    ...
    console.error('[Strategic Scheduler Error]:', err.message);
  }
}
