...
async function processBridgeBulkSync(itemId, db) {
  ...
  const rowCounter = db.prepare("SELECT COUNT(*) as cnt FROM bridge_injector_items WHERE campaign_id = ? AND id <= ?").get(campaignId, itemId);
  ...
  try {
    if (fs.existsSync(itemDir)) {
      ...
      console.log(`[Bridge Bulk Scheduler] Cleaned up local temp directory for item #${itemId}`);
    }
  } catch (cleanErr) {
    console.warn(`[Bridge Bulk Scheduler] Failed to cleanup local files for item #${itemId}: ${cleanErr.message}`);
  }
