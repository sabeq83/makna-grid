...
async function processBridgeBulkDownload(itemId, db) {
  ...
  if (isNextcloudFolder) {
    ...
    localClip1 = await downloadNextcloudFile(f => isVideoFile(f.filename) && (f.basename.toLowerCase().includes('clip_1') || f.basename.toLowerCase().includes('clip1') || f.basename.toLowerCase().endsWith('1.mp4')), 'video_clip_1.mp4');
...
`;
  ...
  db.prepare(`
    ...
    WHERE id = ?
  `).run(injected_vo_1, injected_vo_2, injected_vo_3, injected_vo_4, clip2_t2i_prompt, clip2_i2v_prompt, relativeMdPath, itemId);
...
async function processBridgeBulkSync(itemId, db) {
  ...
  const { uploadFileToNextcloud } = await import('./nextcloud-helper.js');

  const cleanParent = (item.nextcloud_folder || 'MAKNA_Bridge_Bulk').replace(/\/+$/, '');
  ...
  if (item.injected_script_md_path) {
    ...
    if (fs.existsSync(absMdPath)) {
      await uploadFileToNextcloud(absMdPath, `${cleanParent}/naskah_baru.md`);
    }
  }
  ...
  if (item.clip2_t2i_image_path) {
    ...
    if (fs.existsSync(absImagePath)) {
      ...
      await uploadFileToNextcloud(absImagePath, `${cleanParent}/start_frame_clip2_baru.${imageExt}`);
    }
  }
  ...
  if (item.clip2_video_path) {
    ...
    if (fs.existsSync(absVideoPath)) {
      await uploadFileToNextcloud(absVideoPath, `${cleanParent}/video_clip_2_baru.mp4`);
    }
  }

  const absFinalVideoPath = path.join(itemDir, 'bridging_video_final.mp4');
  if (fs.existsSync(absFinalVideoPath)) {
    await uploadFileToNextcloud(absFinalVideoPath, `${cleanParent}/bridging_video_final.mp4`);
  }
  ...
  if (item.enable_tts === 1) {
    ...
    for (const cNum of [2, 3, 4]) {
      ...
      if (fs.existsSync(absAudioPath)) {
        await uploadFileToNextcloud(absAudioPath, `${cleanParent}/audio_clip_${cNum}_baru.${ext}`);
      }
    }
  }
