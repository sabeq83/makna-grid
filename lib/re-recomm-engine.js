...
import { uploadVideoToGemini } from './gemini.js';
...
export async function runSourcingGroundingPipeline(jobId) {
  const db = getDb();
  const job = db.prepare("SELECT * FROM re_plus_recomm_jobs WHERE id = ?").get(jobId);
  if (!job) return;
  ...
  if (!apiKey) {
    db.prepare("UPDATE re_plus_recomm_jobs SET status = 'failed' WHERE id = ?").run(jobId);
    throw new Error('No active API Key found for Gemini Grounding');
  }

  db.prepare("UPDATE re_plus_recomm_jobs SET status = 'scraping' WHERE id = ?").run(jobId);
  ...
  for (const url of urls) {
    ...
    try {
      ...
      db.prepare("UPDATE re_plus_recomm_jobs SET status = 'analyzing' WHERE id = ?").run(jobId);
  ...
  db.prepare("UPDATE re_plus_recomm_jobs SET status = 'completed' WHERE id = ?").run(jobId);
}
...
export async function updateRecommendedProduct(outputId, updateData, uploadedFile = null) {
  const db = getDb();
  const currentData = db.prepare("SELECT * FROM re_plus_recomm_outputs WHERE id = ?").get(outputId);
  if (!currentData) {
