...
export async function tickCloudSync() {
  ...
  try {
    ...
    if (data.success && Array.isArray(data.jobs)) {
      ...
      for (const job of data.jobs) {
        ...
        let exists = false;
        if (job.type === 'RE') {
          const check = db.prepare('SELECT id FROM re_campaigns WHERE id = ?').get(localCampaignId);
          exists = !!check;
        } else if (job.type === 'OPC') {
          const check = db.prepare('SELECT id FROM pillar_campaigns WHERE id = ?').get(localCampaignId);
          exists = !!check;
        }
  ...
  try {
    const runningReCampaigns = db.prepare(`
    ...
    for (const campaign of runningReCampaigns) {
      ...
      const items = db.prepare('SELECT * FROM re_campaign_items WHERE campaign_id = ?').all(campaign.id);
      ...
      if (item.ffmpeg_status === 'completed' && item.drive_link) {
        ...
        try {
          ...
          if (res.ok) {
            db.prepare("UPDATE re_campaigns SET status = 'completed' WHERE id = ?").run(campaign.id);
        ...
        if (retryCount >= 3) {
          ...
          try {
            ...
            if (res.ok) {
              db.prepare("UPDATE re_campaigns SET status = 'failed' WHERE id = ?").run(campaign.id);
  ...
  try {
    const runningPillarCampaigns = db.prepare(`
    ...
    for (const campaign of runningPillarCampaigns) {
      const jobId = campaign.id.replace('cloud_', '');
      const items = db.prepare('SELECT * FROM pillar_campaign_items WHERE campaign_id = ?').all(campaign.id);
      if (items.length === 0) continue;
      ...
      if (item.ffmpeg_status === 'completed' && item.drive_link) {
        ...
        try {
          ...
          if (res.ok) {
            db.prepare("UPDATE pillar_campaigns SET status = 'completed' WHERE id = ?").run(campaign.id);
        ...
        if (retryCount >= 3) {
          ...
          try {
            ...
            if (res.ok) {
              db.prepare("UPDATE pillar_campaigns SET status = 'failed' WHERE id = ?").run(campaign.id);
